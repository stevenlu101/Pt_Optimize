...
同类隐患：②′ 的限值同样是 0，同样没有裕度。目前实测 $+1.6\sim+1.8\ \mathrm{W}$ 离 0 够远
...
说明**慢模式不在抽热向量上**。Anderson 加速或对「抽热 $\leftrightarrow$ 管根」
这个低维环做 Newton 是下一步的候选。
...
| 判据 | 限值 | 管壁 0.8 | 管壁 0.6 |
|---|---|---|---|
| ① 升温 空管到目标 | 72 h | 0.06 h | 0.08 h |
| ②″ 圆盘区最高温 − 管温 | 5 K | 0.96 K（裕度 81 %） | 1.17 K（77 %） |
| ②′ 管孔净流入 | $>0$ W | $+1.79$ W | $+1.63$ W |
| ③ 法兰增量温降 | 10 K | 5.38 K（46 %） | 6.29 K（37 %） |
...
| 项目 | 局限 | 影响 |
|---|---|---|
| 外层耦合 | 环路增益 0.96，慢模式未被加速手段解决 | 剩余误差 0.65–0.75 K；③ 携带 $\pm0.7$ K |
