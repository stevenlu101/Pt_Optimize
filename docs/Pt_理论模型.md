...
**结果**：给出两档全判据通过的定案构型，整线总铂重 **3106 g**（管壁 0.8 mm，
每条判据都有裕度）与 **2388 g**（管壁 0.6 mm，被焊接烧穿下界与管电流密度同点咬住），
...
| 判据 | 限值 | 管壁 0.8 | 管壁 0.6 |
|---|---|---|---|
| ① 升温 空管到目标 | 72 h | 0.057 h | 0.078 h |
| ②″ 圆盘区最高温 − 管温 | 5 K | 1.050 K（裕度 79 %） | 1.186 K（76 %） |
| ②′ 管孔净流入 | $>0$ W | $+1.35$ W | $+1.58$ W |
| ③ 法兰增量温降 | 10 K | 5.52 K（45 %） | 5.97 K（40 %） |
...
| | 管壁 0.8 | 管壁 0.6 |
|---|---|---|
| 板厚（入口/共用1/共用2/出口）mm | 2.11 / 3.33 / 3.12 / 1.76 | 1.82 / 2.85 / 2.66 / 1.49 |
| 环内级倍率 $\mu$ | 1.22（四片同） | 1.20（四片同） |
| 舌保温（四片）mm | 18.7 / 1.6 / 1.4 / 3.9 | 同左 |
| 管（三段）g | 2466 | 1842 |
| 法兰（四片）g | 641 | 547 |
| **合计 g** | **3106** | **2388** |
| 相对现状基准 7141 g | $-56\ \%$ | $-66\ \%$ |
| 外层耦合剩余误差估计 | 0.42 K | 0.54 K |
...
6. **给出两档全判据通过的定案**：3106 g（管壁 0.8，处处有裕度）与
   2388 g（管壁 0.6，被焊接下界与管 J 同点咬住），相对现状基准省 56 % 与 66 %。
