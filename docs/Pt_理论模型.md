...
**结果**：给出两档全判据通过的设计记录构型，整线总铂重 **3547 g**（管壁 0.8 mm）与
**2656 g**（管壁 0.6 mm，被焊接烧穿下界与管电流密度同点咬住），
相对现状基准 7141 g 分别省 **50 %** 与 **63 %**。两档均给出可加工的 3DM 几何，
并以逐件质量对账验证几何与计算模型一致（差 $+0.1\ \%$）。

> **2026-08-17 修订。** 本文初版给出的 3106 g / 2388 g 两档**在装配上不成立**：
> 舌片长 90 mm，扣掉圆盘切点 26 mm 与压接段 40 mm，自由段只剩 **24 mm**，
> 铜排（长 100、宽 60–80 mm）装不上。
>
> 修订做了三件事：
> (i) 把装配可行性升为**硬判据 ⑤**（自由段 $\ge 100$ mm）；
> (ii) 把舌长从自由变量改为**由装配式子算出的因变量**
>      $L_\text{舌}=\sqrt{R^2-w^2}+L_\text{压接}+L_\text{自由}$；
> (iii) 重写定尺寸器（下述第 4 点）。
>
> 代价是 $+441$ g / $+268$ g（约 $+14\ \%$ / $+11\ \%$）。
> 本文其余章节的模型、闭式与方法论结论**均不受影响**，受影响的只有设计记录数值本身。

4. **判据体系与控制律必须同构。** 初版控制律把三个旋钮分派给 ②′ 与 ②″，
   **③ 没有任何旋钮** —— 在初版几何上从未暴露（那里 ③ 天然为 5.5），
   换形状后 ③ 停在 135（限 10）十余轮不动。
   修订版据实测闭式 $③=\gamma D,\ \gamma=2.40\ \mathrm{K/W}$ 把 ②′ 与 ③ 合并为
   **抽热 $D$ 的一个区间** $0<D\le ③_\text{max}/\gamma$，
   并把该区间交给**不花铂的舌保温**去守，板厚只做接力与省铂。
...
2. **几何只有一个来源。** 设计记录值出自 `DesignSpec`。曾有辅助命令各自钉着几代之前的几何，
...
偏离设计记录点的算例收益最大：「管保温 1 mm」档纯 Picard 需 **734 轮／17.4 min**，
Anderson **109 轮／2.7 min**。

### 8.2.2 加速器暴露了一个更严重的问题：收敛判据本身是代理量

装上 Anderson 后出现一个无法两全的现象：**同一个算例，两个迭代格式都自称收敛，
③ 却差 12.5 K**（纯 Picard 报 6.18，Anderson 报 18.70），而两者的**无法兰基线完全相同**
—— 差的是解本身。最多只有一个是对的。

原因在收敛判据。式 (8.2) 的 $\Delta_\infty\approx\delta/(1-g)$ 用**步长** $\delta$
估计到不动点的距离，这个换算只在**线性定常**迭代下成立。Anderson 是非定常的：
外推可以让 $\delta\to0$ 而 $x$ 并不在不动点上（Anderson 的经典停滞模式）。

$$\text{正确的度量是残差本身：}\quad
\bigl\|F(x)\bigr\|_\infty=\bigl\|G(x)-x\bigr\|_\infty
\;\xrightarrow{\;\times\,1/(1-g)\;}\;\text{到不动点的距离}
\qquad\text{(8.4)}$$

$G(x)$ 与 $x$ 每轮都是现成的，**没有任何理由再用步长做代理**。
改判式 (8.4) 之后（要求 $\|F\|_\infty\cdot 25<\mathrm{tol}$），两个格式落到同一点：

| 构型（旧板厚 2.11/3.40/3.18/1.76） | ③ | 轮数 | 用时 |
|---|---|---|---|
| Anderson，tol 1.0 | 18.70 | 19 | 0.5 min |
| Anderson，tol 0.2／2000 轮 | 17.77 | 22 | 0.6 min |
| **纯 Picard，tol 0.2／2000 轮** | **17.68** | **579** | **12.1 min** |

两条互不相干的迭代路径在收紧容差后**差 0.09 K**，真残差 0.004–0.008 K。

**这条结论的代价**：该几何的真实 ③ 是 17.7 K，而不是记录里的 6.18 K —— 它**根本不满足
10 K 的限值**。定尺寸器本身没错，它只是被喂了一个假的 ③；用式 (8.4) 重跑同一条控制律，
**再走一轮**就落到可行点（第 9 章的设计记录值即由此得到）。

> **方法论上这是「代理量不是原量」的第五次发作，也是最贵的一次。**
> 前四次分别是：拿段内最大偏差当管根、拿 ②′ 当 ③ 的靶、拿会切换分支的标量当收敛度量、
> 拿包围盒当几何校验量。这一次的隐蔽之处在于：代理量 $\delta$ 在**原来的**迭代格式下
> 确实等价于残差，是**换了格式之后**才失效的 ——
> 而没有第二条独立路径，失效永远不会自己暴露。
...
**保守取 25 倍放大**，绝不用 $\delta$ 本身。两档设计记录的剩余误差估计为
...
| 判据 | 限值 | 管壁 0.8 | 管壁 0.6 |
|---|---|---|---|
| ① 升温 空管到目标 | 72 h | 0.058 h | 0.073 h |
| ②″ 圆盘区最高温 − 管温 | 5 K | $-0.208$ K | $-0.284$ K |
| ②′ 管孔净流入 | $>0$ W | $+1.12$ W | $+0.82$ W |
| ③ 法兰增量温降 | 10 K | 5.18 K（裕度 48 %） | 6.48 K（35 %） |
| 管 J | 12 A/mm² | 9.51（21 %） | 10.96（9 %） |
| ⑤ 舌片自由段 | $\ge 100$ mm | 100.0（**0**） | 100.0（**0**） |
| 管壁 vs 焊接下界 | 0.6 mm | 0.8（33 %） | 0.6（**0**） |

**管壁 0.8 · 留余量**：咬住它的是**抽热窗口** —— ②′ 与 ③ 是同一个抽热 $D$ 的两侧，
$D$ 已落在区间偏安全的一侧，再削板厚就会转为往管里灌热。
**管壁 0.6 · 底档**：焊接烧穿下界（余量 0）与管 J（余量 9 %）**在同一点咬住** ——
这是可行域的底。

两档的 ⑤ 余量都是 0，这是**构造使然而非巧合**：舌长由装配式子算出，
最优解必然贴着下界（加长只多花铂多发热）。
...
| | 管壁 0.8 | 管壁 0.6 |
|---|---|---|
| 圆盘 / 舌片 mm | $\varnothing60$ / $140\times60$ | 同左 |
| 板厚（入口/共用1/共用2/出口）mm | 0.89 / 2.45 / 2.35 / 0.73 | 0.64 / 1.86 / 1.73 / 0.62 |
| 环内级倍率 $\mu$ | **1.00（不需要环）** | **1.00（不需要环）** |
| 舌保温（四片）mm | 0.4 / 0.4 / 0.5 / 0.3 | 0.4 / 0.4 / 0.4 / 0.6 |
| 管（三段）g | 2465 | 1841 |
| 法兰（四片）g | 1082 | 815 |
| **合计 g** | **3547** | **2656** |
| 相对现状基准 7141 g | $-50\ \%$ | $-63\ \%$ |

**管孔渐变环在新构型上消失了**，这值得单独记一笔：它不是被删掉的，是被
**加宽舌片**取代的。舌宽从 30 加到 60 mm 之后，电流从管孔进入法兰有足够的过流断面，
孔周不再拥塞，②″ 由 $+1.05$ K 降到 $-0.21$ K（限 $+5$），低了一个数量级。
⇒ 少一道两级台阶的机加工。**但这条结论绑在形状上**：舌片一窄回去，尖峰与环都会回来。

![图 9  铂重构成](fig/v5/f09_mass.png)

**两条值得单独指出的结构性结果**：

1. **共用片比端片厚 58–89 %**（3.33 vs 1.76 mm）。这是式 (3.8) 的 $\sqrt3$ 倍电流
...
6. **给出两档全判据通过的设计记录**：3547 g（管壁 0.8）与
   2656 g（管壁 0.6，被焊接下界与管 J 同点咬住），相对现状基准省 50 % 与 63 %。
   两档几何均以逐件质量对账验证，与计算模型差 $+0.1\ \%$。

6b. **判据体系必须与控制律同构，否则优化器在无人区里跑。**
   初版控制律只覆盖 ②′ 与 ②″，**③ 无旋钮**；在初版几何上不暴露（③ 天然为 5.5），
   换形状后 ③ 停在 135（限 10）十余轮不动。
   闭式 $③=\gamma D$（实测 $\gamma=2.40\ \mathrm{K/W}$，跨 $D\in[0.6,144]$ W 吻合 5 % 以内）
   把 ②′ 与 ③ 合并成**抽热 $D$ 的一个区间**，使「该形状有没有解」成为可判定的问题：
   $30$ mm 宽、$166$ mm 长的舌片被判**无解**（板厚顶到上限、保温压到下界，
   仍有 $-52$ W 灌入管子），这是两个旋钮**同时饱和**，不是迭代不足。

6c. **装配可行性是判据，不是事后检查。** 初版设计记录的舌片自由段只有 24 mm，
   铜排装不上，而它通过了当时的全部五条判据、出了 3DM 与论文。
   根因是舌长在程序里是个**谁都不核对的自由常数**。
   修订后舌长 $=\sqrt{R^2-w^2}+L_\text{压接}+L_\text{自由}$，由装配算出；
   代价 $+14\ \%$ / $+11\ \%$ 铂重，且这笔代价**线性挂在铜排尺寸这个输入上**。
...
| 项目 | 局限 | 影响 |
|---|---|---|
| 外层耦合 | 环路增益 0.96；Anderson 已把设计记录点压到 9 轮，但**极端偏离档仍可能超出默认 200 轮** | 剩余误差 0.3–0.6 K；③ 携带 $\pm0.6$ K |
...
- 几何：`Core/DesignSpec`（设计记录几何的唯一来源），交付 `.3dm` 由 `--cli --make3dm` 生成并自校
...
- 全部数值与图：由 `Pt_Optimize` 按 `DesignSpec` 实时生成
