...
| 材料物性 | 铂电阻率、导热系数、比热、密度等 | 电阻率：鉑金電氣計算.xlsx 的二次拟合（0–1500 °C）；导热系数：Touloukian/TPRC 与 Kaye & Laby 的推荐值一支，0–700 °C 与 Kaye & Laby 一致到 2.5 %，**700 °C 以上为推荐值外推、无本项目实测，带宽 ±8 %**；比热：Kaye & Laby 纯铂行 0/100/300/500 °C = 132/135/141/146 J/(kg·K) 的四点最小二乘（2026-09-18 重定），**500 °C 以上为外推**；密度 21450 kg/m³ |
...
为什么不用「管 + 法兰」两节点模型的电流当尺寸依据：两节点里法兰自热会让法兰比管热、热倒灌回管子，管电流算少；而法兰电阻随板厚与温度变，闭式估计与场解可以差一个量级以上，设计电流会跳。尺寸依据不许被「法兰帮管子发热」省掉、也不许随法兰自己漂，所以取管子单独所需的电流（确定、闭式；它不是整线稳态段电流的上界——同一次整线判定里，空管到温的段电流比它大约 0.5 %；差的来源推断是法兰抽热与段间耦合不在它里面，未拆开验）；两节点的结果另算一份当对照，不进尺寸链。
...
| $x$ | 沿管轴的坐标 | m |
| $T$ | 管金属温度 | K（差值即 °C） |
| $k$ | 铂导热系数，71.6（0 °C）→ 83.8（1150 °C）→ 85.4（1300 °C）；700 °C 以上为推荐值外推，带宽 ±8 %（1150 °C 处 77.1…90.5） | W/(m·K) |
| $A$ | 管壁截面积 = π·(外径² − 内径²)/4 | m² |
| $\rho(T)$ | 铂电阻率 | Ω·m |
| $I$ | 管电流 | A |
...
- v6.1 修订（2026-09-16）：3.5 节设计电流一句「上界、确定、闭式」改为「确定、闭式」并注明它不是整线稳态段电流的上界（同一次整线判定：空管到温段 0 电流 1077.0 A 对设计电流 1072.0 A，+0.47 %；带玻璃段 0 +0.07 %）；截面 J 与升温判据的口径不变。v6.0 同句一并改。
