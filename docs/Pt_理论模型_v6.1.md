...
v6.1（2026-09-12）按业主 2026-09-11 的四条意见补齐：每条公式后面附符号表，逐个说明含义并标单位（文末附录汇总）；法兰给出「初始值 → 结果值」的并列比较，并说明中间的计算流程与算完之后各量所依赖的边界环境——铜排、保温、工况、供电、焊接（第 10.5 节）。同时补上计算出的温度场与电流密度场：现役记录一组，侧 Y 形舌板挖孔前／挖孔后各一组、互相佐证（第 10.6 节），以及侧 Y 形的 3DM 结果图（第 10.3 节）。

结果：现役设计记录「管壁 0.8 · 留余量」总铂重 3547 g（管 2465 + 法兰 1082，3 段 4 片）；在 2 段 3 片的对照工况下，形状搜索给出的最轻可行形状为盘 Ø56／舌宽 56，细网格重解后 2603.8 g，与用户手画的侧 Y 形（2741.9 g，导航网格上可行）对比省 5 %。所有判据表、图纸与本报告的图，都由程序从同一份设计记录生成。
...
![图 3　入口片立体示意（厚度方向放大 7 倍，板厚、环厚、管壁同倍）。一根实体铂管穿过法兰、两面露出（示意图上管端不画孔）；舌片与圆盘是同一块板切出来的；本档没有台阶，盘身与舌片同厚。](fig/v6/fig_iso.png)
...
为什么不用「管 + 法兰」两节点模型的电流当尺寸依据：两节点里法兰自热会让法兰比管热、热倒灌回管子，管电流算少；而法兰电阻随板厚变，第一轮闭式估计与第二轮场解可以差 20 倍，设计电流会跳。尺寸依据不许被「法兰帮管子发热」省掉、也不许随法兰自己漂，所以取管子单独所需的电流（上界、确定、闭式）；两节点的结果另算一份当对照，不进尺寸链。
...
恒流下某处温度升高 → 电阻率升高 → 该处发热更多，构成正反馈。把包保温的舌片视为「两端定温、通电、绝热」的杆，稳定条件与导热漏条件相除后长度与截面全部约掉，只剩材料物性（式子见 v5.0 第 4.5 节，此处只引结论）；纯铂在夹持温度整个范围内裕度 1.8～3.1，全区可行。判据表里以「整片热稳定」「局部热稳定」两条参考量印出（散热随温度涨得比发热快多少倍）。
...
判据表有两列口径：记录值是导航网格（2 mm）上算的；复核值是把网格一档档加密、直到数不再变之后的数。做决定看复核值。现役记录实测：法兰增量温降 4.72 K（粗网格）→ 10.33 K（0.25 mm；2026-09-02 在旧链口径下的加密复算，v6 链下这份记录只解了导航网格、未重跑加密复算），粗网格把孔边与焊脚那一圈的梯度抹平了，这个差足以把「过」变成「不过」，所以核算整线一定走到加密复算。
...
把这份记录按 v6.0 的因果链（v6.1 未改链）解一次（形状与旋钮照记录、不调）：热学判据全过——法兰增量温降 4.72 K（导航网格）、管孔净流入 1.12 W、圆盘区最高温 − 管温 −0.21 K；但**「法兰截面 J」27.7 A/mm² > 11 不过**（出口片舌片，x = −100 处）。记录自己的升温峰值电流是 1214 A（管 J 9.507 A/mm² × 管截面 127.7 mm²），共用片 √3 倍 = 2102 A；出口片舌片 0.73 × 60 = 43.8 mm² ⇒ 1214/43.8 = 27.7 A/mm²。记录里舌片厚等于板厚 0.73～2.45 mm，是旧链的产物；按第 3.6 节的闭式，舌宽 60 的端片舌片至少 1214/(10 × 60) ≈ 2.02 mm（向上落格 2.03）、共用片 2102/(10 × 60) ≈ 3.50 mm（3.51）。这就是业主 2026-09-08 那句「J 20～37 已完全不可用，该加截面就要加」在数字上的样子。第 10.5 节的三个案例都是在新链下从约束盒下角解出来的，不再借记录的厚度。
...
| 案例 | 结果 | 合计 g |
|---|---|---|
| 盘 Ø56／舌宽 56／舌长 140（搜形状最轻） | 导航网格可行 → 加密复算 0.125 mm 不过 → 细网格重解后可行（累计 105 分） | 2603.8 |
| 盘 Ø58／舌宽 58 | 导航网格可行，加密复算 0.25 mm 通过 | 2615.8 |
| 用户手画侧 Y 形（Ø56、舌 146×30 锥形、舌根槽） | 形状固定、旋钮自由：导航网格上可行（未做细网格重解） | 2741.9 |
...
![图 11　现役记录入口片的实体 3DM（APP「导出设计记录 3DM」那条路写的，Rhino 8 着色）：一根空心铂管穿过实体法兰，管口那一圈是壁厚。](fig/v6/rhino_inlet_solid.png)

![图 12　用户手画的侧 Y 形舌板交给求解器定旋钮后出的整机 3DM（2 段 3 片），Rhino 8 着色：舌根一条圆角三角槽，槽两侧是按 J 加厚的叉臂（深色那一段）。出图记录里「环外级 r28.8–31.8 实体创建失败」指的是这一级落在 Ø56 盘外、本就为空；这份 3DM 只作形状图，没有做第 10.4 节的逐件重量对账。](fig/v6/rhino_ysketch_line.png)

![图 13　同一 Y 形的单片正视：锥形舌片、圆角三角槽（宽端朝盘）、叉臂带、圆盘与管孔。槽的位置由电流场定（第 7.3 节），不是画上去的。](fig/v6/rhino_ysketch_front.png)
...
求解器不接受任何初值：「初始值」就是约束盒的下角（第 7.1 节）；工程师画在图上的厚度只用来「解一次」看判据，不当起点。以形状搜索给出的盘 Ø56／舌宽 56（2 段 3 片对照工况，管壁 0.8、管保温 10）为例，逐步是：
...
3. **约束盒下角**：板厚 = max(焊接下界 0.60, 圆盘侧截面下界) = 0.60／1.02／0.60 mm；舌保温 0.30 mm（裸舌）；环倍率 1.00；槽 0°、孔 0。传进来的旋钮值一个都不用。
4. **第 1 轮场解**（导航网格 2 mm）：先验下角三片都不熔；按当地电流方向与温度场把槽心、孔心写回（第 7.3 节）；判据表——法兰增量温降不过。
5. **分配表 → 旋钮**（第 7.2 节）：法兰增量温降的第一候选是不花铂的舌保温，逐片二分求根 → 第 2 轮 5.0／2.8／8.6 mm；第 3 轮 5.1／2.8／8.6 mm 全过 ⇒ 停：只往上走过，这就是最小的可行点。导航网格共 72 次场解、31.6 分钟，合计 2603.4 g。
6. **加密复算**（第 7.4 节）：1.0 → 0.5 → 0.25 → 0.125 mm，法兰增量温降 11.01 → 11.13 → 11.52 → 11.76 K，越限——粗网格上的「过」在细网格上不成立。
7. **细网格重解**（0.125 mm）：仍从下角出发、旋钮只增不减：舌保温 5.2／2.9／8.8 mm，全判据过，2603.8 g。重解本身 61.4 分钟（含它自己再跑一遍导航网格三轮）；从头累计 125 次场解、105.2 分钟。
8. **出图与重量对账**（第 10.4 节），差 ≤ 2 % 才是交付件。

侧 Y 形（挖孔族）多两步：舌孔／槽是旋钮，按场定位置后各抬到上界量一次（第 7.3 节）；切口带内按 I/(J·带内最窄宽) 加厚成叉臂（第 3.6 节），带外还是杆厚。

### 10.5.2 初始值 → 结果值：三个案例并列

| 项 | 盘 Ø56／舌宽 56（搜形状最轻） | 侧 Y 形，挖孔后（用户手画） | 同一锥形舌片，不开槽（挖孔前） |
|:------|:--------|:----------|:--------|
| 形状（固定） | Ø56，舌 140 × 56 等宽 | Ø56，舌 146 × 30 锥形，舌根圆角三角槽（宽端朝盘） | Ø56，舌 146 × 30 锥形，无槽 |
...
| 舌保温 mm（旋钮） | 0.30 → **5.2／2.9／8.8** | 3.0／3.0／3.0 → **5.7／3.6／11.7** | 0.30 → **9.3／6.3／18.2** |
| 环倍率（旋钮） | 1.00 → 1.00 | 1.00 → 1.00 | 1.00 → 1.00 |
...
| 场解次数／耗时 | 累计 125 次／105.2 分（导航 31.6 + 加密 12.2 + 重解 61.4） | 64 次／25.9 分 | 89 次／36.3 分 |
| 判据在哪张网格上成立 | 细网格 0.125 mm | 导航网格 2 mm | 导航网格 2 mm |

三点读法：板厚在三个案例里都停在下角——按 J 定截面之后，热学缺口全由不花铂的舌保温补（第 7.5 节）；用户图上的初始值 3 mm 舌保温解一次就把法兰增量温降推到 90.8 K，求解器把它抬到 5.7～11.7 mm 才过；同一锥形舌片开槽比不开槽轻 13.2 g、舌保温薄约四成，机理见第 10.6 节。Y 形比 Ø56 等宽舌重 138 g，几乎全来自舌片厚：锥形舌最窄处（压接段边界，宽 37 mm）按 I/(J·w_min) 定了整根杆厚 2.64／4.57 mm，比等宽舌的 1.75／3.03 厚 51 %（解析约 +374 g）；锥形少掉的面积省回约 217 g；槽挖掉减叉臂补回净省 13 g。舌片若能随宽度分级减薄，这 138 g 大部分能省回来——这是形状搜索没搜的一维，本报告未做。另：第二、三列的判据只在导航网格 2 mm 上成立（未做加密复算与细网格重解），不可直接交付；只有第一列在细网格上过。
...
![图 14　现役记录入口片的温度场（导航网格，°C）。舌端 450 °C 是铜排夹持温度，从舌端往管孔升温，最高 1234.7 °C 出在舌片近盘处（舌片自热，比管温高 85 K）；圆盘区与管温齐平（圆盘区最高温 − 管温四片最紧 −0.21 K，出在 HC2|HC3）。](fig/v6/field_rec_入口_T.png)

![图 15　现役记录入口片的电流密度场（A/mm²）。电流从舌片单侧汇入管孔：舌根两个圆角与管孔近舌一侧最挤，圆盘背对舌片的那一半几乎不走电流——槽就该开在那里（第 7.3 节）。入口片 J 峰 34.8 A/mm²（色标按 99.5 % 分位截断，图上最亮约 32）；「法兰截面 J」不过的那一刀在出口片 x = −100 处（27.7 A/mm²）。](fig/v6/field_rec_入口_J.png)

![图 16　现役记录共用片（HC1|HC2，承 √3 倍电流）的温度场。](fig/v6/field_rec_HC1_HC2_T.png)

![图 17　现役记录共用片的电流密度场（J 峰 20.7 A/mm²，板厚 2.45 把 J 压到端片的六成）。](fig/v6/field_rec_HC1_HC2_J.png)

侧 Y 形的两组场图取共用片（HC1|HC2，承 √3 倍电流、最不利的一片），挖孔前是同一锥形舌片不开槽（第 10.5 节第三列），挖孔后是求解器定完槽与叉臂的结果（第二列）：

![图 18　挖孔前：共用片温度场。舌片是一整块导热通路，从舌端 450 °C 到管孔单调升温；要守住法兰增量温降 9.97 K，舌保温得包到 6.3 mm。](fig/v6/field_Y_before_HC1_HC2_T.png)

![图 19　挖孔后：共用片温度场。舌片中段的槽（白色空洞）去掉了那一块两面的散热面，求解器把舌保温减到 3.6 mm 仍守住 9.71 K。](fig/v6/field_Y_after_HC1_HC2_T.png)

![图 20　挖孔前：共用片电流密度场。电流在舌根均匀铺开，J 峰 10.85 A/mm² 在舌盘交界。](fig/v6/field_Y_before_HC1_HC2_J.png)

![图 21　挖孔后：共用片电流密度场。槽把舌片中段切成两条臂，电流挤进臂里，场峰 11.29 A/mm² 移到臂上（诊断量）；叉臂按 I/(J·带内最窄宽) 加厚到 6.58 mm（杆 4.57），闭式截面 J 9.998 ≤ 11（判据）。](fig/v6/field_Y_after_HC1_HC2_J.png)

![图 22　挖孔前：入口片电流密度场（J 峰 11.70 A/mm²）。](fig/v6/field_Y_before_入口_J.png)

![图 23　挖孔后：入口片电流密度场（J 峰 12.17 A/mm²），臂上的集中与共用片同一形态。](fig/v6/field_Y_after_入口_J.png)

两张场图对同一件事各说一半，这就是「互相佐证」的意思：

- **电流场说明槽为什么不伤截面**：槽开在舌片中段（x −88～−31），锥形舌在那里已经变宽，闭式 J 只有 7～9 A/mm²；开槽后电流挤进两条臂，叉臂按 I/(J·带内最窄宽) 加厚，把最紧一刀的截面 J 钉在 9.998 ≤ 11——这是尺寸判据。场图里的 J 峰（10.85 → 11.29 A/mm²，入口片 12.17）是单元值，随网格变、在孔缘与圆角有局部集中，只是诊断量，用来看电流往哪里挤、给场定孔位用，不设门；它超过 11 不是判据不过，是两种量不能互比。
- **温度场说明槽为什么省铂**：两案例都停在法兰增量温降刚好低于 10 K 的最小可行点（9.97 与 9.71 都是二分落点，不能相减当效果）。开槽的效果体现在旋钮上：同一锥形舌片，舌保温从 9.3／6.3／18.2 降到 5.7／3.6／11.7 mm 仍过。机理：叉臂按 J = 10 补回了截面，导热截面只减 2～3 %，谈不上切断；同保温下少抽热主要是槽去掉了那一块两面的散热面（约 2 × 894 mm²），外加叉臂略高的自热——这一效应本报告没有单独量过（没做「同保温」对照解），表里各片抽热 W 是保温减薄与开槽两者的合成，共用片 3.21 → 3.66 W 升是保温 6.3 → 3.6 的结果，不能读成「抽热变少」。铂重上省的 13.2 g（逐片约 3.5／6.0／3.5）是槽挖掉的料（约 51／88 g）减去叉臂加厚补回的料（约 47／81 g）。
- **孔不能替代保温**：挖孔后舌保温仍要 3.6～11.7 mm；槽是「少抽热」的旋钮，主力仍是不花铂的舌保温（第 8.4 节）。

三片各自的数：

| 片 | 挖孔前 最高温 °C／J 峰 A/mm²／抽热 W／铂 g | 挖孔后 最高温 °C／J 峰 A/mm²／抽热 W／铂 g |
|:----|:---------|:---------|
