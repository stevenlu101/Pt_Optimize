...
在此之上建立了三样东西：一套有出处的判据体系与「判据分辨率」纪律；一条传导链（法兰增量温降的闭式、共用片的 √3 倍电流、抽热窗口的两侧）；一个从约束盒下角出发、旋钮只增不减、解与初值无关的求解器，以及能回答「哪种形状最省铂」的形状搜索。
...
结果：现役设计记录「管壁 0.8 · 留余量」总铂重 3547 g（管 2465 + 法兰 1082，3 段 4 片）；在 2 段 3 片的对照工况下，形状搜索给出的最轻可行形状为盘 Ø56／舌宽 56，细网格重解后 2603.8 g，用户手画的侧 Y 形（挖孔族）2741.9 g，导航网格上可行；两族程序不互比重量，这里只是并列。所有判据表、图纸与本报告的图，都由程序从同一份设计记录生成。
...
第 7.2 节的判据 → 旋钮分配表依据下面三条规则，每条都能从第 3、4 章的方程直接看出来：
...
1. 网格粗筛：几个盘径 × 几个舌宽比例，选出当前最好形状与改进方向（这是形状搜索的当前最好点，与旋钮求解器不吃初值是两回事）；工程师当前形状也当基准点算一遍。
...
用户 2026-09-10 手画的侧 Y 形舌板（舌根开一条到根的槽，形成两条臂）是挖孔族的一个自然成员：舌孔一开，舌片最窄有效宽变小，按 J 定截面要求切口那一段加厚，于是「杆 + 叉臂」的 Y 形自动出现。两条机理：

- 按 J 定截面之后，**把整片舌片均匀加厚再开孔只会更差**：截面被 J 钉死，多出来的料只是多抽热；只把切口带加厚成叉臂，孔才有效（第 3.6 节）。
- 孔不能替代保温：孔与槽是「少抽热」的旋钮，主力仍是不花铂的舌保温——挖孔后的 Y 形舌保温仍要 3.6～11.7 mm（第 10.5 节）。

在 2 段对照工况下，用户手画的 Y 形交给求解器定旋钮后 2741.9 g（导航网格上可行，未做细网格重解）；形状搜索给出的盘 Ø56／舌宽 56（不挖孔）细网格重解 2603.8 g。程序不替两族比重量，这里只把两个数并列供参考（口径不同）；两者都在「搜形状结果」下拉里，由工程师自己选。
...
把这份记录按 v6.0 的因果链（v6.1 未改链）解一次（形状与旋钮照记录、不调）：热学判据全过——法兰增量温降 4.72 K（导航网格）、管孔净流入 1.12 W、圆盘区最高温 − 管温 −0.21 K；但**「法兰截面 J」27.7 A/mm² > 11 不过**（出口片舌片，x = −100 处）。记录自己的升温峰值电流是 1214 A（判据「升温」那行的管 J 峰值 9.507 A/mm² × 管壁截面 127.7 mm²），共用片 √3 倍 = 2102 A；出口片舌片 0.73 × 60 = 43.8 mm² ⇒ 1214/43.8 = 27.7 A/mm²。记录里舌片厚等于板厚 0.73～2.45 mm，是旧链的产物；按第 3.6 节的闭式，舌宽 60 的端片舌片至少 1214/(10 × 60) ≈ 2.02 mm（向上落格 2.03）、共用片 2102/(10 × 60) ≈ 3.50 mm（3.51）。这就是业主 2026-09-08 那句「J 20～37 已完全不可用，该加截面就要加」在数字上的样子。第 10.5 节的三个案例都是在新链下从约束盒下角解出来的，不再借记录的厚度。

![图 5　现役记录按 v6.0 链解一次（导航网格 2 mm）的判据裕度：热学判据都有裕度，「法兰截面 J」越限。](fig/v6/chart_margins.png)
...
![图 7　形状搜索结果与用户手画 Y 形的铂重（同一对照工况；与 3 段现役记录不可直接比）。](fig/v6/chart_shapes.png)
...
![图 11　用户手画的侧 Y 形舌板交给求解器定旋钮后出的整机 3DM（2 段 3 片），Rhino 8 着色：从舌根伸到舌片中段的圆角三角槽，槽两侧是按 J 加厚的叉臂（深色那一段）。出图记录里「环外级 r28.8–31.8 实体创建失败」指的是这一级落在 Ø56 盘外、本就为空；这份 3DM 只作形状图，没有做第 10.4 节的逐件重量对账。](fig/v6/rhino_ysketch_line.png)
...
7. **细网格重解**（0.125 mm）：仍从下角出发、旋钮只增不减：舌保温 5.2／2.9／8.8 mm，全判据过，2603.8 g。重解本身 125 次场解、61.4 分钟（含它自己再跑一遍导航网格三轮）；从头累计约 202 次场解（导航 72 + 加密 5 + 重解 125）、105.2 分钟。
...
「③ 结果与出图」页的「配套清单」页签把第 3.5／4.3 节算出来的逐片量整理成现场要的答案：铜排截面按载流（I/J许用）与导热（Q·L/(k·ΔT)）取大、规格按舌端宽给宽×厚、夹持温度按参数表设定或按铜排热导那条边界算出、舌保温覆盖圆盘切点到压接段前、焊脚 = max(板厚, 壁厚)。「安装报告」页签与「导出安装报告」把整线概要、供电、法兰逐片、配套清单、保温、焊接与加工、升温与运行、判据表、出图文件、待现场确认写成一份 .md；三处同一个来源，判据没全过时首行写「不可作为安装依据」。
