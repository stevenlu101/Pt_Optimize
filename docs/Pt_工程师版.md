---
title: "铂金直接加热整线 —— 工程师版"
subtitle: "设计记录两档、四个旋钮各管什么、以及五个会骗人的坑"
...
> **⚠ 2026-08-17 重解。** 原来的设计记录舌片长 90 mm，自由段只有 24 mm ——
> **铜排根本装不上**。那不是余量不够，是设计上不成立。
> 于是新增硬判据 ⑤「舌片自由段 ≥ 100 mm」，并把舌长改成**由装配算出来**的量，
> 重新解出下面两档。代价是 **+441 g / +268 g**（约 +14 % / +11 %）。
> 全过程与九条一并修掉的 bug 见 `deliverable/设计记录重解_2026-08-17.md`。
...
- **舌长不是自由变量**（2026-08-17 起）。它 $=$ 圆盘切点 $+$ 压接段 $+$ 自由段下界，
  由装配算出来。旧设计记录的 90 mm 就是把它当自由变量、又从不与装配核对的结果：
  自由段只剩 24 mm，铜排装不上。
...
> 上表是**旧设计记录（$90\times30$ 舌片、3106 g）**那一次的对账，留在这里是因为
> 它讲的是「**校验量选错**」这条教训本身，与具体是哪一版设计记录无关。
> 新设计记录（$140\times60$、3547 / 2656 g）的 3DM 对账数值另出，方法完全相同。
...
| 想做什么 | 怎么做 |
|---|---|
| 看设计记录、用多少铂 | 「整线设计」页 $\to$ 设计记录 ▾ $\to$ **载入设计记录** $\to$ **核算整线** |
| 出图纸交加工 | 「整线设计」页 $\to$ 选档 $\to$ **导出设计记录 3DM** |
| 改参数试试 | 改左侧参数表或本页控件 $\to$ **核算整线**（分钟级，可取消） |

## 6.2 两个必须知道的行为

**「核算整线」走的是页面上的参数，但几何用的是和设计记录<u>完全同一套</u>构造器**
（2026-08-17 起）。所以「载入设计记录 → 核算整线」**能直接复现设计记录数字**。

本页没有控件的几项（渐变环、逐片舌保温、压接段、舌根圆角、角焊缝、等宽舌片）
按设计记录值参与求解，**每次核算完输出框都会把实际用值逐条列出来** ——
看不见又在起作用的量，是最容易出事的地方。

> 在此之前不是这样：页面自己造一片只有五个字段的简化法兰，
> 「核算整线」解的是**另一片法兰**，数字对不上设计记录，而判据表照样出得来。
> 那正是当初不得不单独做「▶ 复现设计记录」按钮的原因。

「▶ 复现设计记录」仍然保留，用途变了：它**完全不读页面控件**，
...
**求解侧已经解决**（2026-08-16 上了 Anderson 加速）：设计记录点从 60+ 轮降到 9 轮，
