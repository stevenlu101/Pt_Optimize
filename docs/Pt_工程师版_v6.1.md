...
![图 5　入口片立体示意（厚度方向放大 7 倍）：一根实体铂管穿过一片实体法兰（示意图上管端不画孔），舌片与圆盘是同一块板；本档没有台阶。](fig/v6/fig_iso.png)
...
![图 7　现役记录入口片的温度场（°C）。舌端 450 °C 是铜排夹持温度；最高 1234.7 °C 出在舌片近盘处（舌片自热，比管温高 85 K）；圆盘区与管温齐平。](fig/v6/field_rec_入口_T.png)
...
![图 9　用户手画的侧 Y 形交给求解器定旋钮后出的 3DM（2 段 3 片，Rhino 8 着色）：舌根圆角三角槽，槽两侧是按 J 加厚的叉臂。](fig/v6/rhino_ysketch_line.png)
...
![图 13　挖孔前：共用片温度场。舌保温 6.3 mm 才守住法兰增量温降 9.97 K。](fig/v6/field_Y_before_HC1_HC2_T.png)

![图 14　挖孔后：共用片温度场。槽去掉了那一块两面的散热面，舌保温 3.6 mm 就守住 9.71 K。](fig/v6/field_Y_after_HC1_HC2_T.png)

四张图合起来说的是：槽开在舌片中段，锥形舌在那里已经变宽、本来 J 就只有 7～9，所以不伤截面（臂按 J 加厚兜底）；槽去掉了一块散热面，同一锥形舌片的舌保温从 9.3／6.3／18.2 减到 5.7／3.6／11.7 mm 仍过，铂重省 13.2 g（2755.1 → 2741.9，槽挖掉的料减去叉臂补回的料）。两案例的法兰增量温降 9.97 与 9.71 都是求解器停在 10 K 之下的落点，不能相减当效果。孔不能替代保温：挖孔后舌保温仍要 3.6～11.7 mm。

![图 15　形状搜索与对照案例的铂重（同一对照工况；与 3 段现役记录不可直接比）。](fig/v6/chart_shapes.png)
...
| 舌保温 mm（旋钮） | 0.30 → **5.2／2.9／8.8** | 图上 3.0 → **5.7／3.6／11.7** | 0.30 → **9.3／6.3／18.2** |
...
| 场解次数／耗时 | 累计 125 次／约 105 分（导航 32 + 加密 12 + 细网格重解 61） | 64 次／26 分 | 89 次／36 分 |
| 判据在哪张网格上成立 | 细网格 0.125 mm | 导航网格 2 mm | 导航网格 2 mm |

读法：板厚三个案例都停在下角——按 J 定截面之后，热学缺口全由不花铂的舌保温补；你图上的 3 mm 舌保温解一次就把法兰增量温降推到 90.8 K，求解器抬到 5.7～11.7 mm 才过；Y 形比 Ø56 等宽舌重 138 g，几乎全来自舌片厚：锥形舌最窄处按 I/(J·w_min) 定了整根杆厚 2.64／4.57，比等宽舌的 1.75／3.03 厚 51 %；锥形少掉的面积省回一部分；槽与叉臂净省 13 g。第二、三列只在导航网格上过判据，未做细网格重解。中间的每一步（设计电流 → 舌片厚 → 下角 → 逐轮抬旋钮 → 加密复算 → 细网格重解）在理论模型 v6.1 第 10.5 节逐步列出。
