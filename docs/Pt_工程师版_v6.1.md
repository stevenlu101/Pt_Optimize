...
## 6.1 配套清单与安装报告（v6.1 补）

算完整线，「③ 结果与出图」页多出两个页签。**配套清单**逐片给：铜排截面（载流 I/J许用 与导热 Q·L/(k·ΔT) 取大）、铜排规格（宽 = 舌端宽，厚向上取到 0.5 mm）、铜排电流密度、压接段长、夹持温度（参数表给定，或按铜排热导算出）、铜排带走的热、舌保温厚度与覆盖范围（圆盘切点到压接段前；压接段被铜排夹住不包）、焊脚（= max(板厚, 壁厚)）；通用行给圆盘双面保温、管保温、端部额外保温、铜排到冷端长度与冷端温度、焊接方法与烧穿下界。**安装报告**十节：整线概要、供电、法兰逐片、配套清单、保温、焊接与加工、升温与运行、判据表、出图与文件、待现场确认；点「导出安装报告」存成 .md。判据没全过时报告第一行写「不可作为安装依据」。
