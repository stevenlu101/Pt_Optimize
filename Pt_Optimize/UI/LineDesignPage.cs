...
public sealed class LineDesignPage : TabPage
{
    ...
    //   两个数都从生产配方取（DesignSpec 的默认、WrapLimits 的上限），不在界面上另抄一份；
    //   步长本来就是每圈 0.5 mm。越界还有硬安全线「接合区保温缠得出来」兜底（控件只是第一道）。
    private readonly NumericUpDown _flInsT = Num((decimal)new DesignSpec().FlangeInsulMm, 0.0m,
                                                (decimal)WrapLimits.JointZoneMaxMm, (decimal)InsulationSearch.LayerMm, 1);
    ...
    /// <summary>
    /// ★★★★★ R27（用户 2026-09-10 原话：「UI 必须有输入铂金管加热段数的输入框」）。
    /// 段表此前只能靠在 <see cref="_segGrid"/> 里加行/删行改段数——够用但工程师容易漏找。
    /// 这个框与段表行数**双向同步**：改框 ⇒ 段表跟着加/删行（见 <see cref="SegCountChanged"/>）；
    /// 反过来在表里直接加/删行 ⇒ 框跟着变（见 HookAutoRun 里的 SegsChanged）。量程 1–8，整数。
    /// </summary>
    private readonly NumericUpDown _segCount = Num(3m, 1m, 8m, 1m, 0);
    ...
    private static string ShapeRowText((DesignSpec d, double mass, bool ok, string msg) r)
    {
        ...
        // R38：锥形也是搜出来的一维 —— 下拉条目要说清这一条是平行边还是锥形边，不然工程师
        //   选形状时看不出「省下来的那点铂是不是靠锥形」。
        string taper = r.d.TabTaper ? "　锥形" : "";
        string geo = $"{fam}盘Ø{2 * r.d.DiscRadiusMm:0}／舌 {r.d.TabLengthMm:0}×{2 * r.d.TabHalfWidthMm:0}{taper}";
    ...
    private void PickShape()
    {
        ...
        try
        {
            ...
            _tabTaper.Checked = r.d.TabTaper;   // R38：下拉里选的形状可能是锥形（ShapeRowText 现在会说），checkbox 要跟着走，
                                                 // 不然选中的条目写着「锥形」而页面上勾选框还是空的——赋了值却没有接到这个控件。
        }
        finally { _suppressAuto = false; }
    ...
    public LineDesignPage(DesignInputs baseInputs)
    {
        ...
        Row("电流密度 J A/mm²", _jDesign,
            "J 是**设定值**（预设 10，来自设计因果链）；**计算极限值 = J + 1**（预设 11）。\n" +
            "按 J 定舌片厚 = 设计电流 ÷ (J × 舌片最窄有效宽) 与圆盘各截面的板厚下界（约束盒下角），孔径／减重槽的上界也受它约束；\n" +
            "终验判据「法兰截面 J」全体 < J+1。调高 ⇒ 截面变薄、铂更省，但**实况风险由工程师承担**。\n" +
            "改了之后上一次的解作废，要重新核算。");
        ...
        // 2026-09-17 Opus 5：标题上就把现场限制说清（工程师不必去别处查为什么加不上去）。
        Row($"法兰保温厚 mm（≤{WrapLimits.JointZoneMaxMm:0.#}＝{WrapLimits.MaxTurnsAtJoint}圈）", _flInsT);
        Row("铜排夹持 °C", _clamp,
        ...
        // ★★★★★ R27（2026-09-10）：段表上方的段数输入框——与段表行数双向同步，
        //   见 SegCountChanged（这个框 ⇒ 段表）与 HookAutoRun 里的 SegsChanged（段表 ⇒ 这个框）。
        Row("铂金管加热段数", _segCount,
            "有几段直接加热的铂金管就填几——下面的段表行数会跟着变，也可以直接在段表里加/删行，这个框会跟着变回来。\n" +
            "加段：新段名 HCn、长度取「直接加热铂金管的长度」参数表默认值、控温点 = 上一段 − 70 °C（不低于 900）。");
        _segCount.ValueChanged += (_, _) => SegCountChanged();
    ...
    private void HookAutoRun()
    {
        void Watch(Control c)
        {
            ...
            // ★ R27（2026-09-10）：段数框自己已经接了 SegCountChanged（改框 ⇒ 段表加/删行，
            //   那一步再经 SegsChanged 触发 ParamChanged）——这里不再重复挂通用 ParamChanged，
            //   否则同一次改框会触发两次 ParamChanged（一次经 SegsChanged、一次经这里）。
            if (c == _segCount) return;
            switch (c)
            {
        ...
        void SegsChanged()
        {
            ...
            // ★★★★★ R27（2026-09-10）：段表行数变了——不管是谁改的（直接在表里加/删行，
            //   还是段数框自己的 SegCountChanged 加/删的）——段数框都要跟上。
            //   不跟上就是本仓最怕的「同一份状态两处来源」：表 4 行、框还显示 3。
            //   写法照抄 EnforceTabLenFloor：临时开 _suppressAuto，防这一次程序写值
            //   又被 SegCountChanged 当成「工程师在改」而回头去动段表。
            int segN = Math.Clamp(_segs.Count, (int)_segCount.Minimum, (int)_segCount.Maximum);
            if ((int)_segCount.Value != segN)
            {
                bool keepSuppress = _suppressAuto; _suppressAuto = true;
                _segCount.Value = segN;
                _suppressAuto = keepSuppress;
            }
    ...
    private void LoadDesignSpecFrom(DesignSpec fd, bool quiet)
    {
        ...
        //   落定，这里再回填就是最终会显示的那份。
        //   （合并 176acb7 与 75b72bb 时这两句在冲突块里被丢掉，走查 UiWiring §35「舌孔形状逐片 = 新记录」当场红 ⇒ 补回。2026-09-10）
        ShowTongues(fd);
        ShowDerived(fd);
    ...
    private static string AnalyticUsedWhat(DesignSpec d) =>
        ...
        + $"接合区上限 {WrapLimits.JointZoneMaxMm:0.#} mm = {WrapLimits.MaxTurnsAtJoint} 圈，{WrapLimits.SourceNote}）\r\n" +
    ...
    private LineCase BuildCase()
    {
        ...
        };
        {
            ...
            if (_shape is { } sh3)
            {
                ...
                // ★ R39（2026-09-11 边角料「.3dm 模式下法兰截面 J 按平行舌近似」）：R35 读回已经认得出锥形舌与叉臂，
                //   这里接到判据几何 —— 舌根比舌端宽 1 mm 以上按锥形（两边到切点），舌根厚于舌端 0.05 以上按叉臂带
                //   [带起点, 舌盘交界]，法兰截面 J 才按图上真实的舌根截面算，不再一律当平行舌。
                bool taper3 = !double.IsNaN(sh3.TabRootHalfWidthMm) && sh3.TabRootHalfWidthMm > sh3.TabEndHalfWidthMm + 1.0;
                bool arm3 = !double.IsNaN(sh3.TabRootThickMm) && !double.IsNaN(sh3.TabEndThickMm)
                            && sh3.TabRootThickMm > sh3.TabEndThickMm + 0.05 && !double.IsNaN(sh3.TabArmX0Mm);
                double wJ = Math.Min(sh3.TabEndHalfWidthMm, sh3.DiscRadiusMm);
                double xJunction = -Math.Sqrt(Math.Max(0, sh3.DiscRadiusMm * sh3.DiscRadiusMm - wJ * wJ));
                var eq = new FlangePlate
                {
                    ...
                    ThicknessMm = tMax,
                    TabParallel = !taper3,
                    TabFilletMm = taper3 ? 0 : DesignSpec.Current.TabFilletMm,
                    TabThicknessMm = double.IsNaN(sh3.TabEndThickMm) ? double.NaN : sh3.TabEndThickMm,
                    TabArmX0Mm = arm3 ? sh3.TabArmX0Mm : double.NaN,
                    TabArmX1Mm = arm3 ? xJunction : double.NaN,
    ...
    private async Task SearchShapeAsync()
    {
        ...
        sb.AppendLine("搜形状找的是更省铂的形状（盘径／舌宽／锥形舌边"
                     + (both ? "／解法族" : "") + "）；搜完要再点核算整线精算才可出图。");
        sb.AppendLine("=== 搜形状（盘半径 × 舌宽 × 锥形／平行舌边；舌长按装配算）===");
        ...
        sb.AppendLine(both
            ? "解法：两个都算 ⇒ 下面先搜「不挖舌孔」族，再搜「挖舌孔」族，两族独立选各自最轻可行，不比重量；"
              + "写回页面的是「不挖舌孔」族的赢家，「挖舌孔」族的赢家进下面「搜形状结果 ▾」，选它就写回。"
            : (FamilyAllowsCuts ? "解法：挖舌孔那族（舌孔旋钮进候选）" : "解法：不挖舌孔那族（舌孔旋钮不进候选）"));
        sb.AppendLine("随时可以点「取消」——**已经算完的形状结果不会丢**。");
        sb.AppendLine();
        _out.Text = sb.ToString();

        var allRows = new List<(DesignSpec d, double mass, bool ok, string msg)>();

        try
        {
            // ★★★★★ **一轮 = 改一次形状（盘径/舌宽/锥形）+ 在它上面把梯度分布扫一遍**
            //   （用户 2026-08-25：「改变法兰直径与扫梯度分布算一轮」）。此前是**固定网格
            //   全枚举**：3 个盘径 × 2 个舌宽跑满就收工 —— 走不出 25/30/35，也不会因为
            //   「都在变坏」提前停。现在：网格当**第 1 轮**（给基准点与方向），之后沿改善
            //   方向**外推**，一轮下来没有任何邻点更好就停；R38 起锥形也是外推的一个方向。
            //
            //   两族（不挖舌孔／挖舌孔）共用这**同一份**实现 —— 不许复制两份（那是本项目
            //   「同一件事写两遍、改一处忘一处」最典型的病灶）。不是「两个都算」时只调一次，
            //   allowCuts 就是当前唯一在跑的那族，行为与改之前逐位相同（只多了锥形维度与
            //   「舌边」这一列，那是 R38 本身要加的东西，不是两族分支引入的）。
            async Task<SolverResult?> SearchOneFamilyAsync(
                bool allowCuts, string famTag, string famPrefix, string? famProvTag,
                List<(DesignSpec d, double mass, bool ok, string msg)> rows)
            {
                if (both)
                ...
                // ★★ 2026-09-15 Opus 5（J 路，合并把关待办 P1-6）：精算没全过就不许叫「全过形状」、不许写回控件（口径与理由见 ShapeSearchPlan.RefineVerdict）。
                //   返回 null ＝ 这一族没有全过的形状：下面「两族并列」与收尾都按无解走。
                var refine = ShapeSearchPlan.RefineVerdict(fin.Feasible, fin.StopWhy, famPrefix);
                if (!refine.WriteBack)
                {
                    _out.AppendText("\r\n" + refine.Headline + "\r\n");
                    return null;
            ...
            // R38：不是「两个都算」时，family 就是当前唯一在跑的那族（与此前逐位相同）；
            //   「两个都算」时固定先跑「不挖舌孔」再跑「挖舌孔」——与 R32「核算整线」
            //   两族并列时的先后一致，写回页面的也固定是「不挖舌孔」那族（下面）。
            var fin1 = await SearchOneFamilyAsync(
                both ? false : FamilyAllowsCuts,
                both ? "不挖舌孔" : (FamilyAllowsCuts ? "挖舌孔" : "不挖舌孔"),
                both ? "第 1 族（不挖舌孔）　" : "",
                both ? "不挖舌孔" : null,
                allRows);

            SolverResult? fin2 = null;
            if (both)
            {
                var rowsB = new List<(DesignSpec d, double mass, bool ok, string msg)>();
                fin2 = await SearchOneFamilyAsync(true, "挖舌孔", "第 2 族（挖舌孔）　", "挖舌孔", rowsB);
                allRows.AddRange(rowsB);
                // R32 同款「两族并列」小结：各自最优，不比重量，工程师自己选
                _out.AppendText(Environment.NewLine + "★ 两族并列（各自最轻可行，不比重量 —— 你来选）：" + Environment.NewLine
                    + $"   不挖舌孔　{(fin1 is not null ? $"{fin1.MassG:0} g　{fin1.Message}" : "没有全过的形状")}" + Environment.NewLine
                    + $"   挖舌孔　　{(fin2 is not null ? $"{fin2.MassG:0} g　{fin2.Message}" : "没有全过的形状")}" + Environment.NewLine);
            }

            if (fin1 is null && fin2 is null)
            {
                _out.AppendText("\r\n下一步：点核算整线精算。\r\n");
                _status.Text = "无解";
            ...
            // R38：写回的固定是 fin1（不是两个都算时就是那唯一一族；两个都算时固定
            // 是「不挖舌孔」族，R32 原则：APP 不替工程师在两族间比重量，但要有一族
            // 落在页面上，选与旧行为一致的那族）。fin1 没有可行解时（只在两个都算、
            // 且不挖舌孔族无解时才可能）不写回页面，赢家都在下拉里，工程师自己选。
            if (fin1 is not null)
            {
                decimal C(double v, NumericUpDown n) => Math.Clamp((decimal)v, n.Minimum, n.Maximum);
                _suppressAuto = true;
                _discD.Value = C(2 * fin1.Design.DiscRadiusMm, _discD);
                _tabW.Value = C(fin1.Design.TabHalfWidthMm, _tabW);
                _tabLen.Value = C(fin1.Design.TabLengthMm, _tabLen);
                _tabTaper.Checked = fin1.Design.TabTaper;             // R38：胜出的锥形／平行也写回，同一处、同一 _suppressAuto 包裹
                for (int i = 0; i < _tPlate.Length && i < fin1.Design.TabThickMm.Length; i++)
                    _tPlate[i].Value = C(fin1.Design.TabThickMm[i], _tPlate[i]);
                _suppressAuto = false;
                // ★ 统一入口（见 AdoptSolvedDesign）：此前这里只写 _last ⇒
                //   舌保温/环倍率丢掉、状态没发布，与「自动定厚」是同一个病的第三例。
                AdoptSolvedDesign(fin1.Design, fin1.Best);
                _out.AppendText(Environment.NewLine + "★ 已写回上面的盘径/舌宽/舌长/舌边/板厚"
                              + (both ? "（这是「不挖舌孔」族的赢家；「挖舌孔」族的赢家在下面「搜形状结果 ▾」，选它就写回）" : "")
                              + Environment.NewLine);
            }
            else
            {
                _out.AppendText(Environment.NewLine + "★ 「不挖舌孔」族没有全过的形状，页面没有写回；"
                              + "「挖舌孔」族的赢家在下面「搜形状结果 ▾」，选它就写回。" + Environment.NewLine);
            }
            ...
            if (_pipeStep.Length == 0)   // 独立点的才收尾；流水线里跑的留给流水线自己的 Finish（否则会冻结后续①②③④⑤显示）
                _stages.Finish(true, fin1 is not null ? "已写回最轻形状 —— 下一步：点核算整线精算"
                                                       : "不挖舌孔族无解，赢家在「搜形状结果」里 —— 下一步：点核算整线精算");
        }
        ...
        finally
        {
            ...
            //   R38：两个都算时两族的行都在（已按各自的 famProvTag 打了「解法：…」标记）
            _shapeRows = allRows;
            RefreshShapePick();
                ...
                else
                {
                    ...
                    if (srAlt is not null)
                    {
                        ...
                        string FamLine(SolverResult s, string nm)
                        {
                            ...
                            return $"   {nm}　{(s.Feasible ? "可行" : "不可行")}　合计 {s.MassG:0.0} g　{Criteria.Plain(LineResult.Key.ColdUnderTc)} {cold:0.00} K"
    ...
    private bool RebuildPlateRows()
    {
        ...
        string tipTongue =
            "舌片厚 = 设计电流 ÷ (设定 J × 舌片最窄有效宽)，闭式（舌片厚度 = 截面积 I/J ÷ 舌宽；J 在 ① 输入设定，预设 10）。" + Environment.NewLine +
        ...
        string tipHole =
          ...
          + "⚠ 上界闭式：孔缘到舌边要留够桥宽。"
          + Environment.NewLine
          + "⚠ 最小孔径 1 mm：填 0 = 无孔，填在 0～1 之间的数按无孔算；求解器不会探 1 mm 以下。";
    ...
    /// <summary>
    /// ★★★★★ R27（用户 2026-09-10：「UI 必须有输入铂金管加热段数的输入框」）。
    /// <see cref="_segCount"/> 改了 ⇒ 段表（<see cref="_segs"/>）跟着加/删末尾行。
    /// 段表 ⇒ 段数框的反向同步在 HookAutoRun 里的 SegsChanged（那边由 _segGrid 的
    /// RowsAdded/RowsRemoved 驱动，本方法对 _segs 的每一次 Add/RemoveAt 都会触发它）。
    ///
    /// 加段：新段名 HCn、长度取参数表「直接加热铂金管的长度」默认值、控温点 = 上一段 − 70 °C
    /// （不低于 900）—— 都是用户 2026-09-10 原话给的规则。
    /// 删段：只删末尾。若被删的行不是「照这条规则会生成的样子」（工程师手改过名字/长度/控温），
    /// 在输出框写一句说明，**不弹框**（本仓惯例：弹框会打断自动流程、也不是「安静失败」该有的反面）。
    /// </summary>
    private void SegCountChanged()
    {
        // ⚠ _suppressAuto 为真时这次赋值是程序自己写的（见 SegsChanged 里回填段数框），
        //   不是工程师在改 —— 不能反过来又去动段表，那会变成两边互相追。
        if (_suppressAuto) return;
        int want = (int)_segCount.Value;
        int have = _segs.Count;
        if (want == have) return;
        ...
        if (want > have)
        {
            ...
            {
                double prevTemp = _segs.Count > 0 ? _segs[^1].控温C : 1150.0;
                double prevHead = _segs.Count > 0 ? _segs[^1].水头m : 1.0;
                _segs.Add(new SegRow
                {
                    名称 = "HC" + (k + 1),
                    直接加热管长mm = _base.TubeLengthMm,
                    控温C = Math.Max(900.0, prevTemp - 70.0),
                    水头m = prevHead,
                });
            }
        }
        ...
        {
            // 删之前先判「这行是不是规则会生成的样子」——是就悄悄删，不是就留一句话。
            bool LooksAutoGenerated(int idx)
            {
                var row = _segs[idx];
                if (row.名称 != "HC" + (idx + 1)) return false;
                if (Math.Abs(row.直接加热管长mm - _base.TubeLengthMm) > 1e-6) return false;
                double prevTemp = idx > 0 ? _segs[idx - 1].控温C : 1150.0;
                if (Math.Abs(row.控温C - Math.Max(900.0, prevTemp - 70.0)) > 1e-6) return false;
                return true;
            }
            var removedNotes = new List<string>();
            while (_segs.Count > want)
            {
                int idx = _segs.Count - 1;
                var row = _segs[idx];
                if (!LooksAutoGenerated(idx))
                    removedNotes.Add($"{row.名称}（长度 {row.直接加热管长mm:0} mm、控温 {row.控温C:0} °C）");
                _segs.RemoveAt(idx);
            }
            if (removedNotes.Count > 0)
                _out.AppendText("\r\n★ 段数改为 " + want + "：删掉了 " + string.Join("、", removedNotes)
                               + " —— 这几段的值跟自动生成的不一样，删前提醒一下。\r\n");
        }
    }
