...
border:1px solid var(--rule);border-radius:3px;font-size:.88em}
        ...
        sb.Append("<div class=\"note\"><b>「核算整线」走的是页面上的参数，" +
                  ...
                  "「▶ 复现设计记录」键已去掉（2026-09-11）：它只是校验程序本身的路，那是走查的活；工程师要的是「载入设计记录 → 核算整线」。</div>");
        ...
        sb.Append("<table><tr><th>我想…</th><th>点哪个</th><th>看哪里</th></tr>" +
                  ...
                  "<td>设计记录 ▾ → <b>载入设计记录</b> → <b>核算整线</b></td><td>判据表；载入后页面就是记录，核算出来的数与记录逐项一致（走查钉着）</td></tr>" +
        ...
        sb.Append("<table class=\"nw\"><tr><th>步</th><th>做什么</th><th>为什么是这个顺序</th></tr>" +
                  ...
                  "<td>把那组参数灌进控件，从一个**已知可行**的点出发改</td></tr>" +
                  "<tr><td>2</td><td><b>核算整线</b></td><td>先看一眼「已知可行的答案」长什么样，后面才有比较的基准</td></tr>" +
        ...
        // ════════════════════════════════════════════════════════════════
        // ★★★★★ R28（2026-09-10，用户原话：「核算整线 vs ◇ 搜形状 <-- 工程师要怎么用？
        //   何时用？写进使用说明里（或是有随时能提示或限制的功能，别让工程师瞎按）」）。
        //
        //   两个按钮各干什么、能不能互相替代，本页此前只在别处散着提（§按「我想做什么」查、
        //   §四个常见误用），没有一处把两者**并排对照**过——工程师照旧得自己拼。
        //   口径已由用户定死，原样落地：不许把判据代号写进来、不许出现命令行开关名、
        //   不许用 Markdown 星号（本页是 HTML，一律用 <b> 加粗）。
        // ════════════════════════════════════════════════════════════════
        sb.Append("<h3>核算整线 vs ◇ 搜形状：怎么用、何时用</h3>");
        ...
        sb.Append("<table class=\"nw\"><tr><th></th><th>核算整线</th><th>◇ 搜形状</th></tr>" +
                  ...
                  "<td>几小时（预算到点掐停），适合下班前点</td></tr>" +
                  "<tr><td>结束时</td><td>说「可以出图」或停在哪一步、为什么</td>" +
                  "<td>最轻的写回页面；算过的全部在下拉里</td></tr>" +
        ...
        sb.Append("<div class=\"note\"><b>两条边界：</b>" +
                  ...
                  "搜形状写回的是导航网格上的解，不能直接出图，搜完必点核算整线。<br>" +
                  "<b>不需要工程师决定的：</b>调哪根旋钮、调多少、网格加密几档、要不要细网格重解 —— " +
                  "都在核算整线里，①②③④⑤ 状态条告诉你它在哪一步。</div>");
        ...
        sb.Append("<p style=\"font-size:.88rem\"><b>三条通用的：</b>" +
                  "① 会跑很久的按钮（核算整线／自动定厚／搜形状）点下去会<b>变成「取消」</b>，" +
                  "再点一次就是中止，不必等；" +
