...
public sealed class SizerOptions
{
    ...
    /// <summary>
    /// 舌保温量化步 mm —— 纤维厚度现场根本做不到比这更细。
    ///
    /// ⚠ **本轮（2026-09-17，Opus 5）没有跟着改成 0.5，是查过之后决定的，不是漏了。**
    ///   用户 2026-09-17 原话是「**求解器**默认格子改成 0.5」，改的是
    ///   <see cref="SolverOptions.QuantInsulMm"/>（0.1 → 0.5 = 包法每层 <see cref="InsulationSearch.LayerMm"/>）。
    ///   定尺寸器 D8 是**带起点的老路**，交付链早已改走 <see cref="Solver.Solve"/>：
    ///   界面「自动定厚」走 Solver／FlangeAutoSizer，本类只剩命令行的一条旧支路（Program.cs 的 Sizer.Solve）
    ///   与界面控件借用的上下界常量。
    ///   这里的 0.1 还进下面那条「步子比量化步还细就不动」的判断 ⇒ 顺手改成 0.5 会**改掉老路的行为**，
    ///   而没有任何一道门在守它。⇒ 留着，并把这件事写在这里与 HANDOVER 里：
    ///   **谁要让这条老路也交付，先把它的量化步一起接到包法层厚上。**
    /// </summary>
