// ⚠ 2026-09-16，Opus 5（R48 L 路）：**本档临时拷自工作树 D:\WinForm\r48_H**（拷贝时 r48_H 该档 git 状态 AM ——
//   已 git add 但工作区又改过，尚未提交；拷入源 SHA-256 = c0776ec87e645e1fbe77c1eb67adb9c9a4d0e813d6bb34e7e57652a71497fc5f）。
//   L 路只读它算管段伸长探针，一个字没改。**待 H 路合入 r48_M 后以 H 为准**，本拷贝作废，不许在 L 路上改它。
using System;
...
public static class PtThermalExpansion
{
    ...
    private static Dictionary<string, ExpansionCurve> BuildCurves()
    {
        ...
        var list = new List<ExpansionCurve>
        {
            ...
            new("Pt", ExpansionCoeffOrigin.CellFormula,
                ...
                $"{chart} 系列 idx 0：x B5:B14，y D5:D14（D 列 = C 列 LT/L0 按 α 公式）"),
            ...
            FromTable("PtRh10", ExpansionCoeffOrigin.RecomputedSameAlgorithm,
                ...
                $"{chart} 系列 idx 1：x B5:B19，y F5:F19"),
            // PtRh20：系列 idx 2，6 阶，标签文本「y = -1.362330E-17x6 + 6.567769E-14x5 - 1.256153E-10x4
            //   + 1.233031E-07x3 - 6.760337E-05x2 + 2.352578E-02x + 4.206619E+00」。
            FromTable("PtRh20", ExpansionCoeffOrigin.ChartLabelText,
                $"{chart} 系列 idx 2（H5:H19）6 阶趋势线标签文本",
                new[] { 4.206619E+00, 2.352578E-02, -6.760337E-05, 1.233031E-07, -1.256153E-10, 6.567769E-14, -1.362330E-17 }, 15,
                $"{chart} 系列 idx 2：x B5:B19，y H5:H19"),
            FromTable("PtRh30", ExpansionCoeffOrigin.RecomputedSameAlgorithm,
                ...
                $"{chart} 系列 idx 3：x B5:B19，y J5:J19"),
            FromTable("PtRh95", ExpansionCoeffOrigin.ChartLabelText,
                ...
                $"{chart} 系列 idx 4：x B5:B19，y L5:L19"),
            FromTable("Rh", ExpansionCoeffOrigin.ChartLabelText,
                ...
                $"{chart} 系列 idx 5：x B5:B19，y N5:N19"),
    ...
    {
        new GradeExpansionMap("Pt", "Pt", ExpansionLink.Direct, "工作簿列头 Pt（纯铂）；APP 设计默认牌号"),
        new GradeExpansionMap("Pt-Rh/90-10", "PtRh10", ExpansionLink.Direct, "同名义成分 PtRh10（两份工作簿均未注明供应商）"),
        new GradeExpansionMap("Pt-Rh/80-20", "PtRh20", ExpansionLink.Direct,
            "同名义成分 PtRh20；⚠ 6 阶趋势线在数据区间内高温段瞬时 α 下降（形状不可信段见曲线）"),
    ...
    /// （<see cref="ExpansionCurve.InstantAlphaStepAt100"/>；Pt-Rh/80-20 与借用它的 Umicore-PtRh20 是 +22.1 %，见 HANDOVER 0.-6）。
    /// </summary>
