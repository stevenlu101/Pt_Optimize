...
public static class DesignSpecStore
{
    ...
    private sealed class Dto
    {
        ...
        /// <summary>R48（2026-09-14，Opus 5）：逐片圆盘保温 mm；缺 = 全线沿用 flangeInsulMm（旧档逐位不变）。</summary>
    ...
    public static string Save(DesignSpec fd)
    {
        ...
        var dto = new Dto
        {
            ...
            tabInsulMm = JsonSerializer.SerializeToElement(fd.TabInsulMm),
            discInsulMm = fd.DiscInsulMm.Length > 0 ? fd.DiscInsulMm : null,   // R48：空 = 不写（旧档逐字不变）   // 逐片数组（读时也认旧档的单个数，见 TabInsulFrom）
