...
public sealed class ShellMesh
{
    ...
    /// <summary>R48 F（2026-09-15 Opus 5）：这张网格上压接边界是否真的施加在面上 = 开关开 且 整面接触（<see cref="ClampCell"/> 非空）。</summary>
    public bool ClampFaceActive => ClampFaceDirichlet && ClampCell.Length == CellCount;
...
public static class FlangeMesher
{
    ...
    ///     表换了差值未必不变 —— 合并树上已按原判读原门槛重跑（deliverable/R48_压接面上定温_细带去留_2026-09-15_本次开跑于2026-09-15_1536.txt，头上印代码指纹与散热表上限 1768.2 °C）：两片三档「分级不带细带 − 均匀」片0 −0.0045／+0.0006／+0.0001 W、片1 −0.0026／+0.0026／+0.0006 W，「分级 + 细带 − 均匀」与格数也与上面所引四位小数逐个相同 ⇒ 判读仍成立（≤ 0.05 W）；锥形舌细带去留、收敛阶、正偏差定位没有重跑。首跑文件在 r48_F 工作树，合并树里没有（按合并目录规矩未拷）。
