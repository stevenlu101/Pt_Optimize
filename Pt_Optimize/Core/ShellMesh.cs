...
public sealed class MeshRecipe
{
    ...
    public MeshRecipeRule Rule => new(ClampFullFace, ClampAnchorOnNode, ClampBandPerHFine, HoleTagBandMm);
...
public readonly record struct MeshRecipeRule(bool ClampFullFace, bool ClampAnchorOnNode, double ClampBandPerHFine, double HoleTagBandMm)
{
    public string Describe()
        => $"压接整面接触 {(ClampFullFace ? "开" : "关")}；压接边界落成节点 {(ClampAnchorOnNode ? "是" : "否")}；"
...
public static class FlangeMesher
{
    ...
    /// </summary>
    public const double ClampBandPerHFine = 3.0;

    /// <summary>
    /// 压接细带单侧宽度 mm 的解析：<paramref name="clampBandMm"/> 为 NaN（缺省）⇒ 生产配方 <see cref="ClampBandPerHFine"/> × hFine；
    /// 显式给数就照给的（0 或负 = 不铺，老探针「固定 3 mm」等原义不变）。
    ...
    public static readonly MeshRecipeRule ProductionMeshRule = new(
        ClampFullFace: true, ClampAnchorOnNode: true, ClampBandPerHFine: ClampBandPerHFine, HoleTagBandMm: HoleTagBandMm);
    ...
    public static ShellMesh Build(FlangePlate g, double yPlane = 0,
                                  ...
                                  double clampBandMm = double.NaN, bool clampFullFace = true)
    {
        ...
        return BuildFromField(f, g.HoleRadiusMm, yPlane, hFine, hCoarse, fineRadius, clampLenMm,
                              ...
                              clampBandMm: clampBandMm, clampFullFace: clampFullFace);
    }
    ...
    ///
    /// ══ 判定网格配方声明（2026-09-14，Opus 5）—— 下面四条都是生产默认，解析与图纸两条路径同一份：
    ...
    ///   ④ 自相似压接细带：压接边界落成节点时，两侧各 <see cref="ClampBandPerHFine"/>×hFine 铺细步 hFine（<paramref name="clampBandMm"/> 缺省 NaN）。
    ...
    /// <param name="clampBandMm">压接细带单侧宽度 mm（配方 ④）。缺省 NaN = 生产配方 <see cref="ClampBandPerHFine"/>×hFine；
    /// 显式给正数 = 照给的宽度（老探针「固定 3 mm」「3·h」原义不变）；显式 0 = 不铺（改动前的行为）。解析见 <see cref="ResolveClampBandMm"/>。</param>
    ...
    public static ShellMesh BuildFromField(ThicknessField f, double holeRadiusMm,
                                           ...
                                           double holeTagBandMm = 0, double clampBandMm = double.NaN, bool clampFullFace = true)
    {
        var m = new ShellMesh();
        string clampNote = "";
        ...
        //   不加的情形（写进 ClampAnchorNote，不静默）：离舌尖、离已有锚点不足一个最细格（免得生出发丝格，N3 只管四分之一格）。
        {
            ...
            foreach (double ca in cand)
            {
                ...
                //   带宽缺省 = ClampBandPerHFine × hFine（自相似，依据 R48_压接细带自相似_2026-09-14.txt，见 ClampBandPerHFine）；
        ...
        if (clampFullFace)
        {
            ...
            if (holeInClamp == 0) m.ClampCell = cc;
            else
            {
        ...
        {
            ClampFullFace = m.ClampCell.Length == m.CellCount && m.CellCount > 0 && m.ClampCell.Any(b => b),
