...
public static class ShellThermal
{
                                           ...
                                           double insulDiscRadiusMm = double.NaN)
    {
        ...
        bool clampedBC = !busG && p.BusbarClampTempC >= 0;                   // 与下面汇总处的 clamped 同一个式子（那里原样保留）
        ...
        bool clamped = ClampBoundaryOf(p) == ClampBoundary.FixedTemp;   // R48 G2（2026-09-15 Opus 5）：唯一判定，= !busG && 夹持温度 ≥ 0
        double qc = 0;
        if (clamped)
            for (int i = 0; i < n; i++)
            {
