...
public sealed class ThermalRecipe
{
    ...
    public ThermalRecipeRule Rule => new(InsulBlend, LossTableHiC, LossTableNodesByRule, HoleFaceCount > 0, HoleFaceDirichlet, ClampFullFace);
...
/// <remarks>2026-09-15 Opus 5：加 <see cref="HasHoleBoundary"/>（网格上有孔边界面，管温才施加得上）；有孔边界面时描述文字与原先逐字相同。</remarks>
public readonly record struct ThermalRecipeRule(InsulBlendState InsulBlend, double LossTableHiC, bool LossTableNodesByRule, bool HasHoleBoundary, bool HoleFaceDirichlet, bool ClampFullFace)
{
    public string Describe()
        => "分界格混合 " + (InsulBlend switch { InsulBlendState.On => "开", InsulBlendState.OffNoField => "关（网格没有厚度场）", _ => "不适用（保温按 x 划）" })
...
public static class ShellThermal
{
                                          ...
                                          double lossTableHiC = double.NaN, int lossTableNodes = 0)
    {
        ...
        return new ThermalRecipe
        {
            ...
            ClampMode = busG ? ClampBoundaryMode.Conductance : p.BusbarClampTempC >= 0 ? ClampBoundaryMode.Fixed : ClampBoundaryMode.Free,
