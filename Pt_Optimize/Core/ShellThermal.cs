...
public sealed class ShellThermalResult
{
    ...
    /// <summary>
    /// ★ R48 R（2026-09-17，Opus 5）：这一次舌保温用的是不是**分段剖面**（空串 = 单值，那是默认口径）。
    /// 与 <see cref="InsulRule"/> 同理：换了口径必须看得见，不许静默。
    /// </summary>
    public string TabInsulProfileNote = "";

    /// <summary>
    /// ★★★ R48 R（2026-09-17，Opus 5；反解线把关第二轮）：**逐格诊断** —— 这一格实际用了多厚舌保温 mm。
    ///
    /// 为什么要它：把 <see cref="ShellThermal.Solve"/> 里那两个「本格该取哪一段」的闭包注入段号 +1
    /// （整条剖面错位一段），R48TabInsulProfileGateTests 原来的九条门**全绿**，而生产判据的管孔净流入
    /// 从 0.449 W 翻到 29.23 W。原因：那九条守的是「每一段的表建得对」（门 e）与「场真的动了」（门 d），
    /// 没有一条守「**这一格**拿到的是不是它自己那一段的东西」。⇒ 把逐格的实际取值原样记出来，门在下游逐格比（门 i）。
    ///
    /// 剖面为 null ⇒ **空数组**（不分配、不计算），默认路径逐位不变。
    /// </summary>
    public double[] TabInsulMmCell = Array.Empty<double>();

    /// <summary>
    /// ★★★ R48 R（2026-09-17，Opus 5）：逐格诊断 —— 这一格**实际用来组装的那张舌面热流表**在
    /// <see cref="TabSurfaceQProbeC"/> 上的值 W/mm²（= 表的身份指纹）。
    /// 圆盘保温格与分界圆混合格记 NaN（那两类另有门守，见 <see cref="InsulRule"/>）。剖面为 null ⇒ 空数组。
    /// </summary>
    public double[] TabSurfaceQProbeCell = Array.Empty<double>();

    /// <summary>逐格舌面热流指纹的探针温度 °C（见 <see cref="TabSurfaceQProbeCell"/>）。剖面为 null ⇒ NaN。</summary>
    public double TabSurfaceQProbeC = double.NaN;
...
public sealed class SurfaceLossTables
{
    ...
    /// <summary>
    /// ★ R48 R（2026-09-17，Opus 5）：**分段舌保温剖面**时每一段各一张舌面热流表（长度 = 段数）；
    /// 剖面为 null ⇒ 空数组、<see cref="Tab"/> 就是那唯一一张（原路径，逐位不变）。
    /// </summary>
    public LossTable[] TabSeg = Array.Empty<LossTable>();

    /// <summary>剖面本身（null = 单值口径）。</summary>
    public TabInsulProfile? TabProfile;

    /// <summary>x 处该用哪一张舌面热流表 —— 剖面为 null 时恒为 <see cref="Tab"/>。</summary>
    public LossTable TabAt(double x) => TabProfile is null ? Tab : TabSeg[TabProfile.IndexAt(x)];

    /// <summary>x 处舌保温实际的厚度 mm（局部热稳定取它；剖面为 null 时是那个单值，裸露时调用方自己判）。</summary>
    public double TabThickAt(double x, double scalarMm) => TabProfile is null ? scalarMm : TabProfile.At(x);
}
...
public static class ShellThermal
{
    ...
    /// </summary>
    /// <remarks>
    /// ★ R48 R（2026-09-17，Opus 5）：<paramref name="tabProfile"/> 非 null ⇒ 逐段各建一张舌面表（算式与单值那张**逐字相同**，
    ///   只把厚度换成该段的厚度）；null ⇒ 一行不动的原路径。全段等厚时逐段表与单值表的节点值逐位相同（门 R48TabInsulProfileGateTests）。
    /// </remarks>
    public static SurfaceLossTables SurfaceLossTablesFor(DesignInputs p, double tabInsulThickMm, double hiC = double.NaN, int nodes = 0,
                                                         TabInsulProfile? tabProfile = null)
    {
        ...
        var tabInsTab = tabInsul
            ...
            : bareTab;
        // ★ R48 R（2026-09-17，Opus 5）：分段剖面 —— 每段一张，算式与上面那张逐字相同（只换厚度取值）。
        var tabSeg = Array.Empty<LossTable>();
        if (tabProfile is not null)
        {
            tabSeg = new LossTable[tabProfile.SegmentCount];
            for (int k = 0; k < tabSeg.Length; k++)
            {
                double tk = tabProfile.ThickMm[k];
                tabSeg[k] = DesignScreen.FlangeFaceInsulated(tk)
                    ? new LossTable(p.TAmbC, tHi, nLossTab,
                        x => DesignScreen.PlateFluxWPerM2(p, x, tk) * 1e-6)
                    : bareTab;
            }
        }
        return new SurfaceLossTables { Bare = bareTab, Disc = insTab, Tab = tabInsTab, DiscInsulated = discInsul, TabInsulated = tabInsul,
                                       TabSeg = tabSeg, TabProfile = tabProfile };
    }
    ...
    public static ShellThermalResult Solve(ShellMesh m, double[] jMagAPerMm2, DesignInputs p,
                                           ...
                                           TabInsulProfile? tabInsulProfile = null)
    {
        ...
        var tables = SurfaceLossTablesFor(p, tabInsulThickMm, lossTableHiC, lossTableNodes, tabInsulProfile);
        ...
        // ★ R48 R（2026-09-17，Opus 5）：分段舌保温剖面 —— 这两个闭包是「这一格的舌面该用哪张表／该算多厚」的**唯一取值口径**。
        //   剖面为 null ⇒ 恒等于原来的 tabInsTab / (tabInsul ? tabInsulThickMm : 0)，一行不多走 ⇒ 默认路径逐位不变。
        LossTable TabTableAt(double cx) => tabInsulProfile is null ? tabInsTab : tables.TabSeg[tabInsulProfile.IndexAt(cx)];
        double TabInsulMmAt(double cx)
        {
            if (tabInsulProfile is null) return tabInsul ? tabInsulThickMm : 0.0;
            double t = tabInsulProfile.At(cx);
            return DesignScreen.FlangeFaceInsulated(t) ? t : 0.0;
        }
        res.TabInsulProfileNote = tabInsulProfile is null ? "" : "舌保温分段剖面：" + tabInsulProfile.Describe();
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            insulated[i] = insulByRadius
                         ...
                         : symmetricInsul
                             ...
                             : cx >= insulBoundaryX;
            lossFor[i] = insulated[i] ? insTab : TabTableAt(cx);   // R48 R：舌面表按本格 x 取（剖面为 null 时恒为 tabInsTab）
            ...
            else if (fr <= 0.0) lossFor[i] = TabTableAt(cx);
            else { lossFor[i] = LossTable.Blend(insTab, TabTableAt(cx), fr); nBlend++; }
        }
        ...
        // ★★★ R48 R（2026-09-17，Opus 5；反解线把关第二轮查出「段号错位九条门全绿」）：**逐格取值的诊断记录**。
        //   记的是**上面这个循环真的用了的东西**：mmCell 来自 TabInsulMmAt（局部热稳定读的就是它），
        //   qCell 来自 lossFor[i] 本身（组装读的就是它）—— 不是另算一遍，所以段号错位跑不掉。
        //   门 i（R48TabInsulProfileGateTests）在下游逐格比：厚度必须 == 剖面 At(本格形心 x)，表必须 == 该段那一张。
        //   剖面为 null ⇒ 一格都不走、两个数组保持空 ⇒ 默认路径逐位不变、也不多分配。
        if (tabInsulProfile is not null)
        {
            double probeC = Math.Clamp(900.0, p.TAmbC, tHi);
            var mmCell = new double[n];
            var qCell = new double[n];
            for (int i = 0; i < n; i++)
            {
                double cx = m.Centroid[i].X;
                mmCell[i] = TabInsulMmAt(cx);
                // 只记**纯舌面**的格：圆盘保温格（insulated）与分界圆混合格（0 < f）记 NaN，那两类的归属另有门守
                bool pureTab = !insulated[i] && !(insulFrac[i] > 0.0);
                qCell[i] = pureTab ? lossFor[i].Eval(probeC) : double.NaN;
            }
            res.TabInsulMmCell = mmCell;
            res.TabSurfaceQProbeCell = qCell;
            res.TabSurfaceQProbeC = probeC;
        }
        ...
        // ── 局部热稳定：两区各取前 12 个候选精算，取最小裕度
        {
            ...
            void Scan(List<(double Proxy, int I)> cand, bool onTab)
            {
                foreach (var (_, i) in cand.OrderByDescending(x => x.Proxy).Take(NCand))
                {
                    ...
                    // R48 R（2026-09-17，Opus 5）：舌保温经 TabInsulMmAt(本格 x) 取 —— 剖面为 null 时它恒等于原式 (tabInsul ? tabInsulThickMm : 0)。
                    double insMm = insulated[i] ? p.FlangeInsulThickMm
                                 : TabInsulMmAt(m.Centroid[i].X);
                    // R48（2026-09-14，Opus 5；物理把关人第四轮条件 2）：分界圆上混合了两种保温的格子，局部热稳定不跟着混合，
                    //   取两者里**较厚**的 —— 保温越厚冷却越弱，偏保守。
                    if (insulFrac[i] > 0 && insulFrac[i] < 1)
                        insMm = Math.Max(p.FlangeInsulThickMm, TabInsulMmAt(m.Centroid[i].X));
                                          ...
                                          double lossTableHiC = double.NaN, int lossTableNodes = 0)
    {
        ...
        return new ThermalRecipe
        {
            ...
            ClampFullFace = m.ClampCell.Length == m.CellCount && m.ClampCell.Any(b => b),
