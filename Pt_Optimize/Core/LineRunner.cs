        ...
        // ⇒ 这一轮先让它在每个算例上露出来、把量级攒起来，再谈升为硬判据。
        {
            ...
            if (flanges.Length > 0 && plates is { Length: > 0 })
            {
                ...
                var st = FlangeStability.Check(
                    ...
                    sf.DiscAreaMm2, sf.TabAreaMm2, c.DiscInsulEffectiveAt(wj),   // R48（2026-09-14，Opus 5）：逐片圆盘保温走唯一取值（图纸路径的 GeomForJudge 板件不带逐片值，原读法在那条路上仍是整线值）
                ...
                try
                {
                    ...
                    // R48（2026-09-14，Opus 5）：升温两节点也按本片圆盘保温（原读整线 c.Base），走唯一取值 LineCase.DiscInsulEffectiveAt
                    var pRamp = SegmentSolver.Clone(c.Base);
                    pRamp.FlangeInsulThickMm = c.DiscInsulEffectiveAt(wj);
