...
public static class LineRunner
{
    ...
    /// </summary>
    public static bool CoupleConverged(double remainK, double resK, double tolK, bool rootNaN)
        => remainK < tolK && resK * LineCase.FixedPointAmp < tolK && !rootNaN;
                                 ...
                                 CancellationToken cancel = default)
    {
        ...
        for (int outer = 0; outer < c.CoupleMaxRounds; outer++)
        {
            ...
            // （真残差那支的式子 resK × 25 < 容差 现在写在 CoupleConverged 里，同一份）
            ...
            if (CoupleConverged(remain, resK, c.CoupleTolK, rootNaN))
            {
    ...
    public sealed class TubeNewtonResult
    {
        ...
        ///     未计空管管腔轴向辐射）；这些增益与放大倍数只属于那份配方，合并树上没有重跑。
        ///   ★ 2026-09-15 Opus 5（J 路，合并把关待办 P3 第 6 条）：原写「0.972～0.984、放大 36～61 倍」—— 0.984 是 0.9835 进位、36／61 是截尾，两头口径不一。照文件重写：
    ...
    ///   ⚠ 2026-09-15 Opus 5（合并，复审后改）：这份对拍是在 r48_E 工作树上跑的（底板 f206e70：F 合入前的配方 —— 压接形心整格 + 3·hFine 细带、法兰散热表上限「设定 + 200 K、60 节点」、
    ///     未计空管管腔轴向辐射）；增益与放大倍数（带玻璃 0.9720～0.9729 放大 36.7、空管 0.9834～0.9835 放大 61.7，核算见 TubeNewtonResult.DistanceK 注释）只属于那份配方，合并树上没有重跑。文件已拷入合并树 deliverable（2026-09-15 Opus 5（J 路）改：原句「合并树里没有」过时）。
    /// ★ R48 E 审查修改（2026-09-15 Opus 5）新增。
    /// </summary>
