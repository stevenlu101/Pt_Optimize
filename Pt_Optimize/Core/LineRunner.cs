...
public sealed class LineCase
{
    ...
    /// <summary>
    /// ★ R48 R（2026-09-17，Opus 5）：`.3dm` 模式下的**逐片舌保温剖面**（长度 = 片数；短了用最后一片，与
    /// <see cref="TabInsul3dmPerPlateMm"/> 同约定）。**空数组或该片为 null** = 该片走单值 <see cref="TabInsul3dmAt"/>（逐位不变）。
    /// 读值一律走 <see cref="TabInsulProfile3dmAt"/>。
    /// </summary>
    public TabInsulProfile?[] TabInsulProfile3dmPerPlate = Array.Empty<TabInsulProfile?>();

    /// <summary>第 j 片在图纸路径上的舌保温剖面（null = 该片用单值）。</summary>
    public TabInsulProfile? TabInsulProfile3dmAt(int j)
        => TabInsulProfile3dmPerPlate.Length > 0
           ? TabInsulProfile3dmPerPlate[Math.Min(Math.Max(j, 0), TabInsulProfile3dmPerPlate.Length - 1)]
           : null;
...
public sealed class FlangeOut
{
    ...
    /// <summary>
    /// ★ R48 R（2026-09-17，Opus 5）：本片舌保温是不是**分段剖面**（空串 = 单值，默认口径）。
    /// 与 <see cref="InsulRule"/> 同理原样带进判据说明 —— 换了口径必须看得见，不许静默。
    /// </summary>
    public string TabInsulProfileNote = "";
...
public sealed class PlateThermalSetup
{
    ...
    /// <summary>
    /// ★ R48 R（2026-09-17，Opus 5）：本片**沿舌轴的分段常值舌保温剖面**（解析 = 板件 FlangePlate.TabInsulProfile；
    /// 图纸 = LineCase.TabInsulProfile3dmAt）。null = 单值口径，**逐位与从前相同**。
    /// </summary>
    public TabInsulProfile? TabInsulProfile;
    /// <summary>
    /// ★★ R48 R（2026-09-17，Opus 5）：**集总模型**（整片热稳定 FlangeStability、升温两节点 RampTwoNode）吃的那一个舌保温数 mm。
    /// 剖面为 null ⇒ 就是 <see cref="TabInsulThickMm"/>（**逐位不变**）；剖面非 null ⇒ 沿舌长按长度加权的平均厚度
    /// （<see cref="PtOptimize.Core.TabInsulProfile.LengthWeightedMeanMm"/>，**有意的近似**，见那里的注释）。
    /// ⚠ 二维场解不读它 —— 场解逐格取真厚度。两者不许混用。
    /// </summary>
    public double TabInsulLumpedMm = double.NaN;
...
public static class LineRunner
{
                                 ...
                                 CancellationToken cancel = default)
    {
        ...
        else
        {
            ...
            var bIt = IterateNeighbourTemps(c, bnb => RunOnce(c, null, cancel, zero, zeroLR, bnb), null,
                                            Math.Max(30, c.CoupleMaxRounds), c.CoupleTolK, progress, "无法兰基线");
                                      ...
                                      (double A, double B)[]? baseline = null)
    {
        ...
        for (int j = 0; j < nf; j++)
        {
            ...
            flanges[j] = new FlangeOut
            {
                ...
                TabInsulProfileNote = th.TabInsulProfileNote,               // R48 R（2026-09-17 Opus 5）：分段舌保温剖面也要带出来，别成静默口径
    ...
    public sealed class TubeNewtonResult
    {
        ...
        /// ★ 不用「真残差 × FixedPointAmp(25)」：那是按整线环路增益 0.96 定的常数，而共用片处段间耦合实测 0.972～0.984（放大 36～61 倍），
        ...
        ///     未计空管管腔轴向辐射）；增益 0.972～0.984、放大 36～61 倍这些数只属于那份配方，合并树上没有重跑。文件在 r48_E 工作树，合并树里没有。
        /// </summary>
    ...
    /// 为什么不用无法兰基线那份欠松弛迭代（<see cref="SolveTubeWithDraws"/>）：共用片那一处段间耦合增益近 1，实测 60 轮（每轮全部段解一次、约 1.7 s）
    /// 仍没收敛到 0.25 K（deliverable\R48_保温搜索_管侧响应_两种解法对拍_2026-09-15.txt 另有两者同一不动点的对拍）。不动点由同一个段解定义，只是到达方式不同。
    ///   ⚠ 2026-09-15 Opus 5（合并，复审后改）：这份对拍是在 r48_E 工作树上跑的（底板 f206e70：F 合入前的配方 —— 压接形心整格 + 3·hFine 细带、法兰散热表上限「设定 + 200 K、60 节点」、
    ///     未计空管管腔轴向辐射）；增益 0.972～0.984、放大 36～61 倍这些数只属于那份配方，合并树上没有重跑。文件在 r48_E 工作树，合并树里没有。
    /// ★ R48 E 审查修改（2026-09-15 Opus 5）新增。
    /// </summary>
    ...
    {
        var bad = flanges.Where(f => f is not null && f.ClampCoversHole).ToArray();
        if (bad.Length == 0) return;
        string who = string.Join("、", bad.Select(f => f.Name));
    ...
    {
        var bad = flanges.Where(f => f is not null && f.ClampIntoDisc).ToArray();
        if (bad.Length == 0) return;
        string who = string.Join("、", bad.Select(f => f.Name));
        double len = bad.Select(f => f.Mesh?.ClampLenMm ?? double.NaN).FirstOrDefault(v => !double.IsNaN(v), double.NaN);
        foreach (var ck in res.Checks)
    ...
    /// </summary>
    private static readonly string[] independentOfTubeField = { LineResult.Key.FreeTab, LineResult.Key.DiscCover, LineResult.Key.SectionJ, LineResult.Key.Ramp };
    ...
    public static IReadOnlyList<string> IndependentOfTubeFieldFor(bool useMeasuredCurrent)
        => useMeasuredCurrent ? independentOfTubeField.Append(LineResult.Key.TubeJ).ToArray() : independentOfTubeField;
    ...
    {
        var bad = segs.Where(s => s is not null && s.TubeLossTableExceeded).ToArray();
        if (bad.Length == 0) return;
    ...
    public static PlateThermalSetup PlateThermalInputs(LineCase c, int j, double iJoint, ThicknessField? fieldUsed)
    {
        ...
        s.TabInsulProfile = analytic ? plate!.TabInsulProfile : c.TabInsulProfile3dmAt(j);   // R48 R（2026-09-17 Opus 5）：null = 单值口径
        // R48 R（2026-09-17 Opus 5）：集总模型（整片热稳定、升温两节点）吃的那一个数 —— 剖面为 null 时就是单值，逐位不变。
        s.TabInsulLumpedMm = s.TabInsulProfile is null
            ? s.TabInsulThickMm
            : s.TabInsulProfile.LengthWeightedMeanMm(
                  analytic ? plate!.Tangent().X : s.TabInsulProfile.EdgeXMm[0],
                  analytic ? plate!.TabEndXMm : s.TabInsulProfile.EdgeXMm[^1]);
    ...
    public static ShellThermalResult SolvePlateThermal(ShellMesh mesh, double[] jMagAPerMm2, double tRootC, PlateThermalSetup s,
        ...
        => ShellThermal.Solve(mesh, jMagAPerMm2, s.P2, tRootC, s.InsulX,
                              ...
                              tabInsulProfile: s.TabInsulProfile);   // R48 R（2026-09-17 Opus 5）
    ...
    public static string FlangeStabNote(FlangeLumpedOut lumped)
    {
        ...
        return "dQ_散热/dT ÷ dP_发热/dT，**须 > 1**；< 1 即正反馈失控（保温过头那条路）。"
             + (st.Undetermined
    ...
    ///     这些数只读作「那份配方下量到的」，不是合并树的数；文件在 r48_G2 工作树的 deliverable 里，合并树里没有（按合并目录规矩未拷，清单见 HANDOVER 合并注记）。
    /// </summary>
    public static string RampPendingNote(LineCase c, FlangeLumpedOut o)
    {
        ...
        string detail = k.Ok && gi is not null
            ? $"　本片稳态场：法兰均温 {k.TNodeC:0} °C、发热 {k.QGenW:0} W、表面散热 {k.QLossW:0} W、流进铜排 {k.QClampW:0} W、管孔流入 {SizerResult.Signed(k.QFromTubeW)} W（管根 {k.TRootC:0} °C）；"
              + clamp + $"舌片区按 {Insul(gi.TabInsulThickMm)}"
              // R48 R（2026-09-17 Opus 5）：剖面进不了两节点，这一行的数是沿舌长按长度加权的平均厚度 —— 写明，不许静默
              + (o.ThermalSetup?.TabInsulProfile is null ? ""
                 : $"（本片舌保温不等厚：{o.ThermalSetup.TabInsulProfile!.Describe()}；两节点整片只有一个数，吃不下剖面，这里取沿舌长按长度加权的平均厚度。温度场那边逐格取真厚度，这个近似只影响本行这个参考值）")
              + $"、圆盘按 {Insul(o.DiscInsulMm)}算散热。"
            : "　" + k.Note + "。";
        return head + (o.ClampExcluded ? "　压接段压在铜排下，不算进法兰这一节点。" : "") + detail;
    }
    ...
    public static FlangeStability.Result StabilityCheck(LineCase c, FlangeOut fw, FlangeLumpedOut o, double? clampConductanceWPerK)
    {
        ...
        return FlangeStability.Check(
            ...
            o.ThermalSetup.TabInsulLumpedMm,                            // R48 G2：本片热解实际用的舌保温（原读板件 pl.TabInsulThickMm，解析路径同值）。R48 R（2026-09-17 Opus 5）：改读集总口径 —— 剖面为 null 时它就是那个单值，逐位不变
    ...
    public static FlangeLumpedOut? FlangeLumped(LineCase c, IReadOnlyList<FlangeOut> flanges, bool excludeClampCells = true)
    {
        ...
        var o = new FlangeLumpedOut { Index = wj, Plate = pl, Mesh = mesh, ClampExcluded = skip, MeshFromField = meshFromField };
        ...
        try
        {
            ...
            var gRamp = new RampTwoNode.Inputs
            {
                ...
                TabInsulThickMm = o.ThermalSetup.TabInsulLumpedMm,   // R48 R（2026-09-17 Opus 5）：两节点是集总模型，吃集总口径；剖面为 null 时逐位不变
        ...
        if (tcJoints.Length > 0 && blindHot.Length == 0)
        {
            ...
            hot = new ConstraintOut
            {
                ...
                Note = $"基准 = {wh.RefHow}；最热的是{wh.HottestWhat} {wh.HottestC:0.0} °C"
                     ...
                     + $"分区：{fh.DiscZoneRule}；{fh.InsulRule}"
                     + (fh.TabInsulProfileNote.Length > 0 ? "；" + fh.TabInsulProfileNote : "")   // R48 R（2026-09-17 Opus 5）   // R48 B：进界面的说明不写「口径」这类内部词（同 ShellThermal.DiscZoneRule 那条规矩）
                                         ...
                                         FlangeOut[] flanges, DesignInputs[] segParams)
    {
        ...
        if (hottestDisc is not null)
            checks.Add(new ConstraintOut
            {
                ...
                Note = "旧判法参考量（不卡交付；卡交付的是「最热铂高出热偶读数」）：基准是模型算的贴着的管根温度。" +
                       ...
                       (hottestDisc.TabInsulProfileNote.Length > 0 ? hottestDisc.TabInsulProfileNote + "；" : "") +   // R48 R（2026-09-17 Opus 5）
        ...
        // ⇒ 这一轮先让它在每个算例上露出来、把量级攒起来，再谈升为硬判据。
        {
            ...
            if (flanges.Length > 0 && plates is { Length: > 0 })
            {
                ...
                //     合并树的配方下没有重跑；只作那份配方下的出处，文件不在合并树（清单见 HANDOVER 合并注记）。
                ...
                else
                {
                    checks.Add(new ConstraintOut
                    {
                        ...
                        Withheld = true,
                        Note = "★ **暂不给数，待复核**；本设计上还**算不出来**：" + lumped.RampError
