...
public static class EvidenceHeader
{
    ...
    public static string ForLineCase(string title, LineCase lc, IEnumerable<string>? extra = null, string? root = null,
        ...
        => Build(title,
                 ...
                 (extra ?? Array.Empty<string>()).Concat(new[] { "耦合容差口径：" + CoupleTolNote(lc) }),
    ...
    public static string CoupleTolNote(LineCase lc)
        => lc is null ? "不适用"
         ...
         ? $"上限 {lc.CoupleTolK:R} K，按判据裕度收紧 = min(上限, max(下限 {lc.CoupleTolFloorK:R} K, {lc.CoupleTolMarginFrac:R} × 最小硬安全线温度裕度))；实际用的那一个见结果 CoupleTolKUsed"
         : $"常数 {lc.CoupleTolK:R} K（没按判据裕度收紧）";
