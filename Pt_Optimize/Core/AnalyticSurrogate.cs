...
public static class AnalyticSurrogate
{
    ...
    /// 容许的最大电学差（相对）。
    ///
    /// **这个数有据，不是挑的**：闭环自证（解析板 → 厚度场 → 反推 → 替身）在
    /// **同一片板**上量到约 1.5 % —— 那是栅格化 0.5 mm 与网格 2/11 mm 的离散噪声，
    /// 是这把尺子的分辨极限。取 2 % 即「在本网格下已经分不出是不是同一片板」。
    /// 比它松就不是「量不出差别」，是「差别被容忍了」，两者完全不同。
    /// </summary>
    public const double Tol = 0.02;
