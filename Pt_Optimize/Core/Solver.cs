...
public static class Solver
{
                                     ...
                                     CancellationToken cancel = default)
    {
        ...
        double tLo = SnapUpToGridMm(d.DiscFloorMm(baseIn), opt.QuantThickMm);
        for (int j = 0; j < np; j++)
        {
            d.TabThickMm[j] = tLo;
                                       ...
                                       CancellationToken cancel, IProgress<string>? progress = null)
    {
        ...
        // ★★★★★ R48 L 续（2026-09-17，Opus 5）：**改调 ApplyCaseMesh —— 手抄过来的那一行只搬了「FineMm > 0」那一支。**
        //   漏掉的是 ApplyCaseMesh 的第一支「FineMm ≤ 0 且 FineRadiusMm > 0」（导航档也要统一细区半径）：
        //     求根（EvalRaw → ApplyCaseMesh）：细区半径 = lastOpt.FineRadiusMm（本轮 59）
        //     终局复核（此处，改前）        ：什么都不做 ⇒ 退回 LineCase 默认 **50**
        //   细区半径这一维的实测效果在 Solve 里 navOpt 那段记着：同一设计、同一 2.0 mm，只差这一维 ⇒
        //   管孔净流入 **+3.267（R=50）对 −6.533（R=59）**，差 9.8 W、**符号相反**（限值 0）。
        //   ⇒ 凡是「第二遍没跑（okNav 为假，或 FineMm = 0）而 FineRadiusMm > 0」的跑法，
        //     终局复核判的都是另一张网格，而两边都不报错 —— 本项目最怕的「看起来正常的错数」。
        //   现在配方只有 ApplyCaseMesh 一处，这里一行都不许再抄（门：R48LFinishMeshGateTests）。
