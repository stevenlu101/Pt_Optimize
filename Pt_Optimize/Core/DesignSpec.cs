...
public sealed class DesignSpec
{
    ...
    ///   在此之前这里写着 **20**（= 40 圈），**仓库里查不到出处**（R48DiscInsulLeverTests 当时就点了名）——
    ///   也就是说现役两份可交付设计的圆盘保温一直站在一个现场缠不出来的数上。
    ///   ⚠ 上限不在这里写死成字面量：数由 <see cref="WrapLimits.JointZoneMaxMm"/> 给，
    ///     越界由硬安全线「接合区保温缠得出来」（<c>LineResult.Key.WrapTurns</c>）判不可行并点名，
    ///     界面控件的上限也取同一处 —— 手抄的那一份不会跟着层厚改。
    ///   ⚠ 「盘大时外区可以超 10」现在**表达不了**：圆盘保温整盘只有一个厚度（见 WrapLimits 的说明），
    ///     要分内外环得先在 PlateCurrent2D／ShellThermal 里加逐半径厚度。没做的事不许写成已经有。
    /// </summary>
    public double FlangeInsulMm = 10.0;
