...
public sealed class DesignSpec
{
    ...
    /// ⚠ 存档（DesignSpecStore）与界面还没接 —— 目前只给探针与求解器改造用，接之前不许在界面上假装有这个量。
    /// </summary>
    ...
    /// 加 Plate（DiscInsulThickMm = DiscInsulMm[j]）再经 FlangePlate.DiscInsulEffectiveMm 取出来的逐位相同。
    /// 配套清单与安装报告从这里取，不再读页面的 DesignInputs（那份在 BuildCase 之前，数可能不是算的那个）。
    /// </summary>
    public double DiscInsulMmOf(int j)
        => !FlangeInsulated ? 0.0 : (j >= 0 && j < DiscInsulMm.Length ? DiscInsulMm[j] : FlangeInsulMm);
    ...
    public LineCase BuildCase(DesignInputs baseInputs, bool checkRamp = true)
    {
        ...
        return new LineCase
        {
            ...
            HasPerPlateDiscInsul = DiscInsulMm.Length > 0,   // R48
