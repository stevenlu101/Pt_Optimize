...
public class R48ErrorBudgetTests
{
    ...
    public void 整线工作点上的单片离散误差预算()
    {
        ...
        //   证据头里的网格／热解配方要实建网格才知道，建网格函数就位后才打开（下面 ev 赋值处）；在那之前 Say 只进缓冲，打开时整份写出。
        ...
        (LineCase lcH, FlangePlate g, ShellMesh m, DesignInputs p2) Setup(int j, string group, double h)
        {
            ...
            var p20 = SegmentSolver.Clone(lcH.Base); p20.TSetC = tSet0;
            if (j < lcH.ClampTempC.Length) p20.BusbarClampTempC = lcH.ClampTempC[j];
            return (lcH, g, m, p20);
        }

        (double q, double disc, int cells, string note) Solve(int j, string group, double h, double tr)
        {
            var (lcH, g, m, p2) = Setup(j, group, h);
        ...
        {
            // R48 G1（2026-09-15，Opus 5）：证据头 —— 三组各在片0 h=1 上实建一张网格，配方取网格与热解自己记的（ShellMesh.Recipe／ShellThermal.RecipeFor）
            var meshLines = new List<string>(); var thLines = new List<string>();
            foreach (var grp in groups)
            {
                var (_, g0, m0, p20) = Setup(0, grp, 1.0);
                meshLines.Add($"{grp}（片0 h=1 实建；其余片、其余档同开关）：{m0.Recipe!.Describe()}");
                thLines.Add($"{grp}：{ShellThermal.RecipeFor(m0, p20, insulDiscRadiusMm: g0.InsulDiscRadiusMm).Describe()}");
            }
            ev = EvidenceFile.Open(file, EvidenceHeader.Build("R48 离散误差预算（整线工作点上的单片离散误差）", meshLines, thLines, double.NaN, null, false));
