...
public class R48DiscInsulPerPlateGateTests
{
    ...
    public void 解析路径_逐片5与整线5逐位相同_整线20对照不同()
    {
        ...
        string file = Path.Combine(HandoverDoc.Root(), "deliverable", "R48_逐片圆盘保温下游行为门_第二版_2026-09-14.txt");
        ...
        foreach (var r in new[] { rA, rB })
        {
            ...
            Assert.False(double.IsNaN(Row(r, LineResult.Key.RampField).Actual), Row(r, LineResult.Key.RampField).Note);
        ...
        bool rampDiff = diffBC.Any(s => s.StartsWith($"判据「{LineResult.Key.RampField}」实际", StringComparison.Ordinal));
    ...
    {
        var sb = new StringBuilder();
        string file = Path.Combine(HandoverDoc.Root(), "deliverable", "R48_逐片圆盘保温下游行为门_图纸路径_第二版_2026-09-14.txt");
        void Say(string s) => this.Say(sb, file, s);
    ...
    public void 圆盘保温0mm时四个消费方都按裸铂表面()
    {
        ...
        var mesh = FlangeMesher.Build(g, 0, 4.0, 11.0, 50.0, 40.0, clampBandMm: double.NaN, clampFullFace: true);   // 合并 A/D（2026-09-15 Opus 5）：只验表面口径，取生产配方（A 路门 g 要求证据探针显式写两个开关）
