...
public class R48LEndToEndTests
{
    ...
    public void 端到端_求解再过三关(string which)
    {
        ...
        if (sr.Undetermined)
            W("　⚠ 判不了：" + sr.UndeterminedWhy.Replace("**", "") + "（既不当过也不当不过；再算一次不一定得到同一句话）");
        ...
        foreach (var c in emptyRes.Checks) W("    " + Criteria.OneLine(c));
