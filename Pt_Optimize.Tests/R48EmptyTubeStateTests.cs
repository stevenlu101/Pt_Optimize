...
///   D/E. 空管·沿用生产设定 + 管腔辐射 kA 0.023／0.056 W·m/K（DesignInputs.TubeCavityRadKAWmPerK 注释里的两档估计）：
...
public class R48EmptyTubeStateTests
{
    ...
    // ── 管腔辐射敏感度两档（出处：DesignInputs.TubeCavityRadKAWmPerK 注释里的估算：ℓ = 40 mm 串联表面热阻 0.023；与 ℓ 自洽 0.056）
    private const double RadKALow = 0.023, RadKAHigh = 0.056;
    ...
    private void Run(string tag, bool emptyTube, EmptyTubeSetpoint sp = EmptyTubeSetpoint.AsGiven, double radKA = 0, bool fineMesh = true)
    {
        ...
        var p = new DesignInputs { SplitSharedFlangeDraw = true, TubeCavityRadKAWmPerK = radKA };
        ...
        Say($"管腔辐射 kA 钩子 {lc.Base.TubeCavityRadKAWmPerK:0.000} W·m/K（0 = 生产模型，只有管壁导热）　端部额外保温 {lc.Base.EndInsulExtraMm:0.0} mm");
    ...
    {
        var dir = Path.Combine(Root(), "deliverable");
