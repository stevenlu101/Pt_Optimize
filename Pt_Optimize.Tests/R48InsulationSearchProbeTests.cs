...
public class R48InsulationSearchProbeTests
{
    ...
    {
        string file = Path.Combine(HandoverDoc.Root(), "deliverable", fileName);
        if (File.Exists(file)) throw new InvalidOperationException($"{fileName} 已存在 —— deliverable 下已有文件不许覆盖，换新文件名再跑");
