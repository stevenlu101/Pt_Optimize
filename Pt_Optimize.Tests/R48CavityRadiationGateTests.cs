...
public class R48CavityRadiationGateTests
{
    ...
    public void 带玻璃_系数给多少三个指纹都逐位不变()
    {
        ...
        foreach (double k in new[] { 0.0, SegmentSolver.CavityRadKA1150Low, SegmentSolver.CavityRadKA1150High, 1.0 })
        {
            ...
            // ★★ 2026-09-18 Opus 5（合并 H×合并树，**重录并写明变因**）：三个记录值随 R48EmptyTubeGateTests.带玻璃_逐位不变_三个算例指纹 一起重录。
            //   变因实测定死（见那一档的注记）：合并前后逐字段比，进指纹的 17 项里只有 TauWithGlassS 变，场与电流逐位相同；
            //   唯一变因 = H 路把纯铂比热改成 Kaye & Laby 四点最小二乘。本门守的「系数给多少三个指纹都逐位不变」没有放宽。
            // ★ R48 M（2026-09-18，Fable 5.1，改门写明变因）：段解地板降十倍 ⇒ 记录值随 R48EmptyTubeGateTests 一起重录（NewFloorFp*，那一档同时在旧地板下核旧值）。
            //   本门守的「系数给多少三个指纹都逐位不变」没有放宽。
            Assert.Equal(R48EmptyTubeGateTests.NewFloorFpA, R48EmptyTubeGateTests.Fingerprint(a));
            Assert.Equal(R48EmptyTubeGateTests.NewFloorFpB, R48EmptyTubeGateTests.Fingerprint(b));
            Assert.Equal(R48EmptyTubeGateTests.NewFloorFpC, R48EmptyTubeGateTests.Fingerprint(c));
