...
public class R48CavityRadiationGateTests
{
    ...
    public void 带玻璃_系数给多少三个指纹都逐位不变()
    {
        ...
        foreach (double k in new[] { 0.0, SegmentSolver.CavityRadKA1150Low, SegmentSolver.CavityRadKA1150High, 1.0 })
        {
            ...
            Assert.Equal(0xC7FC3FCBD3F4E3E9UL, R48EmptyTubeGateTests.Fingerprint(a));
            Assert.Equal(0x51513712C997642EUL, R48EmptyTubeGateTests.Fingerprint(b));
            Assert.Equal(0x81B62A725D0769CDUL, R48EmptyTubeGateTests.Fingerprint(c));
