...
///   · 2026-09-15 Opus 5（合并）：默认评估函数的式子与限值只调 B 的唯一实现（ThermocoupleBasis.HotSideK／ColdSideK、整线算例的两个限值），
///     整线工作点／配方自检／内层网格走 Solver.ApplyCaseMesh（源码门，辅助）。
///     2026-09-15 Opus 5（合并，复审后改）：删去原句「行为上的逐位核在 WorkPointOf 与配方自检里」—— 不成立：配方自检的两张网格是同一个 ApplyCaseMesh 调用建的，
///     导航支退回细区半径 50 也照样逐位相同、查不出。行为门是随 E 最终版合入的 R48InsulationSearchClosureTests.网格配方_导航支统一半径_加密支自相似。
/// </summary>
public class R48InsulationSearchEngineTests
{
    ...
    {
        var o = new Options();
        // 2026-09-15 Opus 5（合并）：限值不再从 Options 读（原 Options.HotLimitK／ColdLimitK 各 5.0 已删），而是整线算例 LineCase 的两个限值经工作点带进来；
        //   这里按一个新造算例的缺省值给（= LineCase.ThermocoupleErrorK = 5），下面对 5 的断言原样不动。
        // ★ K 路（2026-09-15，Opus 5；审查 P2-7「限值门挡不住写死 5」）：有意改动 —— 限值改用**非缺省**的 3.7／4.3 K（算例上显式给），
        //   评估函数里若把限值写死成 5（任何写法），下面按 3.7／4.3 算的归一裕度就对不上。原断言里的 5 一并换成算例的限值。
        var lcDef = new LineCase { HotOverTcMaxK = 3.7, ColdUnderTcMaxK = 4.3 };
        Assert.NotEqual(LineCase.ThermocoupleErrorK, lcDef.HotOverTcMaxK);      // 自证：真的不是缺省值
        Assert.NotEqual(LineCase.ThermocoupleErrorK, lcDef.ColdUnderTcMaxK);
        var v = new PlateStateView
        {
            Plate = 1, Shared = true, State = 0, StateName = StateGlass,
            HotLimitK = lcDef.HotOverTcMaxK, ColdLimitK = lcDef.ColdUnderTcMaxK,
        ...
        Assert.Equal(lcDef.HotOverTcMaxK, hot.Limit);                      // 2026-09-15 Opus 5（合并）：限值就是算例的那一个
        Assert.Equal(ThermocoupleBasis.HotSideK(v.ReferenceC, v.DiscPeakC, v.TabPeakC, v.RootHotC), hot.Value);   // 式子就是 B 的那一份（逐位）
        var cold = t.Single(c => c.Name == ColdName);
        Assert.Equal(v.ReferenceC - 1112, cold.Value, 12);         // 冷侧取较冷端
        Assert.Equal(lcDef.ColdUnderTcMaxK, cold.Limit);
        var inflow = t.Single(c => c.Name == InflowName);
        Assert.True(inflow.Strict);
        Assert.Equal(2.0 * 1.5 / 4.3, inflow.NormMargin, 12);
        Assert.Equal((4.3 - cold.Value) / 4.3, cold.NormMargin, 12);
        // K 路（2026-09-15，Opus 5）：三项各写上对应的整线判据；卡不卡交付查整线的分工况表（带玻璃稳态三项都卡）
    ...
    /// <summary>
    /// 源码门：搜索不许带热启动进整线（每轮冷启动），整线算例由 BuildCase 新造；判据名不带代号；层号上界的注释要说清出处。
    /// ★ 2026-09-17 Opus 5：**圆盘层上界现在有出处了**（现场缠绕圈数，WrapLimits.MaxTurnsAtJoint）⇒ 它不再要求写「无出处」，
    ///   改为要求它**取生产配方那一份、并写明出处**；管层与舌层仍然无出处，那两条原样守着。
    ///   这是加严不是放宽：原来只要注释里有「无出处」三个字就算过，现在圆盘那条还得真的接到 WrapLimits 上。
    /// ★★ **2026-09-18 Opus 5 退回**（用户当日「还是只给材质保温厚度方案就行」）：缠绕圈数已由硬安全线降为参考行
    ///   ⇒ 它**不再给这条搜索上界当出处**，圆盘层上界退回 §0.-11 之前那个无出处的 **40**。
    ///   ⇒ 本门把圆盘那条并回「三条都要写明无出处」—— 这不是放宽：口径变了，照实标才是严的那一边
    ///   （假装它还有出处，才是本项目最忌的那种「看起来有依据」）。退回值本身由
    ///   <c>R48LWrapTurnsGateTests.门_圆盘保温搜索层上界退回40_并照实标无出处</c> 另钉一道。
    /// </summary>
    [Fact]
    public void 源码门_冷启动_无判据代号_上界写明无出处()
    {
        ...
        // 2026-09-18：三条上界都无出处（圆盘那条当天退回 40，缠绕圈数不再给它当出处）
        foreach (var field in new[] { "TubeLayerMax", "TabLayerMax", "DiscLayerMax" })
        {
            var m = Regex.Match(src, @"///[^\n]*无出处[^\n]*\n\s*public int " + field);
            Assert.True(m.Success, $"{field} 的注释没写「无出处」");
        }
        Assert.DoesNotContain("public int DiscLayerMax = WrapLimits.MaxTurnsAtJoint;", src);
        Assert.Equal(40, new InsulationSearch.Options().DiscLayerMax);
    }

    /// <summary>
    /// ★ 2026-09-15 Opus 5（合并）：源码门（辅助）—— E 路 open issue 与审查 major 修过之后不许回来：
    ///   · 判据式子不许再手写（原 Math.Max(Math.Max(盘峰, 舌区峰), 管根) − 基准、基准 − 管根），要调 ThermocoupleBasis.HotSideK／ColdSideK；
    ///   · 基准不许再自调 ThermocoupleBasis.ReferenceC(设定数组, 片号)，要取 ThermocoupleBasis.At；限值不许在选项里另有一份；
    ///   · 整线／内层网格不许再就地调 MeshAdapt.RefineWholeMesh（≤ 0 那一支漏了导航配方），要调 Solver.ApplyCaseMesh。
    /// </summary>
    [Fact]
    public void 源码门_判据只调唯一实现_网格走ApplyCaseMesh()
    {
        string src = File.ReadAllText(Path.Combine(HandoverDoc.Root(), "Pt_Optimize", "Core", "InsulationSearch.cs"));
        var code = string.Join("\n", src.Split('\n').Select(l => l.Trim()).Where(l => !l.StartsWith("//", StringComparison.Ordinal)));
        Assert.DoesNotContain("Math.Max(Math.Max(", code);
        Assert.Contains("ThermocoupleBasis.HotSideK(", code);
        Assert.Contains("ThermocoupleBasis.ColdSideK(", code);
        Assert.DoesNotContain("ThermocoupleBasis.ReferenceC(", code);
        Assert.Contains("ThermocoupleBasis.At(", code);
        Assert.DoesNotMatch(new Regex(@"\bHotLimitK\s*=\s*\d"), code);
        Assert.DoesNotMatch(new Regex(@"\bColdLimitK\s*=\s*\d"), code);
        Assert.Contains("c.HotOverTcMaxK", code);
        Assert.Contains("c.ColdUnderTcMaxK", code);
        Assert.DoesNotContain("MeshAdapt.RefineWholeMesh(", code);
        Assert.True(Regex.Matches(code, @"Solver\.ApplyCaseMesh\(").Count >= 3, "整线工作点、配方自检、内层网格三处都要调 Solver.ApplyCaseMesh");
        // Solver 自己也只剩这一份（EvalRaw 调它）
        string sol = File.ReadAllText(Path.Combine(HandoverDoc.Root(), "Pt_Optimize", "Core", "Solver.cs"));
        Assert.Contains("public static void ApplyCaseMesh(LineCase lc, SolverOptions o)", sol);
        Assert.Contains("ApplyCaseMesh(lc, o);", sol);
    }
}
