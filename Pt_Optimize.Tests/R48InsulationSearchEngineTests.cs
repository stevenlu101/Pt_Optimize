...
public class R48InsulationSearchEngineTests
{
    ...
    public void 源码门_冷启动_无判据代号_上界写明无出处()
    {
        ...
        foreach (var field in new[] { "TubeLayerMax", "TabLayerMax" })
        {
            var m = Regex.Match(src, @"///[^\n]*无出处[^\n]*\n\s*public int " + field);
            Assert.True(m.Success, $"{field} 的注释没写「无出处」");
        }
        // 圆盘层上界：接到唯一那份现场缠绕上限，且注释里要有出处（不许退回「无出处的 40」）
        Assert.Matches(new Regex(@"///[^\n]*出处[\s\S]{0,400}?public int DiscLayerMax = WrapLimits\.MaxTurnsAtJoint;"), src);
        Assert.DoesNotContain("public int DiscLayerMax = 40;", src);
