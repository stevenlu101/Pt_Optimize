...
public class R48ClampPhaseTests
{
    ...
    public void 实验b_压接边界落节点后钉住长度与舌区电流平方积分跨半径对照()
    {
        ...
        void Say(string s)
        {
            ...
            try { File.WriteAllText(DeliverableOut.Stamped("R48_实验b_压接边界落节点_2026-09-14.txt"), sb.ToString(), new UTF8Encoding(false)); } catch { }
        }
            ...
            for (int j = 0; j < 4; j++)
            {
                ...
                foreach (double R in new[] { 50.0, 59.0, 90.0, double.PositiveInfinity })
                {
                    ...
                    var m = FlangeMesher.BuildFromField(tf, holeR, 0, h, uni ? h : coarseRatio * h, uni ? 1e6 : R,
                                                        clampLen, h, 0, g.TwoTabs, xa, za, clampBandMm: 0, clampFullFace: false);
