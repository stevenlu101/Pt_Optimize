...
public class R48J_SolverMeshAndMarkerGateTests
{
    ...
    public void P3_10_管根算不出不许判收敛()
    {
        ...
        Assert.False(LineRunner.CoupleConverged(bad.Delta * 25, 0.0, 10.0, bad.RootNaN, LineCase.FixedPointAmp),
                     "有段管根算不出还判了收敛 —— 判不了不算过");
        ...
        Assert.True(LineRunner.CoupleConverged(good.Delta * 25, 0.0, 10.0, good.RootNaN, LineCase.FixedPointAmp));

        // 三条各验一次
        Assert.False(LineRunner.CoupleConverged(50.0, 0.0, 10.0, false, LineCase.FixedPointAmp));   // 步长距离那条
        Assert.False(LineRunner.CoupleConverged(5.0, 1.0, 10.0, false, LineCase.FixedPointAmp));    // 真残差那条（1.0 × 25 > 10）
        Assert.False(LineRunner.CoupleConverged(5.0, 1.0, 10.0, false, 90.0));              // 放大由参数给：换成 L 路当场算的量级（90）照样不过
        Assert.False(LineRunner.CoupleConverged(5.0, 0.0, 10.0, true, LineCase.FixedPointAmp));     // 管根算不出那条

        // 源码门：耦合主循环调的就是这两份（不许有人在循环里另写一份判定）
        string s = CodeOnly(Src("Pt_Optimize", "Core", "LineRunner.cs"));
        Assert.Contains("var step = CoupleRootStep(res.Segments, next.Segments, c.SegmentCount);", s);
        // ★ 2026-09-18 Opus 5（合并 J×L 改门，写明变因）：L 路 2026-09-16／17 把主环的**放大倍数**改成当场算的 ampWorst（EndTempFixedPointAmpOf）、
        //   把**容差**改成当场算的 tolNow（CoupleTolKFor）。J 路这一版 CoupleConverged 写死 25 与 c.CoupleTolK，照搬会把 L 的两处修正悄悄退回去。
        //   合并后 CoupleConverged 的放大由参数传入，主环传 tolNow 与 ampWorst；本门两条源码断言随之改钉新写法（口径更严：常数不许回来）。
        ...
        Assert.DoesNotContain("if (CoupleConverged(remain, resK, c.CoupleTolK", s);
        Assert.DoesNotContain("resK * LineCase.FixedPointAmp < tolK", s);
    }
}
