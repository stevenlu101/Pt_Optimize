...
public class R48ThermalSourceTests
{
    ...
    public void 比热只在升温与时间常数处被读到_稳态判据不读比热()
    {
        ...
        Assert.Contains("res.TauWithGlassS = (cMetal + cGlass) /", seg);
        ...
        Assert.Contains("double CapTube(double t) => massTube * Materials.PtCp(t) + capInsulTube;", two);
        Assert.Contains("double CapFlange(double t) => massFlange * Materials.PtCp(t) + capInsulFlange;", two);
