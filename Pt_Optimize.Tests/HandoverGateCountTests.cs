...
public class HandoverGateCountTests
{
    ...
    /// <summary>
    /// ★★ **待办队列必须在仓库里**（2026-08-30）。
    ///
    /// 它此前只存在于对话里 —— 用户问「第 9 件是什么」时已经忘了一次。
    /// 在对话里的清单等于没有清单：换个会话就没了，而本项目的工作跨很多会话。
    ///
    /// ⚠ 这条门挡的不是「队列内容对不对」（那没法自动判），
    ///   而是**它有没有被整段删掉或忘记更新** —— 前者是真实风险：
    ///   本仓库的清单被「顺手清理」过（「没解决」清单那一次：我说了要写却没写，
    ///   而文档已声称写了）。
    ///
    /// ⚠ 顺带记一条**我自己反复犯的错**：HANDOVER 里那条测试计数历史长这样
    ///   「→ 337（…）→ 380（…）」—— 那些是**用例总数**，不是节号。
    ///   2026-08-29 与 08-30 我两次把那种数字当节号写进出处（这条注释里都不敢再写出来 ——
    ///   写出来就会被 DocRefTests 拦下，而它拦得对：它不认上下文，只认形式），
    ///   两次都被 DocRefTests 当场逮住。**引出处前先确认那一节真的存在。**
    /// </summary>
    [Fact]
    public void 待办队列在文档里而不是只在对话里()
    {
        string t = HandoverDoc.Text();
        Assert.Contains("待办队列", t);
        Assert.Contains("这份队列此前**只存在于对话里**", t);

        // 每一条都要有状态标记，且**未做的**要写出来 —— 只列做完的等于报喜不报忧
        Assert.Contains("❌ 未做", t);

        // 正在做的那一条要点名（否则下次又要问「第几件是什么」）
        Assert.Contains("第 9 件是什么", t);
    }
}
