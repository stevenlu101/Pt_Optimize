...
public class R48InsulByRadiusTests
{
    ...
    public void 整线与求解器接线都走同一条规则()
    {
        ...
        // ★ R48（2026-09-14，Opus 5）有意改断言：旧「insulDiscRadiusMm: insulRForZone」→ 新「s.InsulDiscRadiusMm = insulRForZone」＋「insulDiscRadiusMm: s.InsulDiscRadiusMm」。
        //   原因：审查意见「门手抄了逐片热解配方」，逐片热解输入组装搬进 LineRunner.PlateThermalInputs、调用搬进 LineRunner.SolvePlateThermal（纯搬移），
        //   接线从局部变量改经 PlateThermalSetup 字段过手；两段合起来仍是「保温半径真的接进 ShellThermal」。依据文件：Pt_Optimize/Core/LineRunner.cs。
        Assert.Contains("s.InsulDiscRadiusMm = insulRForZone", lr);
        Assert.Contains("insulDiscRadiusMm: s.InsulDiscRadiusMm", lr);
