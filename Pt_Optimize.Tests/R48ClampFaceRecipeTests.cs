...
///   ★ 2026-09-15 Opus 5（合并，复审后改）：首跑在 r48_F 工作树（底板 f206e70），法兰表面散热表上限还是「设定 + 200 K、60 节点」；合并树已改铂熔点上限。
///     合并树上按**原判读原门槛**（两片三档「分级不带细带 − 均匀」抽热都 ≤ 0.05 W）重跑一次，**跑前写死**：一字不改上面的判读；
///     结果只落在带开跑时刻的新文件里（见 Report.Writer），报告头另印代码指纹与散热表上限，免得与 r48_F 那份混读；
///     若有档 &gt; 0.05 W ⇒ 细带缺省 0 的依据在合并树配方下不成立，报出来交数值把关人，本轮不改生产缺省。
///     跑后（同日 15:36，deliverable/R48_压接面上定温_细带去留_2026-09-15_本次开跑于2026-09-15_1536.txt，54 s）：两片三档最大偏差 0.0045 W ≤ 0.05 ⇒ 判读仍成立；各档差值与格数与首跑结论所引四位小数逐个相同。
///
...
public static class R48ClampFaceRecipeTests
{
        ...
        public static Action<string> Writer(string file, int section, ITestOutputHelper o)
            => s =>
            {
                ...
                lock (Gate)
                {
                    ...
                    // ★ 2026-09-15 Opus 5（合并）：G1 的证据防覆盖（EvidenceFile：被 Core 注释引作依据的证据，重跑不许用同名文件整份覆盖）写于 F 之前，
                    //   没管到 F 这几个探针 —— 而 FlangeMesher.ClampBandPerHFine 引的 R48_压接面上定温_细带去留_2026-09-15.txt 正是这里写的。
                    //   证据头（代码指纹、配方）没有加：这几个探针逐行显式传配方参数，头要按实建网格填，留给交接 open issue。
                    // 2026-09-15 Opus 5（合并，复审后改）：**不再写不带时刻的同名文件**，只写下面带开跑时刻的那份。
                    //   合并时的写法是「同名文件本进程第一次写之前不存在 ⇒ 算本进程拥有、照旧整份写」—— 复审查出：合并树（r48_M）的 deliverable 里
                    //   本来就没有 F 路那几份被 Core 注释引作依据的证据（它们在 r48_F 工作树里），于是一跑就按合并树的配方（铂熔点散热表等）
                    //   **新建**出同名文件、冒充被引的那一份。不带时刻的文件名只属于 F 路原跑；此后任何重跑都只落在带开跑时刻的新文件里。
                    try
                    {
    ...
    public static void Header(string file, ITestOutputHelper o, string title)
    {
        ...
        // 2026-09-15 Opus 5（合并，复审后改）：印代码与散热表上限（F 首跑时表上限是「设定 + 200 K」，合并树是铂熔点）—— 同名证据分不清是哪份代码跑的，是合并复审查出的出处问题
        string root = HandoverDoc.Root();
        say($"代码：git HEAD {EvidenceHeader.GitHead(root)}；Core 相对 HEAD 的改动指纹 {EvidenceHeader.CoreDiffSha1(root)}；法兰表面散热表上限 ShellThermal.LossTableHiC = {ShellThermal.LossTableHiC:0.#} °C；"
          + $"生产细带缺省 FlangeMesher.ClampBandPerHFine = {FlangeMesher.ClampBandPerHFine}（本探针各组显式传细带，不读缺省）。");
    }
    ...
    public static ConvResult RunConvergence(int caseIdx, ITestOutputHelper o)
    {
        ...
        //   （分支镜像 2026-09-23 09:15 实跑 21.2501／21.7278／22.0078，按门自己的「打印三位 ⇒ 容差 0.0005」取三位；Linux 数，与 Windows 差 1e-9 量级远在容差内；
        //   证据：印出三档新值的那跑 deliverable/R48_压接面上定温_收敛阶_2026-09-15_本次开跑于2026-09-23_0915.txt（对旧记录红），重录后复跑过 …_0938.txt）。三档都往下走 0.72／0.31／0.13 W：F6 把孔面上的电流出口从格形心挪回孔圆，孔边抽热随之变。
