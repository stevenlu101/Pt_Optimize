...
public class R48ClampRecipeImpactTests
{
    ...
    public void 一_组合配方收敛_整面均匀对整面自相似细带分级_四片()
    {
        ...
        Say("      「组合」= MeshAdapt.RefineWholeMesh(h) 分级网格 + 自相似压接细带（clampBandMm NaN = 两侧各 3×hFine、clampFullFace true）= 生产配方。");
        ...
        for (int j = 0; j < 4; j++)
        {
            ...
            var G = new Dictionary<double, Sol>();
            {
                ...
                var mN = FlangeMesher.Build(gN, 0, lcN.MeshFineMm, lcN.MeshCoarseMm, lcN.MeshFineRadiusMm, lcN.Base.BusbarClampLengthMm,
                                            lcN.MeshInnerMm, lcN.MeshInnerRadiusMm, clampBandMm: double.NaN, clampFullFace: true);
            ...
            foreach (double h in hs)
            {
                ...
                var mU = FlangeMesher.Build(gU, 0, h, h, 1e6, lcU.Base.BusbarClampLengthMm, clampBandMm: 0, clampFullFace: true);
                ...
                var mG = FlangeMesher.Build(gG, 0, lcG.MeshFineMm, lcG.MeshCoarseMm, lcG.MeshFineRadiusMm, lcG.Base.BusbarClampLengthMm,
                                            lcG.MeshInnerMm, lcG.MeshInnerRadiusMm, clampBandMm: double.NaN, clampFullFace: true);
    ...
    public void 二_片0配方影响_抽热_圆盘区温差_J峰_发热()
    {
        ...
        Say("口径：外圈 = clampFullFace false（改动前）；整面 = clampFullFace true；细带 3 mm／自相似 = clampBandMm 3／NaN。");
        var (d, p) = Design();
        var (_, radius) = MeshVerify.RequiredMeshFor(d);
        ...
        foreach (double h in new[] { 1.0, 0.5, 0.25 })
        {
            ...
            Say(Row($"整面 h={h}", Solve(d, lc, 0, FlangeMesher.Build(g, 0, h, h, 1e6, lc.Base.BusbarClampLengthMm, clampBandMm: 0, clampFullFace: true), g)));
        }
        foreach (double h in new[] { 0.0, 1.0 })
        {
            ...
            ShellMesh B(double band, bool full) => FlangeMesher.Build(g, 0, lc.MeshFineMm, lc.MeshCoarseMm, lc.MeshFineRadiusMm, lc.Base.BusbarClampLengthMm,
                                                                    lc.MeshInnerMm, lc.MeshInnerRadiusMm, clampBandMm: band, clampFullFace: full);
            Say(Row("老口径（外圈、不铺细带）", Solve(d, lc, 0, B(0, false), g)));
            Say(Row("外圈 + 固定 3 mm 细带", Solve(d, lc, 0, B(3.0, false), g)));
            Say(Row("外圈 + 自相似细带", Solve(d, lc, 0, B(double.NaN, false), g)));
            Say(Row("整面、不铺细带", Solve(d, lc, 0, B(0, true), g)));
            Say(Row("新配方（整面 + 自相似细带）", Solve(d, lc, 0, B(double.NaN, true), g)));
        }
        Say("");
    ...
    public void 三_孔位对比x185那一行的分解()
    {
        ...
        Say($"板同 HolePlacementTests.Heater1：舌尖 x {g0.TabTipXMm}、压接长 {clampLen} mm（DesignInputs 缺省）⇒ 压接段 x ∈ [{g0.TabTipXMm}, {g0.TabTipXMm + clampLen}]；R8 孔；1000 A；"
          + "网格 hFine 2、hCoarse 8、细区半径 50（HolePlacementTests.Run 同一调用，只多写两个开关）。");
        Say("列：峰值 J 与所在格形心、孔边最小格边（形心距孔心 < 孔半径 + 1.5 mm 的格，√面积）、单元数。");
        var configs = new (string name, double band, bool full)[]
        {
            ("老口径（外圈、不铺细带）", 0, false), ("只开整面接触", 0, true), ("只铺自相似细带", double.NaN, false), ("新配方（两个都开）", double.NaN, true)
        };
        foreach (double? hx in new double?[] { null, -20, -50, -100, -150, -185 })
        {
            ...
            foreach (var (name, band, full) in configs)
            {
                var mesh = FlangeMesher.Build(g, 0, 2.0, 8.0, 50.0, clampLen, 0, 0, clampBandMm: band, clampFullFace: full);
                ...
                if (double.IsNaN(band) && full)
                {
                    // 防漂移：本探针的复刻与 HolePlacementTests.Run（生产缺省）逐位同一个峰值
    ...
    public void 四_整线集总模型排除压接格前后()
    {
        ...
        foreach (var f in r.Flanges)
            Say($"   {f.Name,-8} 发热 {f.QGenW:0.000} W　抽热 {f.QFromTubeW:+0.000;-0.000} W　管根 {f.TRootC:0.00} °C　单元 {f.CellCount}"
              + $"　压接格 {f.Mesh?.ClampCell.Count(b => b) ?? -1}　{(f.MeshRecipe?.ClampBandMm > 0 ? "有细带" : "**没细带**")}");
        ...
        Say("");
        Say("✓ 判据表里的两条参考量就是 excludeClampCells = true 那一份。");
        Say("");
        Say("── 判据表（生产配方，导航网格）");
