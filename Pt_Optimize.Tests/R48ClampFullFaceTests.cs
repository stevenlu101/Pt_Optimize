...
public class R48ClampFullFaceTests
{
    ...
    public void 压接段只钉外圈与整面接触对照()
    {
        ...
        // ★ R48 G1（2026-09-15，Opus 5；数值把关人第十三轮）：写文件改走 EvidenceFile —— 文件头写明代码（git HEAD、Core 改动指纹）与配方；
        //   同名文件已存在且头不同（或是没有头的旧证据）⇒ 不覆盖，改写到带时间戳的新文件。算法一行不动。
        //   证据头里的网格／热解配方要实建网格才知道，算例建好后才打开（下面 ev 赋值处）；在那之前 Say 只进缓冲，打开时整份写出。
        ...
        // R48 G1（2026-09-15，Opus 5）：建网格与本片物性从 Solve 里原样提成 Setup（证据头要用同一份实建网格的配方），调用与参数逐字未改；
        //   p2 的组装原在电流场解之后，挪到之前 —— 它不读电流场，数不变。
        (FlangePlate g, ShellMesh m, DesignInputs p2) Setup(int j, double h, bool full)
        {
            ...
            var m = FlangeMesher.BuildFromField(tf, g.HoleRadiusMm, 0, h, h, 1e6, clampLen, h, 0, g.TwoTabs, xa, za, clampBandMm: 0, clampFullFace: full);
            double tSet0 = d.SetpointC[Math.Min(j, d.SetpointC.Length - 1)];
            var p20 = SegmentSolver.Clone(lc.Base); p20.TSetC = tSet0;
            if (j < lc.ClampTempC.Length) p20.BusbarClampTempC = lc.ClampTempC[j];
            p20.FlangeInsulThickMm = g.DiscInsulEffectiveMm(lc.Base);
            return (g, m, p20);
        }
        {
            var meshLines = new List<string>(); var thLines = new List<string>();
            foreach (bool full in new[] { false, true })
            {
                var (g0, m0, p20) = Setup(0, 1.0, full);
                string lab = full ? "B 整面接触" : "A 只钉外圈";
                meshLines.Add($"{lab}（片0 h=1 实建；片1、其余档同开关，均匀网格）：{m0.Recipe!.Describe()}");
                thLines.Add($"{lab}：{ShellThermal.RecipeFor(m0, p20, insulDiscRadiusMm: g0.InsulDiscRadiusMm).Describe()}");
            }
            ev = EvidenceFile.Open(file, EvidenceHeader.Build("R48 压接段只钉外圈 vs 整面接触", meshLines, thLines, double.NaN, null, false));
