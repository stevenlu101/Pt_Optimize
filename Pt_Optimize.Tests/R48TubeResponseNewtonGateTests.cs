...
/// （2026-09-15 Opus 5（I 路） 注：这句一次性实测没有文件；第一版输出第 13–20 行的实数是共用片带玻璃 194～207 轮到 0.25 K、每轮约 1.9 s，空管 300 轮停在 0.30 K；端片第 5–12 行 1 轮 0.10～0.14 K。）
///
/// ══ 公共设置（跑前定下）
...
///   判读 M0～M4 本身不依赖这些数。合并后**未重跑**；输出文件同名已存在时本门拒跑（不覆盖）—— 2026-09-15 Opus 5 I 路改：原文件在合并树里已存在 ⇒ 本门必抛；现只写带开跑时刻的新文件（DeliverableOut），重跑后在此重记。
/// </summary>
[Trait("速度", "慢")]
public class R48TubeResponseNewtonGateTests
{
    ...
    {
        // 2026-09-15 Opus 5（I 路）：原按原文件名写 deliverable（会覆盖被引证据）→ 只写带开跑时刻的新文件（DeliverableOut，门 R48DeliverableWriteGuardTests）；原为「同名已存在就抛」，合并树里原文件已在 ⇒ 本门必抛
        string file = DeliverableOut.Stamped("R48_保温搜索_管侧响应_两种解法对拍_第二版_2026-09-15.txt");
        if (File.Exists(file)) throw new InvalidOperationException("带时刻的输出文件已存在（同一秒开跑两次？）—— 不覆盖：" + file);
        ...
        for (int s = 0; s < 2; s++)
        {
            ...
            Say($"M2 {InsulationSearch.StateNames[s]}：‖(I − J)⁻¹‖∞ = {amp[s]:0.0}（整线外层耦合**历史口径**常数 FixedPointAmp = {LineCase.FixedPointAmp}；R48 L 2026-09-16 起生产当场算 1+ℓt/Δx）");
