...
public class R48LCoupleTolGateTests
{
    ...
    {
        // ★ R48 M（2026-09-18，Fable 5.1，改门写明变因）：生产默认已改成**绝对目标**（CoupleTolFromMargin = false，见 LineCase.CoupleTolK 注记与 HANDOVER §0.-16M）；
        //   这里显式打开开关只为验公式本身（注射那条路还要用它），并把「按裕度」那个上限钉回 0.1 —— 公式的三段与生产默认无关。
        var lc = new LineCase { CoupleTolFromMargin = true, CoupleTolK = 0.1 };
        Assert.False(new LineCase().CoupleTolFromMargin, "生产默认必须是绝对目标（不按判据裕度收紧）—— R48 M 2026-09-18");
    ...
    {
        var lc = new LineCase { CoupleTolK = 0.005, CoupleTolFromMargin = true };   // R48 M（2026-09-18，Fable 5.1）：开关显式开
    ...
    {
        var lc = new LineCase { CoupleTolFromMargin = true };   // R48 M（2026-09-18，Fable 5.1）：验的是「按裕度」那条路上「没判据可读」的退路，开关要显式开
    ...
    public void 门_证据头必须写明容差口径()
    {
        ...
        // ★ R48 M（2026-09-18，Fable 5.1，改门写明变因）：生产默认改成绝对目标；「按判据裕度收紧」只剩注射口径，头上要写明
        Assert.Contains("绝对目标", head);
        Assert.Contains("CoupleTolKUsed", head);

        R48LCoupleTolKit.ApplyTol(lc, R48LCoupleTolKit.TolMode.FromMargin);
        string head2 = EvidenceHeader.ForLineCase("门自证", lc);
        Assert.Contains("按判据裕度收紧", head2);
        Assert.Contains("注射口径", head2);
    }
