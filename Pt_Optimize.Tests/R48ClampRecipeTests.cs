...
///   · ★ R48 F（2026-09-15 Opus 5）：配方 ⑤ 压接面上定温进生产默认；整面 + 面上定温下重验细带，**缺省改为不铺**
///     （deliverable/R48_压接面上定温_细带去留_2026-09-15.txt：分级不带细带 − 均匀 ≤ 0.0045 W）。(b)(d)(h) 随之改写，面上定温自己的快门在 R48ClampFaceGateTests。
///
/// ══ 八道门（工单四道 + 第一轮补的两道 + 复审补的两道）
///   (a) 解析板片0 均匀 h=2：默认网格的 ClampCell 数 = 形心在压接段内的格数 = 舌宽/h × 压接长/h（直舌的闭式数），
///       外圈格（压接边界面相邻格）全在整面格里；与老口径外圈格数比。
///   (b) 【2026-09-15 改：缺省入口不铺细带、窗口里有粗格；显式 3·hFine 时下述机制照旧】**不带两个开关**调 Build（导航网格与 h=1 分级网格两种）：压接边界两侧 ResolveClampBandMm(NaN, hFine) 内的 x 向节点间距细到 hFine 量级；
...
///       【2026-09-15 Opus 5：散热表换过之后，逐位比用只供测试的旧表参数（设定 + 200 K、60 节点）；新表下的容差由新旧表之差推出，见门内注释】
///   (e) 退化几何（压接段盖到管孔）不填 ClampCell、退回只钉外圈，⚠ 警告进 LineRunner 输出，而且说人话（改后第一遍快套件抓到的）。
///   (f) LineRunner 建网格不覆写这两个开关（生产链读的就是默认配方）；R48 F（2026-09-15 Opus 5）补：也不覆写配方 ⑤ 压接面上定温（clampFaceDirichlet）。
///       【2026-09-15 Opus 5：降为辅助，主证据是 R48RecipeFingerprintTests 的行为门（2026-09-15 Opus 5（合并）：行为门的生产配方常量已含配方 ⑤）】
...
///       ⛔ 「偏保守」那半句 2026-09-15 撤回（R48 G2，Opus 5）：升温两节点已补铜排通道与舌片保温，附注改为说明标定与假设、写「待复核」，不下方向结论。
///
...
public class R48ClampRecipeTests
{
    ...
    private static (DesignSpec d, DesignInputs p) Design()
    {
        ...
        // ★ 2026-09-17 Opus 5：**把圆盘保温钉在取记录时的那个值上**。
        //   本类比的是**压接口径**（整面接触、细带、舌端边界三模式），逐位记录取自基线树
        //   （deliverable/R48A_基线树老口径记录_2026-09-14.txt），那一版的圆盘保温默认是 20 mm。
        //   同日把默认改成 10 mm（现场只缠得了 20 圈，用户 2026-09-17）之后，这里若跟着默认走，
        //   逐位记录会因为**一个与压接无关的输入**而对不上（实测抽热 −9.646 → +1.479 W）——
        //   那不是压接口径变了，是比的东西变了。⇒ 显式钉住，记录才还有对象。
        //   ⚠ 这里显式钉 20 的**理由**是「记录要有对象」，与生产默认是多少无关。
        //   ★ 2026-09-18 更正这条注释：当时写的是「生产默认与上界都已经是 10」——
        //     用户当日「还是只给材质保温厚度方案就行」之后，缠绕圈数降为参考行，
        //     生产默认与界面上界都**退回 20 mm／60 mm**（R48LWrapTurnsGateTests 守着）。
        //     也就是说这一行现在**恰好**与生产默认相等；显式写死照旧保留，免得默认哪天再动、记录又失去对象。
    ...
    /// </summary>
    private static (ShellCurrentResult sc, ShellThermalResult th, DesignInputs p2) SolvePlate0(DesignSpec d, LineCase lc, FlangePlate g, ShellMesh m, string mode = "定温",
                                                                                           double lossTableHiC = double.NaN, int lossTableNodes = 0)
    {
        ...
        // lossTableHiC／lossTableNodes：只有门 d 传（复现基线树的表，2026-09-15 Opus 5）；缺省 = 生产表
    ...
    /// <summary>
    /// (b) ★ R48 F 改（2026-09-15 Opus 5）：生产缺省**不铺**压接细带（FlangeMesher.ClampBandPerHFine 3.0 → 0.0，依据 deliverable/R48_压接面上定温_细带去留_2026-09-15.txt）。
    /// 改后门的两半：缺省入口（解析 Build 与图纸 BuildFromField）不铺、记录里没有「铺细步」、窗口里有远场粗格；
    /// 显式传 2026-09-14 的自相似宽度 3·hFine 时机制照旧（窗口内是细步）—— 老探针靠它复现旧证据，机制不许坏。
    /// </summary>
    [Fact]
    public void b_缺省入口不铺压接细带_显式3h细带仍是细步()
    {
        var (d, p) = Design();
        foreach (var (name, lc) in GradedCases(d, p))
        {
            ...
            Assert.Equal(0.0, FlangeMesher.ClampBandPerHFine);
            Assert.Equal(0.0, FlangeMesher.ResolveClampBandMm(double.NaN, hF));
            double band = 3.0 * hF;                                     // 2026-09-14 的自相似宽度（显式传，看机制）
            ...
            // 缺省入口：不铺细带 ⇒ 记录里没有「铺细步」，同一窗口里有远场粗格；整面接触照旧
            var mDef = BuildDefault(g, lc);
            var (_, touchDef, noteDef) = Window(mDef);
            _out.WriteLine($"{name}：缺省入口 窗口里最大格 {touchDef:0.###} mm　记录：{noteDef}　配方：{mDef.Recipe!.Describe()}");
            // 2026-09-15 Opus 5（合并）：G1 的结构化配方断言为主（G1 原写「细带 = 3·hFine、规则 = 生产」，那时缺省还铺细带；F 改缺省为不铺 ⇒
            //   按合并后的生产配方：缺省入口细带 = ResolveClampBandMm(NaN, hFine) = 0、规则 = 生产常量（含配方 ⑤ 面上定温）），记录文字与窗口粗格辅助
            Assert.Equal(FlangeMesher.ResolveClampBandMm(double.NaN, hF), mDef.Recipe.ClampBandMm, 12);
            Assert.Equal(FlangeMesher.ProductionMeshRule, mDef.Recipe.Rule);
            Assert.True(mDef.Recipe.ClampFaceDirichlet, $"{name}：缺省入口的网格配方不是面上定温");
            Assert.DoesNotContain("铺细步", noteDef);
            Assert.True(touchDef > 1.25 * hF + 1e-9, $"{name}：缺省入口窗口里最大格只有 {touchDef:0.###} —— 缺省还在铺细带？");
            Assert.True(mDef.ClampCell.Length == mDef.CellCount && mDef.ClampCell.Count(b => b) > RingCells(mDef), $"{name}：缺省入口没有整面接触");

            // 机制：显式 3·hFine 时窗口内是细步（断言同改前的缺省入口）
            var mBand = FlangeMesher.Build(g, 0, lc.MeshFineMm, lc.MeshCoarseMm, lc.MeshFineRadiusMm, lc.Base.BusbarClampLengthMm,
                                           lc.MeshInnerMm, lc.MeshInnerRadiusMm, clampBandMm: band, clampFullFace: true, clampFaceDirichlet: true);
            var (ins, _, note) = Window(mBand);
            _out.WriteLine($"{name}：显式细带 hFine {hF}　细带单侧 {band}　窗口内整格 {ins.Length} 个：{string.Join(" ", ins.Select(v => v.ToString("0.###")))}　记录：{note}");
            // 2026-09-15 Opus 5（合并）：显式细带的结构化配方 —— 细带宽度照给的记、规则与生产不同（生产缺省不铺）
            Assert.Equal(band, mBand.Recipe!.ClampBandMm, 12);
            Assert.NotEqual(FlangeMesher.ProductionMeshRule, mBand.Recipe.Rule);
            Assert.Contains($"压接边界两侧各 {band:0.###} mm 铺细步 {hF:0.###} mm", note);
            Assert.True(ins.Length >= 2 * 3 - 2, $"{name}：窗口里只有 {ins.Length} 格");
        ...
        // 图纸路径的缺省入口：BuildFromField 不带开关（厚度场由解析板栅格化，锚点从场推）同样整面接触、不铺细带、面上定温
        {
            var lc = d.BuildCase(p, checkRamp: false);
            var g = Plate0(d, p, lc);
            ...
            // 2026-09-15 Opus 5（合并）：G1 的结构化配方为主（细带取生产解析值 = 0、规则 = 生产常量含面上定温），F 的记录文字与网格开关辅助
            Assert.Equal(FlangeMesher.ResolveClampBandMm(double.NaN, lc.MeshFineMm), mF.Recipe!.ClampBandMm, 12);
            Assert.Equal(FlangeMesher.ProductionMeshRule, mF.Recipe.Rule);
            Assert.True(mF.Recipe.ClampFaceDirichlet, "图纸路径缺省入口的网格配方不是面上定温");
            Assert.DoesNotContain("铺细步", mF.ClampAnchorNote);
            Assert.Contains("已落成节点", mF.ClampAnchorNote);
            Assert.True(mF.ClampFaceActive, "图纸路径缺省入口不是面上定温");
        }
    }
    ...
    public void c_整面接触格全部是电极且钉在夹持温度_定温热场闭合()
    {
        ...
        foreach (var (name, lc) in GradedCases(d, p))
        {
            ...
            var (sc, th, _) = SolvePlate0(d, lc, g, m);
    ...
    public void d_显式老口径与改动前一致()
    {
        ...
        // 记录出处 deliverable/R48_实验b_压接边界落节点_2026-09-14.txt 片0 h=2 R=50 行（改动前跑的，单元 1006、钉住格数 36）
        {
            ...
            // 2026-09-15 Opus 5（审查意见 minor）：结构化配方为主（老口径：整面接触未生效、细带 0、锚点落成节点），记录文字辅助
            Assert.False(m.Recipe!.ClampFullFace);
            Assert.Equal(0.0, m.Recipe.ClampBandMm);
            ...
            //   记录值出处：deliverable/R48A_基线树老口径记录_2026-09-14.txt —— 在基线树（改动前代码）上跑的探针
            ...
            //
            // ★★ 2026-09-15 Opus 5 正规化（常驻数值把关人第十四轮判：09-15 上午那版「看过自由端差 1.14e-4 W 之后把相对 1e-6 放到 1e-5」是**事后挪门槛**，必须改）。
            //   散热表上限从「设定 + 200 K、60 节点」改为「铂熔点、约 22 K 一个节点」（ShellThermal.LossTableHiC／LossTableNodes）之后，本门分两步：
            //   ① **逐位**：给 ShellThermal.Solve 传只供测试用的 lossTableHiC = 设定 + 200、lossTableNodes = 60（= 基线树的表），
            //      与基线树记录比，容差相对 1e-12 —— 表换回去就该逐位相同；不同就说明除散热表以外还有东西动了。
            //   ② **新表下的容差由表本身推出**（式子跑前定下，不看新旧结果之差）：
            //      δq = 在工作温度区间（新旧两次解的场温度范围取并）上逐 1 K 比较新旧两张表（裸面／圆盘保温面／舌保温面三张取最坏）的最大相对差 |q新 − q旧| ÷ q旧；
            //      放大系数：抽热 = 总散热 ÷ |抽热|（表面散热整体偏 δq，最多 δq·总散热 的热要改从管子来或不来）⇒ 界 = 3 · δq · 总散热（W）；
            //                温度（舌端均温）= q ÷ (dq/dT)（在该温度上，三张新表取最大）⇒ 界 = 3 · δq · q/(dq/dT)（K）；
            //                铜排等效长 = k(T)·A/G 随舌端温度走 ⇒ 界 = 等效长 · |dk/dT ÷ k| · 温度界（mm）；
            //      3 是数值把关人给的安全系数。**推导界 ≥ 实测差才过**，两者都打印。
            ...
            })
            {
                double tSet = d.SetpointC[0];
                // ① 旧表（基线树的表）⇒ 逐位
                var (_, thOld, p2) = SolvePlate0(d, lc, g, m, mode, lossTableHiC: tSet + 200, lossTableNodes: 60);
                double areaW = ring.Sum(i => thOld.T[i] * m.Area[i]) / ring.Sum(i => m.Area[i]);
                _out.WriteLine($"导航网格 老口径 {mode}（旧表 设定+200、60 节点）：舌端均温 {thOld.TTabEndMeanC:R}（面积加权会是 {areaW:R}）　抽热 {thOld.QFromTubeW:R}　铜排等效长 {thOld.BusEquivLenMm:R}");
                Assert.True(thOld.Converged, $"{mode}：热场没收敛（旧表）");
                Assert.Equal(tSet + 200, thOld.Recipe.LossTableHiC);
                Assert.Equal(60, thOld.Recipe.LossTableNodes);
                AssertBitwise($"{mode} 旧表 舌端均温", thOld.TTabEndMeanC, tTab);
                AssertBitwise($"{mode} 旧表 抽热", thOld.QFromTubeW, q);
                AssertBitwise($"{mode} 旧表 铜排等效长", thOld.BusEquivLenMm, busLen);
                if (mode != "定温")
                    Assert.True(Math.Abs(areaW - tTab) > 1e-6, $"{mode}：面积加权与按格数平均分不开（{areaW:R}），这条对照空转");

                // ② 新表（生产缺省）⇒ 与记录之差 ≤ 由表推出的界
                var (_, thNew, _) = SolvePlate0(d, lc, g, m, mode);
                Assert.True(thNew.Converged, $"{mode}：热场没收敛（新表）");
                Assert.Equal(ShellThermal.ProductionThermalRule.LossTableHiC, thNew.Recipe.LossTableHiC);
                var bound = TableBound(p2, g.TabInsulThickMm, tSet + 200, 60, thOld, thNew);
                double qOverSlope = bound.QOverSlopeAt(thOld.TTabEndMeanC);
                double bQ = 3 * bound.DeltaQ * thOld.QLossW;
                double bT = 3 * bound.DeltaQ * qOverSlope;
                double kT = Materials.PtThermalK(thOld.TTabEndMeanC);
                double dkdT = Materials.PtThermalK(thOld.TTabEndMeanC + 0.5) - Materials.PtThermalK(thOld.TTabEndMeanC - 0.5);   // 中心差分，步 1 K
                double bL = Math.Abs(thOld.BusEquivLenMm) * Math.Abs(dkdT / kT) * bT;
                double dT = thNew.TTabEndMeanC - tTab, dQ = thNew.QFromTubeW - q, dL = thNew.BusEquivLenMm - busLen;
                _out.WriteLine($"   新表（上限 {thNew.Recipe.LossTableHiC:0.#} °C、{thNew.Recipe.LossTableNodes} 节点）：工作区间 {bound.LoC:0.#}～{bound.HiC:0.#} °C 上 δq = {bound.DeltaQ:E3}（最坏在 {bound.WorstAtC:0} °C）；"
                             + $"总散热 {thOld.QLossW:0.###} W；q/(dq/dT) @ 舌端均温 = {qOverSlope:0.###} K");
                _out.WriteLine($"   推导界 vs 实测差：抽热 {bQ:E3} vs {dQ:E3} W　舌端均温 {bT:E3} vs {dT:E3} K　铜排等效长 {bL:E3} vs {dL:E3} mm");
                Assert.True(bound.DeltaQ > 0, "新旧两张表在工作区间上逐点相同 —— 本门的第 ② 步空转");
                Assert.True(Math.Abs(dQ) <= bQ, $"{mode}：抽热偏离基线记录 {dQ:E3} W，超过由散热表推出的界 {bQ:E3}");
                Assert.True(Math.Abs(dT) <= bT, $"{mode}：舌端均温偏离基线记录 {dT:E3} K，超过由散热表推出的界 {bT:E3}");
                Assert.True(Math.Abs(dL) <= bL, $"{mode}：铜排等效长偏离基线记录 {dL:E3} mm，超过由散热表推出的界 {bL:E3}");
            }
        }
        ...
        // h=1 分级：同误差预算的「分级」组（clampBandMm 0，那时还没有整面接触）与「细带」组（clampBandMm 3）。
        {
            ...
            double tSet = d.SetpointC[0];
            var (_, thOldT, p2) = SolvePlate0(d, lc, g, mOld, lossTableHiC: tSet + 200, lossTableNodes: 60);
            var (_, thOld, _) = SolvePlate0(d, lc, g, mOld);
            _out.WriteLine($"h=1 分级 老口径：单元 {mOld.CellCount}　抽热（旧表）{thOldT.QFromTubeW:R} W　（新表）{thOld.QFromTubeW:R} W");
            Assert.Empty(mOld.ClampCell);
            Assert.Equal(0.0, mOld.Recipe!.ClampBandMm);                                  // 2026-09-15 Opus 5：结构化配方为主，记录文字辅助
            ...
            var mDef = BuildDefault(g, lc);
            Assert.Equal(mOld.Nodes.Count, mDef.Nodes.Count);
            for (int k = 0; k < mDef.Nodes.Count; k++)
            {
                Assert.Equal(mOld.Nodes[k].X, mDef.Nodes[k].X);
                Assert.Equal(mOld.Nodes[k].Z, mDef.Nodes[k].Z);
            }
            Assert.Equal(mOld.CellCount, mDef.CellCount);
            Assert.Empty(mOld.ClampCell);
    ...
    /// <summary>
    /// 新旧两套表（调生产的 ShellThermal.SurfaceLossTablesFor，只换上限与节点数，不手抄热流配方）在工作温度区间上逐 1 K 比（区间右端点也比）。
    /// 工作区间 = 新旧两次解的场温度范围取并 —— 用的是**温度范围**，不是结果之差。2026-09-15 Opus 5。
    /// </summary>
    private static TableDiff TableBound(DesignInputs p2, double tabInsulMm, double oldHiC, int oldNodes, ShellThermalResult thOld, ShellThermalResult thNew)
    {
        var tOld = ShellThermal.SurfaceLossTablesFor(p2, tabInsulMm, oldHiC, oldNodes);
        var tNew = ShellThermal.SurfaceLossTablesFor(p2, tabInsulMm);
        double lo = Math.Min(thOld.TMinC, thNew.TMinC), hi = Math.Max(thOld.TMaxC, thNew.TMaxC);
        double worst = 0, at = lo;
        var pairs = new[] { (Old: tOld.Bare, New: tNew.Bare), (Old: tOld.Disc, New: tNew.Disc), (Old: tOld.Tab, New: tNew.Tab) };
        var ts = Enumerable.Range(0, (int)Math.Floor(hi - lo) + 1).Select(k => lo + k).Append(hi);
        foreach (double t in ts)
            foreach (var pr in pairs)
            {
                double qo = pr.Old.Eval(t), qn = pr.New.Eval(t);
                double rel = Math.Abs(qn - qo) / Math.Abs(qo);
                if (rel > worst) { worst = rel; at = t; }
            }
        double QOverSlope(double t) => pairs.Max(pr => Math.Abs(pr.New.Eval(t) / pr.New.Slope(t)));
        return new TableDiff(lo, hi, worst, at, QOverSlope);
    }
    ...
    public void e_压接段盖到管孔时退回只钉外圈_警告进输出且说人话()
    {
        ...
        Assert.Empty(m.ClampCell);
        // 2026-09-15 Opus 5（审查意见 minor：门里残留按记录文字判「盖到了管孔」）：主断言读网格上的布尔位与结构化配方；记录文字只作辅助核对
        ...
        Assert.All(r.Flanges, fo => Assert.True(fo.ClampCoversHole, fo.Name + " 应带压接盖孔位"));   // 2026-09-15 Opus 5：位带到了下游
    ...
    /// ⚠ 2026-09-15 Opus 5（数值把关人第十三轮）：本门降为**辅助**。主证据改为行为门 R48RecipeFingerprintTests.a ——
    ///   LineRunner.Run 算出来的每片网格／热解实际配方（ShellMesh.Recipe／ShellThermalResult.Recipe）== 生产配方常量。
    ///   字符串门看不见生成器里的退回分支（例如压接段盖到管孔时整面接触不生效）与没落成节点的锚点，行为门看得见。
    /// </summary>
    [Fact]
    public void f_LineRunner建网格不覆写配方开关()
    {
        ...
        // R48 F（2026-09-15 Opus 5）：配方 ⑤ 压接面上定温同样不许在生产链上覆写（生成器参数与网格上的开关两种写法都不许出现）
        ...
        var setter = typeof(ShellMesh).GetProperty(nameof(ShellMesh.ClampFaceDirichlet))!.SetMethod!;
        Assert.Contains(setter.ReturnParameter.GetRequiredCustomModifiers(), t => t.FullName == "System.Runtime.CompilerServices.IsExternalInit");
        Assert.True(new ShellMesh().ClampFaceDirichlet);
    }
    ...
    ///   R48 F（2026-09-15 Opus 5）：clampFullFace 不是字面 false 的调用还必须写出 clampFaceDirichlet:（配方 ⑤，改动前的整面接触证据钉 false）。
    ///   R48 F 复审修（2026-09-15 Opus 5，审查意见 minor）：clampBandMm 写成 double.NaN 不算「显式」—— NaN 解析出来就是生产缺省
    ///     （FlangeMesher.ResolveClampBandMm），缺省 3.0 → 0.0 时写 NaN 的探针照样悄悄换口径 ⇒ 判红，必须写数字。
    ...
    public void g_证据探针建网格必须显式写明两个开关()
    {
        ...
        foreach (var (file, src) in gated)
            foreach (Match mm in Regex.Matches(src, @"FlangeMesher\.Build(FromField)?\("))
            {
                ...
                // R48 F（2026-09-15 Opus 5）：配方 ⑤ 压接面上定温只在整面接触时起作用 ⇒ 没把整面接触钉成 false 的调用还必须写明 clampFaceDirichlet
                //   （同一个理由：默认一改，证据探针悄悄换口径、重跑覆盖同名证据）
                else if (!Regex.IsMatch(args, @"\bclampFullFace\s*:\s*false\b") && !Regex.IsMatch(args, @"\bclampFaceDirichlet\s*:"))
                    bad.Add($"{file} 第 {src.Take(mm.Index).Count(ch => ch == '\n') + 1} 行（整面接触没写 clampFaceDirichlet）");
                // R48 F 复审修（2026-09-15 Opus 5）：clampBandMm: double.NaN = 生产缺省，不是钉住 ⇒ 判红，必须写数字（见类注释 g）
                if (Regex.IsMatch(args, @"\bclampBandMm\s*:\s*double\.NaN\b"))
                    bad.Add($"{file} 第 {src.Take(mm.Index).Count(ch => ch == '\n') + 1} 行（clampBandMm 写的是 double.NaN = 生产缺省，要写数字）");
            }
        _out.WriteLine($"生产代码引的证据文件 {cited.Length} 个；受管探针 {gated.Count} 个：{string.Join("、", gated.Keys.OrderBy(k => k))}；建网格调用 {calls} 处");
        Assert.True(bad.Count == 0, "这些证据探针建网格没显式写明 clampFullFace／clampBandMm／clampFaceDirichlet，或 clampBandMm 写成 double.NaN（生产默认一改就悄悄换口径）：" + string.Join("；", bad));
    ...
    ///   · LineRunner.Run 出来的每片 Flanges[j].Mesh 都带整面接触（ClampCell 非空）与面上定温（ClampFaceActive），不铺压接细带
    ///     —— R48 F（2026-09-15 Opus 5）改：原为「带压接细带」，生产缺省改为不铺（依据见 FlangeMesher.ClampBandPerHFine）；
    ///     2026-09-15 Opus 5 起读整线带出的结构化网格配方（细带宽度 = 生产值、规则 = 生产常量；原先查 ClampAnchorNote 含／不含「铺细步」）；
    ///     2026-09-15 Opus 5（合并）：两路合一 —— 结构化配方为主（生产常量已含配方 ⑤），网格开关与记录文字辅助；
    ...
    /// </summary>
    [Fact]
    public void h_整线网格带整面接触与面上定温_集总参考量排除了压接格()
    {
        var p = new DesignInputs();
        var d = DesignSpec.W08.Clone();
        d.SetpointC = new[] { 1150.0 }; d.SegLengthMm = new[] { 300.0 };
        ...
        foreach (var f in r.Flanges)
        {
            ...
            // 2026-09-15 Opus 5（审查意见 minor：原先查 ClampAnchorNote 里有没有「铺细步」，是字符串协议）：改读整线带出的结构化网格配方
            Assert.NotNull(f.MeshRecipe);
            Assert.Same(m.Recipe, f.MeshRecipe);
            ...
            // 2026-09-15 Opus 5（合并）：F 的面上定温断言保留（网格开关辅助、配方项为主），记录文字辅助
            Assert.True(f.MeshRecipe.ClampFaceDirichlet, $"{f.Name}：整线出来的网格配方不是面上定温");
            Assert.True(m.ClampFaceActive, $"{f.Name}：整线出来的网格不是面上定温");
            Assert.DoesNotContain("铺细步", m.ClampAnchorNote);
            Assert.Contains("已落成节点", m.ClampAnchorNote);
        }
        ...
        // ★ R48 G2 复审（2026-09-15 Opus 5）有意改断言：原「判据表升温那一行 = 排除那一份的峰值，且 ≠ 不排除那一份」→
        //   现「判据表那一行判不了、不给数；排除那一份照样解出来（留给前后对照探针）；排除与否的升温输入不同（非空转）」。
        //   依据：审查意见 blocker —— 物理把关人复核前那一行不许给数（两节点模型的法兰温度是整片平均，看不到孔边比管热），见 LineRunner.RampPendingNote。
        //   本算例（W08 一段原样）两片场里热从法兰流进管、节点均温低于管根 ⇒ 管孔一项标定不了、升温两节点不解（R48G2RampClampChannelGateTests 四乙），所以不要求 yes.Ramp 非空，要求「解了，或说了为什么没解」。
        Assert.True(yes.Ramp is not null || yes.RampError.Length > 0); Assert.NotNull(no.RampIn);
        Assert.True(ramp.Undetermined && double.IsNaN(ramp.Actual), "升温那一行复核前应判不了、不给数");
        Assert.True(yes.RampIn!.FlangeMassG < no.RampIn!.FlangeMassG && yes.RampIn.FlangeAreaBareMm2 < no.RampIn.FlangeAreaBareMm2,
            "排除压接格后升温两节点的质量与舌片区面积应当变小");
        ...
        // ★ R48 G2（2026-09-15 Opus 5）有意改断言：原 Assert.Contains("偏保守") → 现 DoesNotContain("偏保守")、Contains("待复核")。
        //   依据：物理把关人第十一轮 —— 模型已补铜排通道（稳态场标定）与舌片保温，而「偏保守」这个方向判断本身不成立（同一模型里舌片按裸铂多算散热，方向相反），
        //   工单要求任何情况下附注不许再写「偏高偏保守」，复核前写「模型已补铜排通道与舌片保温，待复核」。附注仍须说到铜排（通道怎么标定）。
