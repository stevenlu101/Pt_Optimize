...
///   (f) LineRunner 建网格不覆写这两个开关（生产链读的就是默认配方）。【2026-09-15 Opus 5：降为辅助，主证据是 R48RecipeFingerprintTests 的行为门】
...
public class R48ClampRecipeTests
{
    ...
    public void b_不带开关的网格在压接边界两侧自相似细带内是细步()
    {
        ...
        foreach (var (name, lc) in GradedCases(d, p))
        {
            ...
            // 2026-09-15 Opus 5（审查意见 minor：门里残留按文字判配方）：主断言读结构化配方；网格记录那句文字只作「给人看的记录写对了」的辅助核对
            Assert.Equal(band, mDef.Recipe!.ClampBandMm, 12);
        ...
        // 图纸路径的缺省入口：BuildFromField 不带两个开关（厚度场由解析板栅格化，锚点从场推）同样整面接触 + 铺细带
        {
            ...
            Assert.Equal(band, mF.Recipe!.ClampBandMm, 12);                               // 2026-09-15 Opus 5：结构化配方为主
    ...
    ///   · LineRunner.Run 出来的每片 Flanges[j].Mesh 都带整面接触（ClampCell 非空）与压接细带（2026-09-15 Opus 5 起读整线带出的结构化网格配方：细带宽度 = 生产值、规则 = 生产常量；原先查 ClampAnchorNote 含「铺细步」）；
