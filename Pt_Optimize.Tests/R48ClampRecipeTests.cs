...
public class R48ClampRecipeTests
{
    ...
    public void b_缺省入口不铺压接细带_显式3h细带仍是细步()
    {
        ...
        foreach (var (name, lc) in GradedCases(d, p))
        {
            ...
            _out.WriteLine($"{name}：缺省入口 窗口里最大格 {touchDef:0.###} mm　记录：{noteDef}");
    ...
    public void d_显式老口径与改动前一致()
    {
        ...
        // 记录出处 deliverable/R48_实验b_压接边界落节点_2026-09-14.txt 片0 h=2 R=50 行（改动前跑的，单元 1006、钉住格数 36）
        {
            ...
            })
            {
                ...
                // ⚠ 2026-09-15 Opus 5：表面散热表上限从「设定 + 200 K」改为铂熔点、节点间距保持约 22 K（ShellThermal.LossTableHiC／LossTableNodes，
                //   数值把关人第十三轮查出原上限超界静默钳住）⇒ 样条节点变了，上面三组逐位记录（基线树跑的）不再逐位相同。
                //   实测：定温模式抽热 −9.645643968612903 → −9.645643852053805（相对差 1.2e-8）。本门要守的是「显式老口径网格行为不变」，
                //   不是散热表节点 —— 改为相对比对并把新旧差打印出来。容差先定 1e-6，实跑自由端模式抽热差 −1.14e-4 W（相对 2.1e-6：
                //   自由端舌片更热、离旧上限更近，样条节点变化影响大一些）⇒ 看过数据后放到相对 1e-5，照实记在这里；再大就说明除散热表以外还有东西动了。
                _out.WriteLine($"   与基线树逐位记录之差：舌端均温 {th.TTabEndMeanC - tTab:E2}　抽热 {th.QFromTubeW - q:E2}　铜排等效长 {th.BusEquivLenMm - busLen:E2}");
                Assert.True(Math.Abs(th.TTabEndMeanC - tTab) <= 1e-5 * Math.Max(1.0, Math.Abs(tTab)), $"{mode}：舌端均温偏离基线记录 {th.TTabEndMeanC - tTab:E2}");
                Assert.True(Math.Abs(th.QFromTubeW - q) <= 1e-5 * Math.Max(1.0, Math.Abs(q)), $"{mode}：抽热偏离基线记录 {th.QFromTubeW - q:E2}");
                Assert.True(Math.Abs(th.BusEquivLenMm - busLen) <= 1e-5 * Math.Max(1.0, Math.Abs(busLen)), $"{mode}：铜排等效长偏离基线记录 {th.BusEquivLenMm - busLen:E2}");
                if (mode != "定温")
        ...
        // h=1 分级：同误差预算的「分级」组（clampBandMm 0，那时还没有整面接触）与「细带」组（clampBandMm 3）。
        {
            ...
            // 2026-09-15 Opus 5：同上（散热表上限改为铂熔点），逐位改相对 1e-5
            _out.WriteLine($"   h=1 分级老口径 与基线树逐位记录之差 {thOld.QFromTubeW - (-9.225247067674745):E2}");
            Assert.True(Math.Abs(thOld.QFromTubeW - (-9.225247067674745)) <= 1e-5 * 9.225247067674745, $"h=1 分级老口径抽热偏离基线记录 {thOld.QFromTubeW + 9.225247067674745:E2}");
    ...
    public void f_LineRunner建网格不覆写配方开关()
    {
        ...
        Assert.Contains("public bool ClampFaceDirichlet = true;", mesh);
    }
    ...
    ///   · LineRunner.Run 出来的每片 Flanges[j].Mesh 都带整面接触（ClampCell 非空）与面上定温（ClampFaceActive），不铺压接细带（ClampAnchorNote 不含「铺细步」）
