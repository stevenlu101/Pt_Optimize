...
public class R48ClampFaceGateTests
{
    ...
    public void b_导热一维条带_面上定温精确_形心口径差半格()
    {
        ...
        {
            var m = Strip(face);
            var th = ShellThermal.Solve(m, new double[m.CellCount], p, tCold, double.PositiveInfinity);
    ...
    private static (ShellThermalResult th, double tHash) Thermal(LineCase lc, ShellMesh m, double[] j, string mode)
    {
        ...
        var th = LineRunner.SolvePlateThermal(m, j, B2TRoot0, ts);
        double tHash = 0;
    ...
    public void c_开关关逐位等于改动前代码_开关开抽热不同()
    {
        ...
        foreach (string grid in new[] { "导航网格", "均匀h2" })
        {
            ...
            {
                var r = rec[(grid, mode)];
                var (th, tH) = Thermal(lc, mOff, scOff.JMagAPerMm2, mode);
                ...
                var (thOn, _) = Thermal(lc, mOn, scOn.JMagAPerMm2, mode);
