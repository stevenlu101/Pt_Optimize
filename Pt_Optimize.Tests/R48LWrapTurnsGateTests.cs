...
//    ③ 公式不手抄 10（层厚改了跟着改）；
//    ④ 越界 ⇒ 判**不可行**并点名到底是哪一片／哪一段，不是判不了、更不是过；
//    ⑤ 算不出来 ⇒ 判不了（判不了不算过）；
//    ⑥ 舌板与管身（离开端部）**不受本限** —— 自证门：舌保温 18.7 mm 照样过；
...
public class R48LWrapTurnsGateTests
{
    ...
    /// <summary>管端接合区 = 管保温 + 端部额外保温；它超了同样判不可行，并点名到管端段。</summary>
    [Fact]
    public void 门_管端渐变保温段的总厚也受限()
    {
        double cap = WrapLimits.JointZoneMaxMm;
        var d = DesignSpec.W08.Clone();
        ...
        Assert.Equal(5.0, d.TubeInsulMm, 12);                    // 自证：内置设计的管保温就是 5
        var k = WrapLimits.Judge(Case(5.0, null, p));
        Assert.False(k.Ok, "管端接合区 11 mm 应当判不可行");
        Assert.Equal(11.0, k.Actual, 9);
        Assert.Contains("管端段", k.Where);

        // 端部段长度为 0 ⇒ 没有那一段，只剩管保温 5 mm ⇒ 过
        var p0 = new DesignInputs { EndInsulExtraMm = 6.0, EndInsulLengthMm = 0.0 };
        Assert.True(WrapLimits.Judge(Case(5.0, null, p0)).Ok);
        Assert.Equal(5.0, WrapLimits.TubeJointTotalMm(Case(5.0, null, p0).Base), 9);
        Assert.True(cap > 5.0);                                   // 自证：上面那个「过」不是因为上限太小
    }
