...
public class R48InsulBlendTests
{
    ...
    private static (ShellMesh m, double[] jm, FlangePlate g) Plate(DesignSpec d, DesignInputs p, LineCase lc, int j, double h, double iA)
    {
        ...
        var m = FlangeMesher.BuildFromField(tf, holeR, 0, h, h, 1e6, lc.Base.BusbarClampLengthMm, h, 0, g.TwoTabs, xa, za);
    ...
    public void 四五_分界平移残差与九点平均()
    {
        ...
        string file = Path.Combine(Root(), "deliverable", "R48_保温分界混合_门_2026-09-14.txt");
        void Say(string s)
        {
    ...
    {
        var sb = new StringBuilder();
        string file = Path.Combine(Root(), "deliverable", "R48_保温分界混合_门六_2026-09-14.txt");
        void Say(string s)
        {
            _out.WriteLine(s); sb.AppendLine(s);
            try { File.WriteAllText(file, sb.ToString(), new UTF8Encoding(false)); } catch { }
    ...
    {
        var sb = new StringBuilder();
        string file = Path.Combine(Root(), "deliverable", "R48_保温分界混合_门七_2026-09-14.txt");
        void Say(string s)
        {
            _out.WriteLine(s); sb.AppendLine(s);
            try { File.WriteAllText(file, sb.ToString(), new UTF8Encoding(false)); } catch { }
    ...
    {
        var sb = new StringBuilder();
        string file = Path.Combine(Root(), "deliverable", "R48_保温分界混合_门八_2026-09-14.txt");
        void Say(string s)
        {
            _out.WriteLine(s); sb.AppendLine(s);
            try { File.WriteAllText(file, sb.ToString(), new UTF8Encoding(false)); } catch { }
