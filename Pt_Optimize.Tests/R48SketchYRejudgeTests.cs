...
public class R48SketchYRejudgeTests
{
    ...
    public void 拍脑袋Y形样机_决103复判_判决网格三关_只印()
    {
        ...
        W("工艺参数 new DesignInputs()（决 103 口径，FlangeStabGeomClampFallback 开，分叉点钉舌长中点 开）；对照口径 new DesignInputs { CriteriaRuleSet = 决103前 }；圆盘保温：记录未写，取底稿 W08 的 20 mm（决 104 下不过，另按 10 mm 判一次 ②）");
        ...
        // ★ 决 104（2026-09-25）：记录（spec.json）没写圆盘保温，底稿 W08 的 20 mm 在决 103 口径下被「接合区保温缠得出来」判不过；
        //   上面三关按记录原样判（与 09-12 旧数可比），这里再按上限 10 mm 判一次 ②，看这形状在能造的保温下过不过（只印）。
        var d10 = d.Clone(); d10.FlangeInsulated = true; d10.FlangeInsulMm = Math.Min(d10.FlangeInsulMm, p.DiscInsulCapMm); d10.DiscInsulMm = Array.Empty<double>();
        var lc10 = d10.BuildCase(p); Solver.ApplyCaseMesh(lc10, meshFinal);
        var sw10 = Stopwatch.StartNew();
        var g10 = LineRunner.Run(lc10, progress);
        W($"── ② 带玻璃稳态（决 103，圆盘保温改 {d10.FlangeInsulMm:0.#} mm = 决 104 上限；同一设计其余不动、同一判决网格）　耗时 {sw10.Elapsed.TotalSeconds:0} s　解出 {g10.Ok}　收敛 {g10.Converged}　AllOk {g10.AllOk}　铂重 {g10.TotalMassG:0.0} g");
        if (!g10.Ok) W("　解出失败：" + g10.Message);
        foreach (var c in g10.Checks.Where(c => c.Kind is CheckKind.HardSafety or CheckKind.Target)) W(Row(c));
        if (g10.MissingChecks.Length > 0) W("缺判：" + string.Join("、", g10.MissingChecks));
        if (g10.FieldUndeterminedReasons.Length > 0) W("场判不了：" + string.Join("；", g10.FieldUndeterminedReasons));
        W();
