...
internal static class R48LCoupleTolKit
{
    ...
    internal enum TolMode
    {
        ...
        LegacyConst,
        /// <summary>主环 = 新上限当常数用（不按裕度）；基线按生产口径（它服务的是参考量）。R48 M 起与 Production 同一口径（留着是为了旧文件的标签能对上）。</summary>
        NewConst,
        /// <summary>主环 = 判据相关；基线**也一起**收到上限（上一跑那一行的口径）。</summary>
        FromMarginTightBase,
        /// <summary>**生产默认**：R48 M（2026-09-18）起 = 主环绝对目标（不按裕度）；基线按它自己服务的那条判据（参考量，限值 10 K ⇒ 1 K）。</summary>
        Production,
        /// <summary>★ R48 M（2026-09-18，Fable 5.1）：**注射** —— 把「按判据裕度收紧」（§0.-10 的口径：上限 0.1／下限 0.02／比例 0.1）放回去；基线 1 K。</summary>
        FromMargin,
    }
    ...
    internal static void ApplyTol(LineCase lc, TolMode m)
    {
        switch (m)
        {
            ...
            case TolMode.FromMarginTightBase:
                lc.CoupleTolFromMargin = true; lc.CoupleTolK = 0.1; lc.BaselineTolK = lc.CoupleTolK; break;   // R48 M：这条历史口径要显式打开开关
            case TolMode.FromMargin:
                lc.CoupleTolFromMargin = true; lc.CoupleTolK = 0.1; break;   // §0.-10 的生产口径原样：上限 0.1、下限／比例走 LineCase 默认（0.02／0.1）
    ...
    internal static string TagOf(TolMode m) => m switch
    {
        ...
        TolMode.FromMargin => "⑤ 注射：主环按判据裕度收紧（§0.-10 口径）+ 基线 1 K",
        _ => "④ **生产默认**：主环绝对目标（不按裕度）+ 基线按它自己的判据（1 K）",
    };
