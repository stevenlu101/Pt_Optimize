...
| **③ 法兰增量温降** | **10 K** | **总纲 C2**「稳态法兰与管温差 < 10 °C」+ 现场条件表 | ✅ 有据 |
...
现场铂热偶 >1000 °C 误差 **±10 K**，而 ③ 的限值正是 **10 K**
⇒ **实际温降是 9 K 还是 11 K，现场分不出来**。
限值本身合理（是总纲定的工艺要求），但叠加**数值不确定度 ±5 K（限值的一半）**之后，
③ 要有意义，**收敛必须先修好**（§1.85）——
这就是为什么 §1.85 从「重要」升级为「唯一挡路的一条」。
