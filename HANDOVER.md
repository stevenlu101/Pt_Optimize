...
模型：法兰约 150 W/片、根部冷点 220 K。实测：从玻璃总共只抽走约 450 W。对不上。

| 候选 | 说明 |
|---|---|
| **舌片辐射** | 200 mm 舌片末端自由辐射（`BusbarClampTempC = -1`），末端算出 1240 °C，是最大单一热路。真实装置若舌片也包保温，抽热会小得多 |
| **热电偶位置** | 若 1150/1080/1050 是外壁或保温内读数而非内壁金属温度，比较基准整个偏掉（见待补数据 ④） |
| **保温实际做法** | 与模型默认（法兰双面 10 mm 纤维、`InsulBoundaryXMm=-200` 全包）是否一致 |

**三条都指向输入数据而非模型缺陷 —— 先问清现场，不要继续猜着改代码。**
