...
| R23 | **舌片按温场挖孔 ⇒ J 超设计值 ⇒ 舌片重新搜形状 ⇒ 可能得「倾斜 90° 的 Y 字形」** —— 用户 09-09 下午原话（答我「R13 舌孔形状族到不了」）：「这个状态的解法是以法兰的温场决定，法兰与舌片的接触位置有可能因导热而使接触测温度偏低，此时舌片就需以挖孔的形式来解决(孔大小与位置温场决定)，但挖孔又会造成J超过设计值，所以舌片必须重新搜形状最终有可能的形状是类似倾斜90度的 Y 字形」；我说叉口槽不在形状族里，用户：「这个其实可以用直椭圆+弯椭圆的形状组合」⇒ 不加新几何：舌片的直椭圆（顺流长椭圆孔）落在舌根 + 圆盘的弯椭圆槽落在舌侧，两个切口夹出两条臂；用户又补：「还有椭圆三角形也可以达到」⇒ 圆角三角孔（`TabHoleSides`=3）底边朝盘、顶角朝铜排，一个孔就是叉口，两臂在舌根两角 | 09-09 | ✗ **未做**（2026-09-09 下午登记）。现状与病灶：① 舌片厚 = I/(J·最窄有效宽) 在**已提交态**才重算，孔径旋钮的 J 上界按当前舌片厚算 ⇒ 恒为 0，旋钮死（R13 ◐ 的原因）；② 舌孔孔心的场规则（移除优先级最高处 = 舌根 x=−14.5）被 09-05 逐点实测推翻（−50 最好、−20 最坏，峰值 J +23.9）—— 那次实测是**舌片厚不变**时做的，J 超正是用户说的那一步，不是规则错，是少了「重新定厚／搜形状」；③ 弯椭圆槽的槽心由场定后冻结，规则不变。做法：(a) 孔径探针里按最窄有效宽**重定舌片厚**（闭式已有 `SizeTongues`），铂重计入候选、④ 由构造满足，上界只剩桥宽；(b) 舌孔孔心与槽心照场的移除优先级定，允许落在舌根／舌侧；(c) 截面枚举：舌盘交界切口同时扣两种切口，叉臂各自成截面；(d) 出图不用改（两种切口各自会画）；(e) 只在 ③／②′ 绑定且舌保温到界后进候选，探针实测 ③ 变不变好。物理上要说清：截面按 J 定死时挖孔再补厚**不减导热**，减的只有被挖掉那块本来就不导电的部分（舌根中央），所以挖哪里由场量、由探针验，不许假设 | — |
