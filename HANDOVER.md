...
| R18 | **法兰 J 判不了要判得了**（C1：孔那一带网格从未细化，法兰 J_max 是下界 ⇒ 一律「判不了不算过」⇒ 挡住每一档的全绿交付）。用户 09-08：烧毁三缺口之一，排进计划 | 09-08 | ✗ 未做 | 验收 = 孔边网格加密到网格无关（`MeshVerify` 口径）后法兰 J 那一行能出「过／不过」，0.8 档判据表不再有「判不了」；门：`HoleMeshResolutionTests`（现有，需扩） |
...
dotnet build
dotnet test --filter "速度!=慢"   # 快门 677 条，实测约 20 s（提交钩子跑的就是它；2026-09-08 数）
dotnet test --filter "速度=慢"    # 慢门 15 条：真跑场解/出图，合计数小时，发版前跑（2026-09-08 数）
...
`dotnet test` 应为 **692/692**（2026-09-08 由 HandoverGateCountTests 反射数出；+3 = BranchMarksAreReachedTests「断言走到了」那三条；再 +5 = 下角第三来源「不熔化」：BranchMarksAreReachedTests 2 慢 + MeltIsNotASolutionTests 1 慢 + MeltFloorTests 2 快）。
