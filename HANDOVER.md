...
- 规则 A／B 的去留（依据随 F6a 失效；「关掉规则 A」那组数没有证据档）；门 e 分辨力；门 3 截断切到峰格；守恒门与 J 峰门的改回余量薄；5-1、5-2 两处红；压接面的切向偏移没量；`DrawingPathParityTests` 栅格步 0.5 离群的根因（孔弧缺口已排除）。
