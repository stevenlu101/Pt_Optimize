...
dotnet build
dotnet test          # 应为 82/82 通过（这个数由 HandoverGateCountTests 自己盯着）
...
> 历史：18（初版）→ 19 → 15（2026-08-12 拔掉一维法兰模型）→ 30 → 31（本条自身）→ 42 → 52 → 80 → 81 → 82。
>
...
| 缺口 | 现状 | 为什么没做完 |
|---|---|---|
| **替身在 .3dm 上被放行那条路无端到端实测** | 接线那半段已端到端验（`SolveByLevelWiringTests`：跑一轮外层、收满四片即中止，6 ms）；**「图纸上替身真的被放行并解完」仍未跑过** | 仓库里没有「无槽 + 多级」的 .3dm。`WriteFinal3dm` 写出来是**多图层**的定案几何，而分析器要单图层；造一个要改 Rhino 子进程 |
| **B′ 多段共用片取两侧较大值** | 单段契约已有门（`SizeFlanges_OneSegment_…`，14 s）；共用片那条逻辑要 ≥2 段、约 1 分钟，未覆盖 | 时间预算：单测套件目前 15 s，加一分钟会让提交门更难用 |
| **两条热稳定判据仍是参考量** | 已进判据表、每轮 selfcheck 打印四档（整片 8.2–44×、局部 1.9–2.9×） | **有意为之**：限值 1.0 是精确物理，但跨几何的量级还没攒够。拿没量过分布的判据卡交付是把风险换一侧 |
...
| **提交钩子已需 11–12 分钟** | 构建 + UiWiring（含两次真解）+ selfcheck 7.9 分 | selfcheck 是核心回归门，不该为省时间削弱。若要缩，先缩 UiWiring 里那两次真解，别缩 selfcheck |
