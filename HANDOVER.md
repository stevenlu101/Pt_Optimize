...
| **⑧ 共用法兰的实测电流** | 判定 §4.2h 的 √3 | **⚠ 2026-08-11 重新打开**（2026-08-10 曾按「已推导」划掉）。收敛解下 √3 使共用片算到 3619 °C，超铂熔点，而实物在运行；且换回 1.5 仍 Φ>1。**推导做出了一个被现场证伪的预言 ⇒ 必须实测**。钳表夹共用片引线一次即可 |
...
**⓪ 查清「模型判现役共用法兰熔化」这条矛盾 —— 压倒一切（§4.2q）**

在此之前不要做任何优化扫描：模型连现状都算不对，优化出来的省铂率没有意义。
三条嫌疑与证伪方式见 §4.2q 的表；最省事的是待补数据 ⑧ 或 ⑨，各只需现场量一次。

代码侧可以先做的两件（不依赖现场）：
- 把共用片叠加系数做成**可扫的输入**（现在 `LineSolver.JointCurrentA` 写死 √3，
  `UseWorkbookSharedFactor` 只是个静态开关，CLI 够不着），扫出「使共用片不熔」的系数上界 —— 
  若该上界远低于 1.5，说明问题不在系数而在散热路径。
