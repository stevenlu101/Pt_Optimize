...
>
> **端片曾被判为「结构性死路」的结论已被推翻**，整线拿到第一个可交付量级的解：
> **2204 g vs 现状 7141 g ⇒ 省 69.1 %**（`--linefinal`，盘舌等厚方案，§4.3e 末）。
>
> 判据现状：**升温 ✓**（0.11 h／限 3）、**管根温差 C2 ✓**（+9.9 / +1.1 / +6.6 K）、
> 能量残差全 0.0、耦合收敛、玻璃温降仍对得上实测（20.9 vs 20 K）。
> **唯一未过：判据②「法兰最高温 ≤ 管温」仍超 1.7–2.9 K**（紧贴管孔的局部，
> 管孔净流入 +1 W 仍是安全方向）—— **未闭合，方案不能算全过。**
>
> 三条支撑它的新结果：
...
> ⚠ **单片筛偏乐观约 0.8 K**：同一构型下单片筛给 +0.00、整线耦合给 +0.8。
> 输入差别（电流场取控温点 vs 管根温度的 ρe、fineRadius 45 vs 50）都很小，
> 但峰值是尖锐局部特征，对输入敏感。**单片筛只可用作排序，不可用作过/不过的判据。**
...
> ⚠ **这一步与「同板等厚」冲突**（§4.3f 结论 3）。两级渐变环需要局部增厚，
> 不是一张等厚板切出来的。可能的做法：厚板机加工出台阶、或另焊一个加强环（多一条焊缝）。
> **这是一个必须向工艺方确认的问题，不能由计算单方面决定** —— 见 §6 待补。
> 若工艺上只能等厚，则 ② 停在 **+2.85 K**（无加厚）或 **+1.9 K**（单级环），
> 两者都过不了硬安全线。
