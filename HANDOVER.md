...
**控制方式只改变过热的幅度与时刻，不改变门槛。** §4.2q 的稳态门槛同样是 1.2 ——
三条独立的路给出同一个数。
...
**升温工况**（§4.2s）：控制方式已确认为**温控功率 + 20 °C/h**，冷启电流仅 71 A，
急性烧断风险不存在（法兰峰值 1320 °C，离熔点 450 K）。剩下两条：
① 把壳解做成**瞬态**，判孔周/舌片的**局部**峰值会不会到熔点（集总只给面均温）；
② 现场实际烧断（若有过）发生在什么温度段？模型说温差在管温约 880 °C 处最深。
