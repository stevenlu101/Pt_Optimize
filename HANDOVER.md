...
## 0.-12(S) ★★★★★ R48 S：**盘径与舌半宽这根杠杆，实测走到哪里为止** —— 2026-09-18，Opus 5

> 承 §0.-11 ④ 第 4 条（「W08 没有可交付设计……要量得先把盘径放开，那是『搜形状』那条路」）
> 与求解器自己两路的自报（`Solver.cs` 的三处停机出口、`FlangeAutoSizer.cs` 的「下一根是增宽过流带」、`NextAction.cs` 的指路句）：
> **下一根杠杆是盘径与舌半宽**。本节把这根杠杆**量到底**，结论先写在这里：
>
> ⇒ **盘半径 30 → 32 把「⑥ 圆盘盖不住管孔＋焊脚」这条几何硬线解开了**（⑥ 的闭式反解要 31.8），
> 　但解开之后露出来的是**「最热铂高出热偶读数」超 15.8～16.9 K**，而**法兰侧九根旋钮全部顶到上界只把它拉 0.25 K**。
> ⇒ **本节跑过的形状里一个可行形状都没有**。**这不是「搜形状没搜到」，是量到的两件事**：
> 　① 形状能给的那点余量，量级上补不上这个缺口；② 缺口所在的那条判据，法兰侧的厚度旋钮对它近乎无效。
>
> ⚠ **本节一行生产代码都没改**（`Pt_Optimize/` 下非测试文件一个字未动），只新增探针与 deliverable 输出。

### ① 先查出处：「盘径上界 30」**根本不是一条上界**

工单问「盘径上界 30 写在哪，是用户给的、图纸来的、还是无出处」。查全仓的结果：

| 处 | 是什么 | 值 | 出处 |
|---|---|---|---|
| `Core/DesignSpec.cs` `DesignSpec.DiscRadiusMm` | 字段**默认值** | `DiscRadiusMm = 30.0` | 无注释、无出处 |
| `DesignSpec.W08`／`DesignSpec.W06` 两个内置档 | 两个内置档的**形状输入** | 30.0 | 见下 |
| `UI/LineDesignPage.cs` `_discD` | 界面输入框（**直径**） | `Num(60, 30, 300, 2, 0)` ⇒ **半径 15–150 mm** | 控件量程 |
| `LineDesignPage.SearchDiscs` | 搜形状**第 1 轮网格** | `SearchDiscs = {25, 30, 35}` | 网格，不是界；邻域外推可以走出去 |
| `Core/GeometryScreen.MinDiscRadiusMm` | 判据 ⑥ 的**闭式下界** | 管孔半径 + 焊脚 | 第一性原理，全仓唯一一份 |
| `Core/Solver.cs` `SolverOptions` | —— | **没有盘径这根旋钮，也没有盘径的界** | 求解器手上只有法兰侧九根 |

⇒ **30 没有任何地方把它写成上界。** 它是**两个内置档的形状输入值**，而那个值的来历查得到：
HANDOVER §「`--shape --from3dm Pt_Heater1.3dm 法兰 --discs 60,45,30 --halfws 40,30`」那一跑
（2026-08，本文档「两条输入路线在形状上会合了」那一段）——**盘R30／舌 140×60 是 APP 自己搜出来的最轻全过形状**，
并与设计记录逐位相同。**不是用户给的，不是图纸来的，也不是谁拍的上界。**
⇒ 本节因此**不需要**在 `D:\WinForm\r48_S` 里放宽任何界，那个目录**一个字都没建、没改**。

**舌半宽的界同查**：`DesignSpec.TabHalfWidthMm` 默认 15.0、两个内置档 30.0（= 盘半径，正好贴在几何天花板上）；
界面框 `_tabW` 5–150 mm；真正的硬约束是**几何**的「舌半宽 ≤ 盘半径」
（`PlateCurrent2D` 超了静默夹住并置 `HalfWidthClamped`、`GeometryScreen.MinDiscRadiusForTabMm`、
参考量「· 舌宽被盘径夹住」）；搜形状把比例夹在 `[0.25, 1.0]`（`ShapeSearchPlan.Neighbours`，
注释写「再窄就没有过流截面可言」，是代码里的判断，**没有用户或图纸出处**）。
⇒ 用户 2026-09-17「我没有限制舌板的形状」**与代码一致**：舌半宽没有任何人为的界，只有盘半径这个几何天花板。

### ② 方向实测（跑在解上，一次整线解一个点）

输出：`deliverable/R48_形状探针_方向实测_W08_本次开跑于2026-09-18_090213.txt`（23.6 分钟，12 点，同一进程）
基准 = `R48LW08NavDesign`（照 09-17 093013 输出表复原的 4245 g 那一份）。⚠ 它的舌保温是 **0.1 格**那一跑的值，
**不落 0.5 格** ⇒ 只作方向探针的基准点，**不作交付判定**。判读门槛跑前写死。

**先验前提**（网格这一维自己值多少）：形状完全不动、只把细区半径 59.0 → 64.6 → 67.2，
三条判据各摆 **管根 0.090 K／最热铂 0.119 K／净流入 0.043 W** ⇒ 与下面几何组 10～170 K 的位移不是一个量级，
**几何组的位移是几何自己的**（§0.-10 记过同一个病那一次净流入差 9.8 W 且符号相反 —— 这一次不是那个形态，实测过了才敢这么说）。

| 形状（只动一根） | 管根低于热偶读数 | 最热铂高出热偶读数 | 管孔净流入 W | 法兰截面 J | 铂重 g | 规矩③ 判词 |
|---|---|---|---|---|---|---|
| 基准 盘30／舌30 | 19.922 | −9.460 | +6.257 | 9.983 | 4245 | — |
| 盘31 | 18.587 | **0.903** | +2.757 | 9.983 | 4370 | **净改善**（四条都没翻）|
| 盘32 | −2.900 | **42.369** | **−11.340** | 9.983 | 4410 | 不算（最热铂、净流入都翻了）|
| 盘33 | −3.596 | 54.828 | −14.453 | 10.300 | 4462 | 不算 |
| 盘34 | −2.707 | 65.838 | −17.285 | **11.015** | 4516 | 不算 |
| 盘38 | 8.889 | 97.201 | −23.287 | **12.102** | 4818 | 不算 |
| 舌半宽 29.063 | −7.856 | 32.502 | −9.276 | 9.992 | 4330 | 不算 |
| 舌半宽 28.125 | −20.184 | 54.588 | −16.999 | 9.993 | 4364 | 不算 |
| 舌半宽 26.25 | −37.854 | 91.380 | −29.678 | 9.986 | 4413 | 不算 |
| 舌半宽 22.5 | −69.212 | 161.127 | −53.313 | **11.365** | 4475 | 不算 |

（「净改善」的定义跑前写死：管根下降 ≥ 1.0 K **且** 最热铂 ≤ 限值 **且** 净流入 > 0 **且** 截面 J ≤ 限值。
1.0 K 的门槛依据 §0.-10 实测的复现摆幅 0.015 K 与耦合剩余误差 0.02～0.9 K，大一个量级。）

**变号区间（只报区间，不插值）**：盘径上 **管孔净流入在 31 与 32 之间越过 0**（+2.757 → −11.340），
**管根在 31 与 32 之间越过 5**，**最热铂也在 31 与 32 之间越过 5** —— 三条在同一格里一起翻面。
舌半宽上，从 30 往下**第一小步**（29.063，只窄 0.94 mm）就已经三条一起翻。

⇒ **形状这根杠杆的可用窗口极窄**：盘径 30→31 是唯一的净改善，它把管根买好 **1.334 K**，
代价是把最热铂的余量从 14.460 K 花掉 **10.363 K**、把净流入的余量从 6.257 W 花掉 **3.500 W**。
而管根的缺口是 **14.922 K** —— 按这个兑换率，**买不到**。

⚠ 这张表**没有动法兰侧九根旋钮**（求解器会在每个形状上重新求根）⇒ 它回答的是「形状往哪走热学上会怎样」，
**不是**「这个形状最好能做到多少」。后一个问题由 ③ 回答。

### ③ 搜形状：真跑，按它自己的规则

「◇ 搜形状」的入口 `LineDesignPage.SearchShapeAsync` 是 **private 的 UI 方法**（读页面控件、写输出框），
无头进程调不到。⇒ 探针**不重写规则**：决策全调 `ShapeSearchPlan`（LiveDiscs／Worth／Key）、盘径下界调
`GeometryScreen.MinDiscRadiusMm`（判据 ⑥ 的闭式反解）、每个形状的评估与 `EvalShape` 同一行
（`Solver.Solve(seed, 输入, {AllowTabCuts=false, MaxRounds=16, ScreenCoarseMm=0})`）、并发走 `ShapeBatchEval.RunAsync`、
形状→设计的映射（含「舌长 = 切点 + 压接段 + 自由段下界」）逐位相同。

**先量成本再定跑几个**：第一版按生产第 1 轮网格排了 6 个形状、4 路并发跑 45 分钟 —— **一个点都没跑完**
（最前面那个才走到第 1 轮的第二片）。⇒ 改成跑生产**自己会先去解的那两个点**，让它们真的跑到底：
① `Rsafe` = 第 1 轮网格里最大的盘径（= 35，生产「① 先在盘Ø70 解一次拿板厚」那一步）；
② ⑥ 的闭式反解那一点（生产「② 判据下界给出 盘半径 ≥ … ⇒ 在这里再解一次」那一步），
取**最保守**的 need：板厚顶到上界 6.00 ⇒ 焊脚 6.00 ⇒ need = 管孔 + 6.00，再按 `LiveDiscs` 自己的规则向上取 0.5 格
⇒ 两个档都是**盘半径 32**（W08：25.8+6.0 = 31.8 → 32；W06：25.6+6.0 = 31.6 → 32）。
舌半宽两点都取最宽比例 `fWide = 1.00`（= 盘半径），与界面 `SearchWFrac` 一致。

输出（两个档同机并跑，每边 2 路；判据值是纯函数不带机器争用，「分钟」那一列带）：
- `deliverable/R48_形状探针_搜形状第一轮网格_W08_本次开跑于2026-09-18_102404.txt`（3.00 h，**时间闸切断**，1/2 点跑完）
- `deliverable/R48_形状探针_搜形状第一轮网格_W06_本次开跑于2026-09-18_102407.txt`（2.03 h，2/2 点跑完）

| 档 | 盘半径／舌半宽 | 可行? | 铂重 g | 管根 | 最热铂 | 净流入 W | 舌保温 mm | 板厚 mm | 场解 | 停因 |
|---|---|---|---|---|---|---|---|---|---|---|
| W08 | 35／35 | **不可行** | 4744 | 24.111 | 30.035 | −0.463 | 10.0/4.0/6.5/**20.0** | 6.00/6.00/6.00/1.28 | 62 | 片3 管根：**舌保温已在上界**，槽开不出，本族不挖舌孔 ⇒ 法兰侧穷尽 |
| W08 | 32／32 | **时间闸切断**（3 h，跑到第 3 轮续轮）| — | — | — | — | — | — | — | 见 ④ |
| W06 | 32／32 | **不可行** | 3726 | 16.922 | 23.284 | −1.357 | 10.0/4.5/6.5/**20.0** | 6.00/6.00/6.00/0.89 | 62 | 同上 |
| W06 | 35／35 | **不可行** | 3825 | 45.976 | 43.885 | +0.532 | 15.0/5.5/9.0/**20.0** | 6.00/6.00/6.00/1.31 | 73 | 同上 |

**跑完的三个点解出来的舌保温全部落在 0.5 的格子上**（落格门当场查过）⇒ 不涉及 L 路 09-18 那个二分退回病。

⚠ **新出现的咬点**：三个点的**片3 舌保温都顶到 `SolverOptions.InsHiMm = 20.0`**。
那个 20 在 `Core/Solver.cs` 的 `SolverOptions.InsHiMm`，**写成一个裸数字、没有注释、没有出处**（`ThickHiMm = 6.0` 同排也一样）。
它与现场的缠绕上限**不是一回事**：用户 2026-09-17 说的 20 圈上限只管**圆盘与管端接合区**
（`WrapLimits`，已落地成判据），**舌板与管身「好缠绕」、不设限**。
⇒ 「舌保温上界 20 mm」是求解器约束盒里的一个**无出处的数**。要不要放开是下一位的事；
　**本轮没有放开，理由不是保守，是量过**：往上抬舌保温会让最热铂与净流入一起变差（§0.-9 ③ 的机理，
　本节 ② 那张表也是同一个方向），而这三个点卡住的正是最热铂与净流入 —— **放开它只会把咬点挪得更深**。

### ④ 最接近的那个点：W08 盘32／舌32，四条过三条，卡在最热铂

W08 盘32 那个点被 3 h 时间闸切在第 3 轮，但**轨迹留住了它每一轮的判据**（探针把求解器轨迹进了环形缓冲，
切断时照印）。两轮的数（同一份文件，`§ 求解器轨迹`）：

| 轮 | 板厚 mm | 舌保温 mm | 铂重 g | 管孔净流入 W | 管根低于热偶读数 | 最热铂高出热偶读数 | 法兰截面 J |
|---|---|---|---|---|---|---|---|
| 第 2 轮 | 0.86/1.26/1.26/0.86 | 7.5/3.0/5.0/17.5 | 4279 | **+0.18 过** | **4.79 过** | **21.89 不过（超 16.89）** | 9.98 过 |
| 第 3 轮 | **6.00×4**（顶到上界）| 7.5/3.0/5.0/17.5 | 4620 | +0.25 过 | 4.93 过 | **21.64 不过（超 16.64）** | 9.98 过 |

⇒ **板厚从 0.86/1.26/1.26/0.86 顶到 6.00×4，多买 341 g 铂，只把最热铂拉了 0.25 K。**
环倍率顶到上界 2.50 再拉 0.01 K。**这条判据上，法兰侧的厚度旋钮近乎无效** —— 这不是推理，是这一跑逐行印出来的。

**换到判决网格上再判一次**（输出 `deliverable/R48_形状探针_判决网格复核_W08盘32舌32_本次开跑于2026-09-18_132939.txt`，
12.1 分钟，四次整线解同一进程；设计照上表复原，**先做复原自证**）：

| 轮 | 网格 | 管根 | 最热铂 | 净流入 W | 铂重 g | 判词 |
|---|---|---|---|---|---|---|
| 第 2 轮 | 导航（复原自证：净流入 0.203 vs 轨迹 0.18／最热铂 21.857 vs 21.89／管根 4.833 vs 4.79，**三条都对得上**）| 4.833 | 21.857 | +0.203 | 4279 | 不过：最热铂 |
| 第 2 轮 | **判决网格**（细区 1.000 mm／半径 59.0）| **3.805 过** | **20.753 不过** | **−0.207 不过** | 4279 | 不过：最热铂、净流入 |
| 第 3 轮 | 导航（自证同样对得上）| 5.004 | 21.564 | +0.266 | 4620 | 不过：最热铂、管根 |
| 第 3 轮 | **判决网格** | **4.003 过** | **20.485 不过** | **−0.153 不过** | 4622 | 不过：最热铂、净流入 |

⚠ **「管孔净流入」在两张网格之间变号**（+0.203 → −0.207／+0.266 → −0.153）：它的裕度只有 0.2 W，
而换网格的位移就是这个量级 ⇒ **这条判据在这个点上「过」与「不过」分不开**，只能报「判不了那么细」，
不许拿导航网格那个 +0.2 去说它过了。（同族的病 §0.-10 ⑦ 已经钉在 `MeshVerify` 那张表的注释里。）
⚠ 管根在判决网格上反而更好（4.833 → 3.805），与 §0.-10 ④ 量到的方向一致。

### ⑤ 结论口径（按工单第 5 条写死）

**没有可行形状。** 最接近的是 **W08 盘半径 32／舌半宽 32／舌长 140**（判决网格、第 2 轮那一刻，4279 g）：
- 管根低于热偶读数 **3.805/5 过**（余 1.195 K）
- 法兰截面 J **9.985/11 过**
- 管孔净流入 **−0.207/0 不过**（且裕度与换网格位移同量级 ⇒ **判不了那么细**，不当过）
- 最热铂高出热偶读数 **20.753/5 不过，缺口 15.753 K** ← **就是这一条卡住整件事**

**搜形状穷尽了什么**（不许含糊）：穷尽的是**本轮跑到底的那三个形状**上、**法兰侧九根旋钮**从约束盒下角求根的全过程
（每个点 62～73 次场解）。**没有穷尽**：第 1 轮网格的其余点、生产那条路的 ③ 可行区二分／④ 黄金分割／
⑥ 剩余舌宽比例／⑦ 邻域探索（含翻转锥形）与步长收缩到 0.625 mm 的那一层、挖舌孔那一族、锥形舌边。
⇒ **只能说「生产会先解的那两个点是这个结果」，不能说「盘径与舌半宽这条路走完了」。**

### ⑥ 还开着

1. **缺口在「最热铂高出热偶读数」上，而它的物理来源本轮没拆**。已知的：它随盘径、随舌窄**单调变坏**（② 的表），
   随板厚几乎不动（④ 的两轮）。下一步该问的是「这 20 K 是圆盘上的峰、舌区的峰，还是管根较热端」——
   判据自己记着 `Where`（本轮几个点印的是「入口」），但**逐点位置随设计走，本轮没有逐片拆**。
2. **`SolverOptions.InsHiMm = 20.0` 与 `ThickHiMm = 6.0` 两个界都没有出处**（见 ③）。
   它们现在**真的在咬**（三个点的片3 舌保温都顶到 20）。要放开先补出处，并且先答 ③ 里那句「放开只会把咬点挪得更深」对不对。
3. **判决网格上的那两个点没有重新求根**（本轮只是拿导航网格解出来的旋钮值换网格判了一次）。
   真正的「盘32 最好能做到多少」要在判决网格上重解，本机估 ≥ 3 h（导航网格单点实测 102～122 分钟，判决网格单次场解还慢 1.3 倍）。
4. **W08 盘32 那个点本身没跑到底**（3 h 时间闸切在第 3 轮的续轮）。它还剩 `MaxPartialRounds` 的续轮没走完。
5. **两个档都没跑三关**（① 升温全程／③ 空管到温）——本轮没有一个点过得了 ② 带玻璃稳态，
   跑三关等于给一个不可交付的点补手续。
6. **每片舌保温可行窗口本轮一次都没量**（`InsulWindow.Measure` 没跑）。这是**跑前写死**的：
   解值那一点自己就不过时，绕着它扫量不到窗口 —— §0.-11 ⑥ 已经栽过一次（四片全是「解值自己就不过」）。

### ⑦ 本轮改了什么

**生产代码一行未动。** 新增三个探针文件（都在 `Pt_Optimize.Tests/`，编译 0 错误）：

| 文件 | 用例 | 跑什么 |
|---|---|---|
| `R48SShapeProbeTests.cs` | 1 慢门 | 方向实测（12 点，含网格半径对照）|
| `R48SShapeSearchProbeTests.cs` | 2 慢门（W08／W06 各一）| 按搜形状的规则真跑生产会先解的那两个点 |
| `R48SShapeFineCheckTests.cs` | 1 慢门 | 最接近那个点的复原自证 + 判决网格复核 |

测试计数：`dotnet test` 应为 **1271/1271**（本节 +4；由 `HandoverGateCountTests` 反射数出，2026-09-18）。

⚠ **方向实测那份输出有过一个 v1（同日 08:47:53），已删。** 它的数没错，错的是「一句话」那一段：
只看了管根一列就判「方向有用 ⇒ 往下跑」，而同一份文件里跑前写死的规矩③ 明写「管根变好而最热铂或净流入
翻过限值 ⇒ 记代价，不算净改善」——那四个点的最热铂 65～161 K、净流入全为负。**判词与自己的规矩打架**，
按 §0.-7／§0.-10 的同一条处置（同一件事不留两份带开跑时刻的报告）删掉重跑，v2 另补了盘径细档与网格对照两组。

---

## 0.-11 ★★★★★ R48 U：**温差预算改成工程师填的两项输入；每片舌保温的可行窗口进生产** —— 2026-09-18，Opus 5
...
测试计数：`dotnet test` 应为 **1271/1271**（本轮 +16 = `R48LCoupleTolGateTests` 12 快门
...
测试计数：`dotnet test` 应为 **1271/1271**（本轮 +11 = `R48LQuantInsul05GateTests` 7 快门 ＋ `R48LQuantInsul05EndToEndTests` 2 慢门（W08/W06 端到端 0.5 格）＋ `R48LInsulWindowTests` 2 慢门（W08/W06 舌保温可行窗口））。
...
| `接合区保温缠得出来` | 10 mm（= 20 圈 × 每圈 0.5 mm，`WrapLimits.JointZoneMaxMm`） | **硬判据** | **用户 2026-09-17 现场**：「现场法兰与管保温带一般缠绕20圈以下，再往上很难缠绕(会渐成球形)」「圆盘与管子接触区都是20圈以下，其它地方(舌板与管)好缠绕」。接合区 = 圆盘靠管孔的内环带 ＋ 管子贴法兰的端部段（管保温＋端部额外保温）；圆盘外区、舌板、管身不设限。闭式、纯输入、不吃场。⚠ 越界**不外推**：缠过 20 圈会渐成球形，那个形状本程序没有模型，硬算出来的是「看起来正常的错数」。⚠ 现模型圆盘保温整盘只有一个厚度（分不了内外环）⇒ 整盘按接合区判；管保温沿全长等厚 ⇒ 管端接合区跟着管身厚 | **硬判据**（同左；纯输入，两态逐位相同） |
