...
| R26 | **流水线缺「细网格重解」这一步**（我 09-09 晚发现：Ø56 导航网格全过、加密复算 ③ 11.76 > 10；流水线加密不过后没有在细网格上重解的命令，只有搜形状的精算有） | 09-09 | ✗ 未做（登记）。做法：Flow 加命令 `core.fineResolve`（加密复算不过 ⇒ 从当前解出发 `Solver.Solve(FineMm = 复核网格)`，旋钮只增不减）；`BestShapeFineMeshTests` 已按这条路跑 Ø56 | — |
