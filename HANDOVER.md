...
| R6 | 孔的大小与形状要由**温度场与电流场**决定，不是工程师拍脑袋 | 09-05 | ◐ 已量出「移除优先级 = 导热÷电流」并据此做出圆盘槽旋钮；**舌孔尚未接上同一套判据** |
| R7 | 扩 `steps` 加舌孔参数，**UI 输入也要有这功能** | 09-05 | ◐ `steps` 已扩；**UI 侧孔径控件未做** |
| R8 | 3DM 读出几何后参数化，像 UI 那样任意级数 | 09-05 | ✗ **未做** —— `ShapeToAnalytic` 仍把各级压成一个平均板厚 |
| R9 | 要落进 APP 能自动判断，别停在对话里 | 09-05 | ◐ 槽已走完全链路；**孔未走完** |
...
| ↳ **舌板开孔**（孔位／孔径） | ◐ **算得对但不当旋钮**（2026-09-05）。`FlangePlate.TabHoles` 已接进 `Inside`，电流真绕行、孔越大越挤且单调。**但实测不划算**：挖同样的料，舌片只换到抽热 −0.5 %，圆盘换到 −37 % ⇒ 每 1 % 体积收益差 **60 倍** | `PlateCurrent2D.Inside` |
| ↳ **圆盘背侧减重槽** | ✓ **已是旋钮**（`Knob.SlotSpan`，2026-09-05）。位置由**场**算出（导热÷电流最高处），张角是可二分的标量，上界走闭式（防割断）。实测 27–40 mm/180° ⇒ 抽热 **−42.2 %**、峰值 J 只 +2.9 % | `Solver.Allocation`（③ 那一行） |
...
dotnet build
dotnet test          # 应为 629/629 通过（这个数由 HandoverGateCountTests 自己盯着）
