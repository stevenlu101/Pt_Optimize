...
| 盘R | 舌长 | 种子＝W08 定案 | 种子＝均匀 2.0（中性，即 Pt_Heater1 那块板） | 差 |
|---|---|---|---|---|
...
⚠ **还没做**：0.6 档没跑（本次只跑 0.8）；中性程度只试了均匀 2.0
  —— 均匀 1.0 / 3.0 会落在哪、能不能把 3547 夹出来，未知。

**起点**（`--geom Pt_Heater1.3dm` 实测，不是页面默认）：
盘Ø120（R60）／舌长 200／半宽 60／板厚 **2.0 均匀**／管壁 1.0；单片 **1012 g**。
...
**⚠ 还没做**：把「片名-*」这 5 个图层合成一片的**适配**。没有它，两条路
在定案这个零件上**无法比较**，用户那条「至少非常接近」的判准就还验不了。
这是一个新能力，不是修 bug ⇒ 需要先定，不擅自做。
