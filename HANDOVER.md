...
- **在跑、结果另记**：N 门 c（盘径 28–34 一致门）与门 a 全量；`DrawingPathParityTests`、`R48ClampFaceRecipeTests`、`R48LineDumpTests`（六条记录是 Windows 数，红；只取新 Linux SHA 并与第二跑比）；`R48MStopTolCostTests`（新参照）排在门 c／a 之后。
