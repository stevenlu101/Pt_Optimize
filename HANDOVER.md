...
- **在跑、结果另记**：N 门 c（盘径 28–34 一致门）与门 a 全量；`R48MStopTolTwoPointTests`（决 92：F6 后根又移，预期仍红）、`R48NMeshInjectTests.门f_对拍`（历史基线，预期红、不重录）、`DrawingPathParityTests`、`R48ClampFaceRecipeTests`、`R48LineDumpTests`（六条记录是 Windows 数，红；只取新 Linux SHA 并与第二跑比）。
