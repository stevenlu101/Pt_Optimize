...
> **2026-08-14（最新，读这一段就够）**
>
> **端片曾被判为「结构性死路」的结论已被推翻**，整线出现第一个可交付的量级：
> **2104 g vs 现状 7141 g，省 70.5 %**（`--linefinal`，能量残差 0.0、耦合收敛），
> 其中一段（HC1）尚未过 C2，正由双旋钮自动定尺寸收敛中。
>
> 三条支撑它的新结果：
...
#### ★★ 整线自洽复核：**2104 g，省 70.5 %**（`--linefinal`，第一版）

把单片候选放进 LineRunner（壁 0.6、盘Ø60×0.5、等宽舌片、压接 40 夹 300 °C、含焊缝）：

| 段 | 控温 °C | 电流 A | 管根 ΔT K | 管 J | 克 |
|---|---|---|---|---|---|
| HC1 | 1150 | 842 | **−51.3** ✗ | 8.83 | 614 |
| HC2 | 1080 | 716 | −6.7 | 7.51 | 614 |
| HC3 | 1050 | 667 | +6.5 ✓ | 6.99 | 614 |

**铂重：管 1841 + 法兰 263 = 2104 g（基准 7141 ⇒ 省 70.5 %）**，
四片能量残差全 0.0、外层耦合 6 轮收敛。

**但 HC1 不过 C2**（−51 K = 法兰比管热），且入口片的舌片保温已顶到下界 0.5 mm。
原因清楚：单片筛用的 687 A 是**壁厚 0.4** 时的旧电流，改到焊接下界 0.6 后
I ∝ √壁厚 ⇒ **842 A**，发热 ∝ I² 涨 1.5 倍 ⇒ 该片几何本身发热过头。
按闭式 A/ℓ = I√(ρe/(2kΔT))，842 A 要 A ≈ 37 mm²，而 末30×0.9 只有 27。

⇒ `--linefinal` 已改成**双旋钮自动定尺寸**：先调不花铂的舌片保温，
顶到边界才动舌厚（发热 ∝ 1/t）。这就是把「铂重」交给求解器决定。
...
**用现役实物把系数夹住**：Ø120×2.0 环宽 34 mm、现场焊得出来 ⇒ 真实斜率 ≤ 0.0588。
这**否掉了最保守的取法**（k_b=0.43 给 0.0693，它预言现役件会鼓曲，而没有）。

| 圆盘 | 环宽 mm | 屈曲下界（含 SF 2.0） | 取大后 | 控制机理 |
|---|---|---|---|---|
| Ø120 | 34 | 4.00 | 4.00 | ★ 屈曲 |
| Ø72 | 10 | 1.18 | 1.18 | ★ 屈曲 |
| **Ø60** | **4** | **0.47** | **0.50** | 烧穿 |
...
2. Ø60 以下**下界改由焊接方法决定**：手工 TIG ≈0.5／自动 TIG ≈0.3／激光或电阻缝焊 ≈0.1，**差一个量级**。
3. **舌片与圆盘同板切出、无焊缝 ⇒ 舌厚不受任何焊接下界约束**，只由 §4.3e 的两条电热约束定。两个自由度就此解耦。

> ⚠ 三个烧穿数是行业常规、**非本项目实测**。「用哪种焊接方法」现在**比材料牌号更值钱**。
