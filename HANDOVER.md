...
#### ⑦ ★ 「从 Pt_Heater1 复现定案」怎么跑 —— 方案已定，**尚未跑**（2026-08-25）

用户要求「先用 `Pt_Heater1.3dm` 为例子复现出定案 0.6 / 0.8 的结果」。
方案定下来了，但**因额度收尾没有执行** —— 写在这里，下次照做即可，不必重新推。
...
dotnet build
dotnet test          # 应为 182/182 通过（这个数由 HandoverGateCountTests 自己盯着）
...
> 历史：18（初版）→ 19 → 15（2026-08-12 拔掉一维法兰模型）→ 30 → 31（本条自身）→ 42 → 52 → 80 → 81 → 82 → 89 → 108（判据缺席那一组 19 条）→ 128（判据表对账那一组 20 条）→ 130（出处引用 DocRefTests 两条）→ 138（角焊缝式子收敛）→ 139（禁止行号引用）→ 147（一路变坏就停）→ 157（搜形状每轮决策）→ 159（现场升温判据进表对账）→ 174（形状搜索的种子必须申报）→ 182（定案出处点名的东西必须真的存在）。
>
