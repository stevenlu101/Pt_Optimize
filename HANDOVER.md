...
dotnet build
dotnet test          # 应为 128/128 通过（这个数由 HandoverGateCountTests 自己盯着）
...
> 历史：18（初版）→ 19 → 15（2026-08-12 拔掉一维法兰模型）→ 30 → 31（本条自身）→ 42 → 52 → 80 → 81 → 82 → 89 → 108（判据缺席那一组 19 条）→ 128（判据表对账那一组 20 条）。
>
...
| **UiWiring §17′「制表位严格递增」在空集上恒过** | 写的是 `tRow1.Length > 0 && Enumerable.Range(1, tRow1.Length - 1).All(...)`。制表位只剩 **1 个**时 `Range(1, 0)` 是空集 ⇒ `All` 恒真 ⇒ 断言照打 ✓。而那张测试表是 **3 列**，正常该有 2 个制表位 —— 掉到 1 个是真回归，这条却看不见 | **用户 2026-08-24 指示：先记，先不做**（属排版一族，与本轮判据体系无关，不混进那几条提交）。改法一行：`> 0` 改成 `>= 2`，并把「本表 3 列 ⇒ 至少 2 个制表位」写进 note。同族的空集恒真已在 `RequiredChecksTests.EmptyTable_IsNotSilentlyOk` 有先例 |
