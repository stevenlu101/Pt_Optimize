...
dotnet build
dotnet test          # 应为 89/89 通过（这个数由 HandoverGateCountTests 自己盯着）
...
> 历史：18（初版）→ 19 → 15（2026-08-12 拔掉一维法兰模型）→ 30 → 31（本条自身）→ 42 → 52 → 80 → 81 → 82 → 89。
>
...
| **两条热稳定判据仍是参考量** | 已进判据表；**A / B / E 三段都打**（2026-08-24 起），每轮攒 6 个可用点：A 定案族 10.2 / 8.2 / 44.0 / 41.1×（局部 2.0 / 1.9 / 2.9 / 2.9×）、B「舌长退回 90」18.5× / 2.2×、E 定尺寸器收敛解 10.3× / 2.0×；B 另两个发散探针正确报**判不了** | **有意为之**：限值 1.0 是精确物理，但分布还是窄——6 个点里 5 个仍在定案族附近。**局部那条最小 1.9×，不算宽裕**。再攒些异族几何（用户自己的 .3dm 每跑一次也在攒）之后再谈升硬判据 |
| **① 快筛在设计范围内不可能不过** | 实测扫遍保温 0–150 mm × 壁厚 0.6–1.2 × 目标 600–1400 °C，裕度恒在 **1.43–1.97**（限 1.0），已钉成断言 | 它检的是材料与工作温度的组合，不随可调设计变量变。留着有意义（换牌号或大幅提温时会说话），但**别把它当一道会拦人的闸** |
| **提交钩子约 11 分钟** | 构建 + **单元测试 15 秒**（2026-08-24 补） + UiWiring **145 秒**（§16、§31 各一次真解，§29、§32 各起一次 Rhino）+ selfcheck 7.9 分 | 一度以为要 19 分 —— 其实是 UiWiring **挂住不退**（见 §7 那条）。修掉之后回到 11 分。再要缩只能动 selfcheck，而它是核心回归门，不该为省时间削弱 |
