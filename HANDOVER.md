...
| R29 | **舌片厚分两段（叉臂段厚、杆段薄）—— Y 形成立的前提**（我 09-10 下午由 R23 仪器推出：均匀加厚让导热涨七倍，孔必然更差；用户 09-09 的「舌片必须重新搜形状」要落在两臂上） | 09-10 | ✗ 未做（登记）。做法：舌片加「舌根加厚带」两个量（带长 L_arm、带厚 t_arm）：t_arm = I/(J·带内最窄有效宽)、杆厚 = I/(J·舌宽)，都闭式；场解按位置取厚（`FlangePlate.ThicknessAt` 已按位置）、`SectionSizing` 舌片各处按位置取厚、出图子进程在 x = xT − L_arm 多切一刀（同现有舌片／圆盘分割手法）、3DM 读回认两级；带长由场定（移除优先级最高处到切点）；进候选比价，验收 = Ø56 舌保温封 3 mm 时 ③ 变好且 J 全过 | — |
