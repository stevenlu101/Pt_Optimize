...
## 0.-6 ★★★★★ R48 R 路：**反解剖面在可交付的那张网格上没有窗口**（2026-09-17，Opus 5）

> 用户 09-17 的备选路线二「反向设计」：先定目标温场，从铜排端沿舌板往回积分，反解每段该包多厚保温。
> 做完了，**这条路走不通**：不但没撑开窗口，在可交付的那张网格上把窗口**关掉了**。
> 值钱的不是路线，是它逼出来的两条实测：阶梯剖面对网格敏感 160 倍；窗口宽度由「温差预算」定。

### 结论（一句话）

把舌保温从「每片一个数」换成「沿舌长 5 段剖面」，在**细网格**（细区 1.000 mm／细区半径 59.0 mm，
`MeshVerify.RequiredMeshFor` —— **这是可交付的那张**）上、同一套生产判据、门槛一位没动：

* **反解剖面：δ ∈ [−0.6, 0.0] 步 0.1 扫过的 7 档，一档都不过。**
  卡住它的是「管孔净流入 须为正」：暖到能满足温度判据的那几档净流入一律是负的（−0.02 … −2.03 W）；
  唯一为正的 δ = −0.6（+0.244 W），管根已经掉了 **18.7 K**（限值 5）。
* **均匀单值：δ = 0 那一档仍然过**（净流入 0.343 W／管根 3.826 K／最热铂 4.556 K）。
* 铂重两条**完全相同**（4245 g，保温是纤维不是铂）；保温用量反解比均匀多 **3.6 %**。

⇒ **可交付口径下，反解剖面比均匀单值差，不是好。**（出处：`R48_R_细网格窗口_…163945.txt`、`R48_R_可行点细网格_…162515.txt`）

### ★ 硬发现一：**阶梯剖面对网格比单值敏感 160 倍**

同一档设计、同一份代码，**只换判决网格**，看「最热铂高出热偶读数」：

| | 导航网格 2.0 mm | 细网格 1.000 mm | 差 |
|---|---|---|---|
| 均匀单值 δ = 0 | 4.529 K | 4.556 K | **+0.03 K** |
| 反解剖面 δ = −0.1（导航网格上那个「可行点」） | 4.399 K | 9.130 K | **+4.73 K** |

**差 160 倍。** 原因看得见：反解剖面在**贴着圆盘的头 20 mm** 上放了 9.4 mm 保温，而全片最热的地方就在那里；
5 段之间的厚度是**阶跃**，阶跃正好压在热点上，2 mm 的导航网格分辨不出来。

⇒ **分段剖面这个做法本身，把「导航网格能不能判」这件事变糟了。**
⇒ **导航网格上关于剖面的任何数，都要按这一条折算** —— 包括本节下面所有标「导航网格」的数。

### ★ 导航网格上那个「窗口宽 1.52 倍」：**已撤，未结案**

导航网格上两条剖面各只过一档（0.1 mm 步进），按两条判据线性内插得 **0.021 mm（均匀）／0.032 mm（反解剖面）**，
曾据此写成「反解宽 1.52 倍」。**这个比值本轮判不出来，已撤**，两条理由各自就够：

1. **上面那条 160 倍**：导航网格对**阶梯**剖面判不准，拿它比两条剖面本来就不公平；
2. **跨树差比窗口还大**：同一份设计、同一张导航网格，本树锚点的「管根低于热偶读数」是 **4.922 K**，
   而 r48_L 2026-09-17 **09:30** 那份（`r48_L/deliverable/R48_L_端到端_细网格_W08_本次开跑于2026-09-17_093013.txt` 的 D 段）是 **4.511 K**，
   差 **0.411 K**（差因：段间停机那批改动没合进本树，本轮没有追）。
   按本轮实测的当地灵敏度（均匀那条：管根 8.781 K @ δ = −0.1 → 4.922 K @ δ = 0 ⇒ 38.6 K/mm），
   0.411 K 相当于 δ 挪 **0.0107 mm** —— 是均匀那条窗口（0.021 mm）的**一半**、反解那条（0.032 mm）的**三分之一**。
   ⇒ **1.52 倍落在跨树差的量级之内。未结案。**

⚠ 细网格那一列两棵树是**对得上**的：均匀单值 0.343 W／3.826 K／4.556 K 与 r48_L 同一份 D 段**逐位相同**。
⚠ r48_L 那份自己就写着这句：管根这条判据「**导航网格不可用于该判据**」（细网格 vs 导航网格裕度差 **2.40 倍**，门槛 2.0）。
  本节凡是导航网格上的管根数，都在这句话的效力范围内。

### ★ 硬发现二：窗口宽度的第一性原理账（导航网格上量的，按上面两条折算着读）

判据允许的**总预算**是 **5 + 5 = 10 K**（「管根低于热偶读数 ≤ 5」＋「最热铂高出热偶读数 ≤ 5」）。
实测这两个值**之和**随保温厚度是一条 V 形曲线，最低点 **9.45 K（均匀）／9.18 K（反解剖面）**，
只比预算低 0.55／0.82 K，而这个和每偏离 0.1 mm 就涨 1.5～1.8 K。

```
窗口宽度 ~ （10 K 总预算 − 两条温差之和的最低点）÷ 这个和对厚度的斜率（实测 15～17.5 K/mm）
```

**预算已经用满，剩下那 0.5～0.8 K 就是全部的窗口。**
⇒ 分母（斜率）由「舌板是两个大数之差」这个结构定，换保温分布动不了；
  分子（0.55～0.82 K）才是能争的 —— 要让**法兰整体更贴近热偶读数**（改几何，或把夹头温度与保温联立求解）。
  **继续在保温分布上找窗口，是在分母上使劲，方向不对。**

### 顺带钉死的三件事（都带实测）

1. **判据主要认「平均包多厚」。** 二维把水平定完之后（二分一个整体倍率 k = 1.4688，只动一个标量、形状一位没动），
   反解剖面的平均厚度与求解器那个单值几乎相等（5.20 / 2.32 / 3.66 / 11.00 vs 5.1 / 2.3 / 3.6 / 10.4 mm）；
   导航网格上两条剖面对 δ 的灵敏度也只差 19.6 %（−8.03 vs −9.61 W/mm，贴着 20 % 的门槛过线）。
   ⚠ **形状不是完全没影响** —— 它的影响主要露在**网格敏感度**上（就是上面那条 160 倍），而不是露在判据值本身。
2. **「只动热端那一段」不是一根有用的旋钮**：热端 20 mm 单独动 ±1 mm，管孔净流入只走 −0.30 W/mm，
   比整条平移的 −9.61 W/mm 弱 32 倍（导航网格；这一模式**没有**在细网格上扫过）。
3. **夹头温度是一根很猛的旋钮，但方向与设想相反**：同一份设计把夹头从 450 降到 200／100 °C，
   管根低于热偶读数从 4.92 → 69.5 → 98.5 K（夹头越冷抽得越狠）。
   ⚠ 这只说明「夹头温度必须与保温联立求解」，**不**说明「夹头降温这条路不通」—— 换夹头温度要重跑求解器，本轮没跑。

### ⚠ 引用本轮的数之前，先看这三个折扣

1. **导航网格上的剖面数**：按「160 倍」与「跨树差 0.411 K」两条折算（见上）。
2. **一维给的是方向，不是水平，而且连方向都带一份偏差。** 一维把整片板压成一根 60 mm 宽的等截面杆，
   x ∈ [0, −30] 那半个圆盘（1414 mm²，占一维所设舌板面积的 **24 %**、而且正是全片最热、发热与散热都最大的那一块）
   在二维里算**圆盘**、在一维里被算进了杆。补偿办法是逐片乘**一个**散热等效系数（1.4678／1.4898／1.6206／1.7490）——
   **缺的面积集中在热端，倍率却按同一个数摊到全长**。⇒ 一维解出来的「热端厚、冷端裸」这个形状，
   **一半来自跑前写死的 T4 选法**（在解族里指定「冷端恰好裸铂」那一条），**另一半带着这份 SurfaceScale 偏差**。
   形状结论**不是实测**，别当实测引。标定后的分项仍差 0.83／1.36／1.91 倍（进夹／焦耳热／散热），绝对量一律不可引用。
3. **「真实窗口 0.021／0.032 mm」是线性内插，不是实测格点**（实测只有 0.1 mm 分辨率，两个模式各只过一档）。

### 代码上留下了什么（默认关闭，默认路径逐位不变）

| 东西 | 在哪 |
|---|---|
| 沿舌轴 x 的分段常值舌保温剖面 | `Core/TabInsulProfile.cs`（新） |
| 舌板一维稳态反解（RK4 + 二分打靶 / 逐点反查厚度） | `Core/TabReverse1D.cs`（新） |
| 板件带剖面 | `FlangePlate.TabInsulProfile` |
| 图纸路径逐片剖面 | `LineCase.TabInsulProfile3dmPerPlate` / `TabInsulProfile3dmAt` |
| 热解输入带剖面 + **集总口径** | `PlateThermalSetup.TabInsulProfile` / `TabInsulLumpedMm` |
| 场解逐格按形心 x 取厚度 | `ShellThermal.Solve(tabInsulProfile:)` |
| **逐格诊断**（本格实际用的厚度／实际用的舌面表指纹） | `ShellThermalResult.TabInsulMmCell` / `TabSurfaceQProbeCell` / `TabSurfaceQProbeC`（剖面为 null ⇒ 空数组） |
| 门 **10 条**（含「全段等厚 ≡ 单值**逐位相同**」与**逐格取值**那一道） | `Pt_Optimize.Tests/R48TabInsulProfileGateTests.cs`（进快速套件） |

**剖面**默认 `null` ⇒ 走单值那条原路径，逐位不变。散热配方一字没动（仍是 `DesignScreen.PlateFluxWPerM2` 唯一配方）。
⚠ 集总模型（整片热稳定、升温两节点）整片只有一个数、吃不下剖面 ⇒ 给它们的是**沿舌长按长度加权的平均厚度**（有意的近似，判据说明里写明了）。
⚠ 剖面**还没进** `DesignSpec` / `.fd.json` 存档 / 界面 —— 结论是这条路撑不开窗口，所以暂不接线；
  将来要用它（省保温、或配合别的路线）时，接线要同时补设计记录往返与界面。

### ★ 门没守住的那一处，已经补上（2026-09-17，Opus 5；把关第二轮）

原来九条门守住了三件事（默认路径逐位不变／新状态位真的接到下游／不许静默换口径），**没有一条守「这一格取的是不是它自己那一段」**。
实测：把 `ShellThermal.Solve` 里 `TabTableAt`／`TabInsulMmAt` 的**段号注入 +1**（整条剖面错位一段，越界钳在末段），

* 九条门**全绿**；
* 同一算例的判据早就被改掉了 —— 片 0 单片场解上，9.1→1.1 斜坡剖面的**管孔净流入 1.371 → 29.004 W**、舌面散热 201.9 → 236.8 W；
  冷端剥光那条 5.147 → 14.475 W；全段等厚那条 2.563 W **一位不动**（等厚时错位看不见 ⇒ 门 c 天生抓不住这个病）。
  把关人在反解线那条剖面上量到的是 0.449 → 29.23 W，同一个病、不同算例。

⇒ 补**门 i（逐格取值）**：非均匀剖面下，**每一格**的舌保温厚度必须 == 剖面 `At(本格形心 x)`，
  这一格实际用的舌面热流表必须 == 第 `IndexAt(x)` 段那一张（拿**生产**那份 `SurfaceLossTablesFor` 建，不手抄配方）。
  带两条反空转：各段的表必须两两不同（否则错位看不出来）、每一段都必须被比到过。
  **加门之后同一份注入：九条仍绿、门 i 当场红**（「第 210 格 形心 x = −71.799 实际 1 mm，按 `At(x)` 该是 3.1 mm」）。
  实测逐格比过 756/1006 格，各段格数 12/204/240/90/210（圆盘保温格与分界圆混合格不比，那两类另有门守）。

### 两个前提被实测推翻，记下来免得下次再照着写

1. **「夹头导热从 191 W 反推」不成立** —— 生产算例的舌端是**定温**边界（`FlangeOut.BusGWPerK = −1`，
   压接段面积加权均温逐位 = 夹头温度 450.000 °C），不是热导边界。191 W 是**结果**，只能当对拍量。
2. **`R48LineDumpTests` 这个测试类在 `D:\WinForm\r48_I` 里，只是**没有合入本树**。
   （先前本节与门文件写「本仓库里不存在」—— 那句只在**本工作树**范围内成立，写成「不存在」是把范围说没了，已改。）
   本树跑不了它 ⇒ 本轮改用「全段等厚 ≡ 单值逐位相同」＋ **逐格取值门** ＋ 快速套件整体不变 ＋ 锚点复现。
   ⚠ **合入时那六个算例的 SHA 必然要重记**：它的记录按「`LineCase`／`SegmentOut`／`FlangeOut`／`DesignInputs` 的
   **公开成员集合**」算，而本轮新加的公开成员（`FlangePlate.TabInsulProfile`、`LineCase.TabInsulProfile3dmPerPlate`、
   `PlateThermalSetup.TabInsulProfile`／`TabInsulLumpedMm`、`ShellThermalResult.TabInsulProfileNote`／`TabInsulMmCell`／
   `TabSurfaceQProbeCell`／`TabSurfaceQProbeC`）全在转储范围内 ⇒ 六个「去文字」SHA 一个都不会保持原值。
   那**不是数变了**（默认路径逐位不变由门 c 与快套件守）；重记时按该类注释的规矩写「**旧值 → 新值 ＋ 依据**」。

### ⚠ 跑一次快速套件会改写 12 份**受追踪**的 deliverable

实测（2026-09-17 18:07～18:10，本树）：`dotnet test --filter "速度!=慢"` 跑完，**12 份受追踪的 deliverable 被重写**，
其中 **8 份内容真的变了**（`md5` 比对）：`R47_第三轮N1_倒角实验_2026-09-13.txt`、`R48_舌根热点的边界条件_2026-09-14.txt`、
`开孔的电流代价.txt`、`孔形对比.txt`、`孔位对比.txt`、`移除优先级.txt`、`按场开槽_效果.txt`、`形状族_对比.txt`；
另 4 份重写后逐字相同（`四片为什么等厚.txt`、`盘径不动点.txt`、`R48_网格结构对比_2026-09-13.txt`、`舌片电流密度分布.txt`）。
⇒ **「快套件整体不变」这条证据只管测试结果，不管 deliverable 目录**；提交前必须看 `git status`，
  否则会把「跑测试的副产品」当成本轮的交付一起提交。

### 出处

`deliverable/R48_R_反向设计_本次开跑于2026-09-17_154512.md`（总报告，含三张 PNG）、
`R48_R_一维反解_…145857.txt`、`R48_R_二维验证_…150517.txt`、`R48_R_可行点细网格_…162515.txt`、
`R48_R_细网格窗口_…163945.txt`、`R48_R_窗口_…151606.txt`、`R48_R_夹头温度_…154126.txt`。
⚠ 总报告的文件名时刻（15:45:12）**早于**它内含的两跑（R3b 16:25:15、R3c 16:39:45）——
  那个时刻是**开始写报告**的时刻，不是最后一次实测的时刻；读的时候以各节点名的证据文件为准。
本轮**没有重跑求解器**，旋钮一律取 r48_L 2026-09-17 01:27 那次的终值。　注记：2026-09-17，Opus 5。
...
| `⑦ 最热铂高出热偶读数`（第 j 片 max(圆盘峰, 舌片区峰, 接头管根较热端) − 热偶读数基准） | 5 K | **硬判据** | **R48 B（用户 2026-09-14 定，2026-09-14 Opus 5 落地）**：控温热偶在段中点、读数误差 5 ℃（`LineCase.ThermocoupleErrorK`，限值 `HotOverTcMaxK` 默认取它）；「10 ℃ 是上下各 5 ℃」。端片基准 = 本段设定；共用片基准 = 两侧设定的**开尔文对数平均**（`LineSolver.ThermocoupleReferenceC`，1150/1080 → 1114.71 °C）。物理把关人核过：只许用设定值，不用模型交界管温（会被设计变量挪走）。判据说明并列「模型算的无法兰交界管温」，与基准差超过 `ThermocoupleBasis.ModelGapNoteK`（1 ℃）时写明。逐片读数唯一来源 `ThermocoupleBasis`。**新代号 ⑦**（②″ 不换主人） |
| `⑧ 管根低于热偶读数`（热偶读数基准 − 接头管根较冷端） | 5 K | **硬判据** | 同一个热偶误差的另一侧（`ColdUnderTcMaxK` 默认取 `ThermocoupleErrorK`）。**取代**「法兰增量温降 ≤ 10（目标）」并**升为硬线**；基准同上一行。与旧判法的差别：不再剔除控温点梯度（基准是设定值算的，不随设计变量挪动，所以不是够不着的靶）。**新代号 ⑧**（③ 不换主人） |
| `②″圆盘区最高温 − 管温（旧判法）` | 5 K | 参考 | 2026-09-14 前的硬判据（基准是模型算的贴着的管根温度）；限值出处 = 现场控温精度（2026-08-15）。现只作对照，不卡交付。代号 ②″ 含义不变 |
| `③ 法兰增量温降（旧判法）` | 10 K | 参考 | 2026-09-14 前的目标（= 无法兰基线 − 实际）；10 K 贴着热偶误差定（2026-08-15），2026-09-14 拆成上下各 5 K 进 ⑦／⑧。现只作对照，不卡交付。代号 ③ 含义不变 |
...
| `· 升温期法兰−管峰值` | 215 K | 参考 | **现场升温工况**（温控 20 K/h、空管）下「法兰温度 − 管温」的全程最大值。★ 215 **不是通过线，是现役基准** ——用户 2026-08-25 确认 §4.2s 那一行（起始 71 A、+215 K、出现在 42.6 h）「比较符合现状」，拿它去判现役等于判掉自己。⚠ 判据 ① 用的 `RampSolver` 把法兰并进管子当**同一个温度**，「法兰比管热」在那个模型里**结构性地看不见**（RampTwoNode 的类注释写着这句），本条改用两节点模型补上。要升硬判据得先有限值的出处。★ R48 G2 起（2026-09-15 Opus 5（合并） 补记）：物理把关人复核前本行**暂不给数**（判不了、不给实际值，LineRunner.Judge 与 RampPendingNote）—— 两节点模型已补铜排通道与舌片保温、四项按稳态场逐项标定，但标定只在标定点上成立；标定所引的数都在 F 合入前配方下测（r48_G2 工作树），合并树未重跑。下面的实测分布是 2026-08-25 老模型的，留作历史。**实测分布（2026-08-25 首轮）**：现役设计记录 **0.8 档 +174 K、0.6 档 +206 K**；已作废两档 +274 / +318 K；发散探针（板厚 ×0.5、管保温 1 mm）+581 / +591 K。⇒ **现役 < 已作废 < 发散**，排序干净，说明它量到的是真东西 |
| `· ② 法兰最高温` − 管温（整片，含舌片） | 0 K | 参考 | 判定口径已由 ②″ 接手（舌片包保温后峰值落在舌片，与管根不可比）；本条留着盯舌片熔点与局部失稳 |
| `法兰截面 J` | 11 A/mm²（= 设定 J 10 + 1；J 由工程师在 ① 输入设定、预设 10，实况风险工程师承担 —— 用户 2026-09-09，R22） | **硬判据** | **用户 2026-09-08 设计因果链第 ④ 步**：设计电流（温控 20 °C/h 空管升温全程峰值，共用片矢量合成）÷ 必经截面积（舌片各处含开孔／舌盘交界弦／孔缘环与各级环含槽带，圆盘按整圈）。按设定 J 定尺寸（约束盒下角），终验全体 < J+1。闭式、与网格无关 |
| `· 法兰 J_max` | 10 A/mm² | 参考 | 场的**逐点峰值**（凹角处随网格涨、无收敛平台）—— 2026-09-08 起是诊断量，给「电流密度低处定孔」用；前任把它当判据追网格 ⇒ 永远判不了，已推翻（用户第 8 条） |
