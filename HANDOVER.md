...
dotnet build
dotnet test          # 应为 52/52 通过（这个数由 HandoverGateCountTests 自己盯着）
...
| **★ 一段从未被执行过的代码，藏了两个静默错和一句假的性能承诺**（2026-08-23） | `SolveByLevel` 的 `makePlateByLevel`（解析路径）**没有任何调用方**。接线时逐行查，里面有：① 片序号靠闭包里的 `callIdx++ % 片数` 推 —— 只在「每轮恰好按序调 nf 次」时才对，多调少调一次，第 2 片就拿到第 3 片的级配比；② 基准级厚一律取 `levelThicknessMm[0]`，逐片基准不同时其余三片全按第 1 片算。两者都不抛异常，只让优化器对着**另一片板**求解。③ 它的文档写着「不经 Rhino，**快一个量级**」—— 实测每片 3.1 ms（.3dm）vs 2.7 ms（解析），**只差 13 %**；`LoadThickness` 本来就带缓存，Geom 子进程每个文件只起一次 | 死代码的注释同样会烂，而且**没有任何东西会去证伪它** —— 它连跑都没跑过。接线前先当它是错的：片序号这种东西**根本不该由闭包里的计数器推**，让调用方显式传（契约从 `Func<double,FlangePlate>` 改成 `Func<double,int,FlangePlate>`，编译器把 8 处调用点逐个点出来）。性能承诺一律先量再信：这一句让我自己在上一轮把「解析路径没接上」写成了「④ 慢的结构性成因」，**那个判断是错的，已收回** |
...
> 历史：18（初版）→ 19 → 15（2026-08-12 拔掉一维法兰模型）→ 30 → 31（本条自身）→ 42 → 52。
>
