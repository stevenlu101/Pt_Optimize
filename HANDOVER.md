...
| R8 | 3DM 读出几何后参数化，像 UI 那样任意级数 | 09-05 | ✗ **未做** —— `ShapeToAnalytic` 仍把各级压成一个平均板厚 | `ShapeToAnalyticTests` ⇒ **无门**（钉的是「压成平均板厚」的现状，不会因为没做而红） |
...
| R11 | **t0 舌片厚度拆成独立逐片旋钮**（现在舌片与圆盘同一个「板厚」；`DesignSpec.Plate` 写 `TabThicknessMm = NaN`）。2026-09-08 用户答复：可以 | 09-08 | ✗ 未做 | **无门**（待建：门 B 六环 Fit／存档／快照／界面／出图／变更清单 + 候选表 + 单调性实测） |
| R12 | **舌板孔心位置由场逐案自动定**（移除优先级 = 导热贡献÷电流密度 最高处），不是默认舌片自由段中点 | 09-08 | ✗ 未做（`TabHoleXMm` 只能手填；开发计划第 3 件） | **无门** |
| R13 | **离散形状族入候选**：圆角三角／方；**长椭圆与弯椭圆（用户：仅用于法兰）** | 09-08 | ✗ 未做（`HolePlacementTests`／`FieldGuidedSlotTests` 量过部分形状，未入候选） | **无门** |
| R14 | **3DM 路先做 R8**：读入多级厚度不压平均，逐级成为 t1/t2 那套旋钮、走求根（与 R8 同一件事，用户 09-08 确认优先） | 09-08 | ✗ 未做 | 与 R8 同：**无门** |
| R15 | 「能造」清单补一条：**孔径 < 1 mm 的孔不考虑**（最小孔径 1 mm）。其余制造约束用户未再加 | 09-08 | ✗ 未做（孔径旋钮下界现为 0/「用途最小孔」，二分可落到 <1 mm） | **无门** |
...
| R18 | **法兰 J 判不了要判得了** —— 2026-09-08 下午用户给了**设计因果链**，R18 按它重排：① 20 °C/h 升温下所需的管电流（功率）→ ② 按 **J = 10** 定法兰 r1/t1/r2/t2 与舌片横截面积 → ③ 分析舌片与法兰温场/电场，电流密度低处定孔的形状与位置 → ④ 复核温场与电流场，**全体截面 J < 11**。用户原话：「如果横截面该加大就要加大，首先是可造可用，J 20–37 已完全不可用；电流使用的大小应该与铂金管升温速率有关」。实测 0.8 档（3480.7 g）舌根 J_max 37、热点 +99 K ⇒ 那份「全过」不可用 | 09-08 | ◐ **第一阶段已做**（2026-09-08 晚）：① `Core/DesignCurrent.cs` 由 `RampTwoNode` 温控 20 °C/h 空管升温（全线目标 1150）算每段峰值电流、共用片矢量合成，求解器轨迹印出；② `Core/SectionSizing.cs` 闭式截面（舌片各处含开孔弦／舌盘交界弦／孔缘环与各级环含槽带，圆盘按整圈）⇒ 板厚下界 = t×max(J/10)，进约束盒下角（`Solver.ApplySectionFloor`，每轮重算、只增不减、留痕 `★ 下角因 J=10 截面上抬`），孔径/槽张角上界也受 J 约束；④ 新硬判据 `法兰截面 J`（< 11，闭式，当场判得了）进 `Required` 与判据表，场逐点峰值 `· 法兰 J_max` 降为参考量。**未做**：③ 场定孔（R12）；0.8 档在新链下的重解正在跑（2 段） | 验收 = ① 设计电流由 20 °C/h 升温算出并印出 ✓；② 截面由 I/10 闭式定出成为下角 ✓（`BranchMarksAreReachedTests.下角因J10截面上抬`）；④ 截面 J 行能判过/不过 ✓（`SectionSizingTests`、`CriteriaTableTests`）；0.8 档（2 段）在新链下重解出可交付解 ⇒ 待本次验算 |
| R19 | **3DM 模式下熔化交棒要有出口**（现在厚度到顶仍熔 ⇒ 交棒「◇ 搜形状」，但 3DM 模式搜形状用不了 ⇒ 死路）。出口 = Flow.cs 里记的那条：把 3DM 反推的形状交给解析路（与 R14/R8 同一条线） | 09-08 | ✗ 未做 | 验收 = `UiWiring --follow3dm Pt_Heater1.3dm` 走到熔化交棒时下一步可点、能改盘径/舌宽；门待建 |
| R20 | **升温判据 ① 进导航**（现在只在终局复核算，最后才发现不可行）；**并且 ① 要判「升温期会不会烧断」**：现在 ① 只判「72 h 内到得了目标」，升温峰值温度 `RampSolver.TPeakC` 算了却没拿去和熔点比（用户 09-08 问「A 的工况下还会烧断吗」暴出来的） | 09-08 | ✗ 未做 | 验收 = 导航网格每轮的判据里有 ①（或有廉价的 ① 前筛）；① 的判定含「升温峰值 < 熔点」；0.8 档对帐逐字不变或说明变了什么；成本先估（RampSolver 每次解多几秒） |
