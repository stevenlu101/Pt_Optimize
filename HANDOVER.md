...
| **二分次数不够，误报「无解」** | `--sweep2` 48 例全报不可行，落点在 ±28…±255 K 而不是靶值 +5 K。查 `--tscan` 发现单调性**是成立的**，只是 ΔT 对法兰厚度的斜率高达 **≈1300 K/mm** ⇒ 7 次二分（分辨率 0.028 mm）对应 ΔT 不确定度 ±37 K，根本落不进 10 K 的窗口 | 二分前先估**灵敏度**：所需迭代数 = log₂(区间/所需分辨率)。这里要 16 次。**报「无解」前先确认求根本身收敛到了靶值**——落点离靶值远就是求根没做完，不是无解 |
