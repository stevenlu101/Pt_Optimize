...
dotnet build
dotnet test          # 应为 129/129 通过（这个数由 HandoverGateCountTests 自己盯着）
...
> 历史：18（初版）→ 19 → 15（2026-08-12 拔掉一维法兰模型）→ 30 → 31（本条自身）→ 42 → 52 → 80 → 81 → 82 → 89 → 108（判据缺席那一组 19 条）→ 128（判据表对账那一组 20 条）→ 129（出处引用 DocRefTests）。
>
