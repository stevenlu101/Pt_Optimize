...
| # | 用户要求（原话摘要） | 提出 | 状态 |
|---|---|---|---|
| R1 | 段数由 UI 输入决定，UI 与 3DM 都适用 | 09-03 | ✅ 已做，4 段实测跑通 |
| R2 | 每段直接加热铂金管的长度可单独设定 | 09-03 | ✅ 已做，段表新增该列 |
| R3 | 过热 ⇒ 加大截面（增厚或增宽），APP 要给出「能用且铂最省」的方案，不许一句话判死刑 | 09-04 | ◐ 加厚已实测化；增宽 = 搜形状已改求根；**「最省」仍未端到端验收** |
| R4 | 自动定厚的 t₁/t₂ 要与搜形状的 r₁/r₂ **成对** | 09-04 | ✅ 已做（t=1 时结构性休眠，由实测淘汰） |
| R5 | **舌板开孔**：尺寸、**孔径**、厚度都要能优化 | 09-05 | ✅ **走完整条链**：旋钮 `Knob.TabHoleR`（上界走桥宽闭式）／求解／界面逐片控件／出图／存档／快照／变更清单。证据：`TabHoleTests` 7 条，含**真写图再读回**（孔心 0、孔外有料）与真解电流场（孔越大越挤且单调） |
| R6 | 孔的大小与形状要由**温度场与电流场**决定，不是工程师拍脑袋 | 09-05 | ◐ 大小已是旋钮（孔径／槽张角）；形状的**连续维**（顺流拉长比、弯椭圆）已入旋钮或成为默认，依据都是等面积实测；**离散形状族（圆角三角／方）与孔位**已量过、未入旋钮，理由与选项已给用户 |
| R7 | 扩 `steps` 加舌孔参数，**UI 输入也要有这功能** | 09-05 | ✅ `steps` 已扩；UI 侧**孔径/拉长/槽张角逐片控件都已做**（`LineDesignPage` 的 `_holeR`／`_holeAsp`／`_slotDeg`，建控件→进页面→写回 DesignSpec 三处齐全）。⚠ 2026-09-07 督导 S7b 指出此格原写「UI 侧孔径控件未做」= **低报**，与同表 R5 自相矛盾；低报与高报一样算「搞混」 |
| R8 | 3DM 读出几何后参数化，像 UI 那样任意级数 | 09-05 | ✗ **未做** —— `ShapeToAnalytic` 仍把各级压成一个平均板厚 |
| R9 | 要落进 APP 能自动判断，别停在对话里 | 09-05 | ◐ 槽与孔（孔径／顺流拉长）都已走完全链路：旋钮→求解→界面→出图→存档→变更清单 |
| R10 | **最有效的是组合**（例：圆盘槽 + 舌板椭圆孔），要在**不同位置 × 不同孔形**的组合里搜出来 | 09-05 | ◐ **跨轮累积已实测成立**（`ComboAccumulateTests` 行为门，1 h 21 m 真跑：求解器自己攒出 ≥2 根旋钮）。**协同仍未搜**：`ChooseKnob` 依旧一轮挑一根，「两样一起才有效、各自单独无效」那种找不到；孔的位置与离散形状也还不是候选 ⇒ 计划 7.5 |
...
dotnet build
dotnet test --filter "速度!=慢"   # 快门 670 条，实测 1 m 25 s（提交钩子跑的就是它）
dotnet test --filter "速度=慢"    # 慢门 9 条：真跑场解/出图，合计数小时，发版前跑
...
`dotnet test` 应为 **682/682**（2026-09-07 由 HandoverGateCountTests 反射数出，与督导独立复核一致）。
